...
import com.alianga.jkit.sql.auto.boot2.fixture.BootUser;
...
/**
 * Spring Boot 2 自动配置在 H2 上建表。
 *
 * @author 郑明亮
 */
public class SqlAutoBoot2Test {

    @Test
    public void createsTableOnStartup() throws Exception {
        String url = "jdbc:h2:mem:boot2_" + UUID.randomUUID().toString().replace("-", "")
        ...
        try {
            ...
            try {
                ResultSet rs = c.getMetaData().getTables(null, null, "BOOT2_USER", new String[] {"TABLE"});
                try {
                    assertTrue("BOOT2_USER should exist", rs.next());
                } finally {
                    rs.close();
                }
            } finally {
