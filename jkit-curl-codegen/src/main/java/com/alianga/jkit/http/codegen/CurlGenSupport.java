...
public final class CurlGenSupport {
    ...
    /**
     * 过滤掉 {@code Content-Length}，避免生成的代码与客户端实际计算的长度打架。
     *
     * @param req 解析模型
     * @return 含认证信息、且不含 Content-Length 的请求头
     */
    public static List<Header> visibleHeaders(ParsedCurlRequest req) {
        List<Header> all = headersWithAuth(req);
