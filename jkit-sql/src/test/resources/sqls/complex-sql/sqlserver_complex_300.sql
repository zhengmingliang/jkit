-- ==============================================================================
-- SQL Server 复杂业务 SQL 300 条
-- 适用版本：SQL Server 2019+
-- ==============================================================================
--
-- 内容说明：
...
WITH prod_city AS (
    ...
    WHERE o.status = 'completed'
      AND o.order_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
...
WITH store_sales AS (
    ...
    WHERE o.status = 'completed'
      AND YEAR(o.order_date) = YEAR(CAST(GETDATE() AS DATE))
...
py AS (
    ...
    JOIN orders   o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
      AND YEAR(o.order_date) = YEAR(CAST(GETDATE() AS DATE)) - 1
...
JOIN categories c ON p.category_id = c.category_id
WHERE o.status = 'completed'
  AND o.order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
-- ------------------------------------------------------------------------------
WITH pm AS (
    SELECT oi.product_id, FORMAT(o.order_date, 'yyyy-MM') AS ym,
           SUM(oi.amount) AS amt
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
      AND o.order_date >= DATEADD(day, -120, CAST(GETDATE() AS DATE))
    GROUP BY oi.product_id, FORMAT(o.order_date, 'yyyy-MM')
),
rk AS (
...
FROM rk cur
LEFT JOIN rk prev ON cur.product_id = prev.product_id AND prev.ym = FORMAT(DATEADD(month, -1, DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1)), 'yyyy-MM')
JOIN products p ON cur.product_id = p.product_id
WHERE cur.ym = FORMAT(DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1), 'yyyy-MM')
...
rf AS (
    ...
    JOIN order_items oi ON o.order_id = oi.order_id
    JOIN products p ON oi.product_id = p.product_id
    WHERE r.refund_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
    GROUP BY p.category_id
)
...
-- ------------------------------------------------------------------------------
SELECT DATEPART(hour, o.order_date) AS hour_of_day,
       CASE WHEN (DATEPART(weekday, o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END AS day_type,
       COUNT(*)                  AS order_cnt,
       SUM(o.pay_amount)         AS gmv,
...
GROUP BY DATEPART(hour, o.order_date),
         CASE WHEN (DATEPART(weekday, o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END
...
WITH dur AS (
    SELECT o.order_id, s.province, s.carrier,
           DATEDIFF(day, o.order_date, s.ship_date) AS deliver_days
    FROM orders o
...
grp AS (
    SELECT customer_id, dt,
           DATEDIFF(day, CAST('2000-01-01' AS DATE), dt)
...
WITH date_seq AS (
    ...
    UNION ALL
    SELECT CAST(DATEADD(day, 1, dt) AS DATE) FROM date_seq WHERE dt < CAST('2024-03-31' AS DATE)
),
daily AS (
...
SELECT CASE WHEN seq = 1 AND next_order_date IS NOT NULL THEN 'has_second' ELSE 'one_time' END AS cust_kind,
       COUNT(*) AS cnt,
       ROUND(AVG(DATEDIFF(day, order_date, next_order_date)), 2) AS avg_gap_days,
       MIN(DATEDIFF(day, order_date, next_order_date)) AS min_gap_days,
       MAX(DATEDIFF(day, order_date, next_order_date)) AS max_gap_days
FROM ranked
WHERE seq = 1
...
SELECT p.product_id, p.product_name, c.category_name, p.launch_date,
       ...
       DATEDIFF(day, COALESCE(ls.last_sold_date, p.launch_date), CAST(GETDATE() AS DATE)) AS idle_days,
       CASE WHEN ls.product_id IS NULL THEN 'never_sold'
            WHEN DATEDIFF(day, ls.last_sold_date, CAST(GETDATE() AS DATE)) > 90 THEN 'long_idle'
            ELSE 'active' END AS sale_status
FROM products p
LEFT JOIN last_sale ls ON p.product_id = ls.product_id
LEFT JOIN categories c ON p.category_id = c.category_id
WHERE p.status = 'on_sale'
  AND (ls.product_id IS NULL OR DATEDIFF(day, ls.last_sold_date, CAST(GETDATE() AS DATE)) > 90)
...
-- ------------------------------------------------------------------------------
WITH pm AS (
    SELECT oi.product_id, FORMAT(o.order_date, 'yyyy-MM') AS ym, SUM(oi.amount) AS amt
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
    GROUP BY oi.product_id, FORMAT(o.order_date, 'yyyy-MM')
...
WITH cart AS (
    ...
    FROM carts
    WHERE add_time >= DATEADD(day, -30, CAST(GETDATE() AS DATE))
),
agg AS (
    SELECT p.category_id,
           COUNT(*)                                          AS add_cnt,
...
WITH rfm AS (
    SELECT o.customer_id,
           DATEDIFF(day, MAX(o.order_date), CAST(GETDATE() AS DATE))  AS recency,
...
WITH cohort AS (
    SELECT c.customer_id,
           FORMAT(c.register_date, 'yyyy-MM') AS cohort_ym,
           DATEFROMPARTS(YEAR(c.register_date), MONTH(c.register_date), 1)    AS cohort_md
    FROM customers c
),
act AS (
    SELECT o.customer_id,
           FORMAT(o.order_date, 'yyyy-MM') AS act_ym,
           DATEFROMPARTS(YEAR(o.order_date), MONTH(o.order_date), 1)    AS act_md
    FROM orders o
    WHERE o.status = 'completed'
    GROUP BY o.customer_id, FORMAT(o.order_date, 'yyyy-MM'), DATEFROMPARTS(YEAR(o.order_date), MONTH(o.order_date), 1)
),
joined AS (
...
SELECT j.cohort_ym,
       DATEDIFF(month, j.cohort_md, j.act_md) AS month_diff,
...
hist AS (
    SELECT customer_id,
           COUNT(CASE WHEN order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN 1 END) AS cnt_90d,
           COUNT(CASE WHEN order_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
                       AND order_date <  DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN 1 END) AS cnt_90_180d
    FROM orders
    WHERE status = 'completed'
    GROUP BY customer_id
)
SELECT ca.customer_id, c.customer_name, c.level,
       ca.last_order_date,
       DATEDIFF(day, ca.last_order_date, CAST(GETDATE() AS DATE)) AS idle_days,
       ca.order_cnt, ROUND(ca.total_amt, 2) AS total_amt,
       h.cnt_90d, h.cnt_90_180d,
       CASE WHEN DATEDIFF(day, ca.last_order_date, CAST(GETDATE() AS DATE)) > 180 THEN 'churned'
            WHEN DATEDIFF(day, ca.last_order_date, CAST(GETDATE() AS DATE)) > 90  THEN 'high_risk'
...
WHERE DATEDIFF(day, ca.last_order_date, CAST(GETDATE() AS DATE)) > 60
...
with_span AS (
    SELECT customer_id, order_cnt, total_amt,
           DATEDIFF(month, first_order, last_order) AS life_months,
           DATEDIFF(day, first_order, CAST(GETDATE() AS DATE))    AS days_since_first
    FROM cust
)
SELECT customer_id, order_cnt, ROUND(total_amt, 2) AS ltv,
       life_months,
       ROUND(total_amt / NULLIF(order_cnt, 0), 2)                     AS avg_order_value,
       ROUND(order_cnt / NULLIF(YEAR(CAST(GETDATE() AS DATE)) - 2020 + 1, 0), 2) AS annual_frequency,
...
WITH level_begin AS (
    ...
    FROM (
        ...
        FROM cust_level_log
        WHERE change_date >= CAST('2024-01-01' AS DATE)
    ) t
    WHERE rn = 1
),
cust_amt AS (
...
sleeping AS (
    ...
    FROM act
    WHERE DATEDIFF(day, last_dt, CAST(GETDATE() AS DATE)) BETWEEN 90 AND 365
),
reactivated AS (
...
SELECT CASE WHEN r.customer_id IS NOT NULL THEN 'reactivated' ELSE 'still_sleeping' END AS wake_status,
       COUNT(*)                          AS cust_cnt,
       ROUND(AVG(DATEDIFF(day, s.last_dt, COALESCE(r.wake_date, CAST(GETDATE() AS DATE)))), 1) AS avg_sleep_days,
...
SELECT c.customer_id, c.customer_name, c.birth_date, c.city,
       MONTH(CAST(GETDATE() AS DATE)) AS cur_month,
       ROUND(DATEDIFF(day, c.register_date, CAST(GETDATE() AS DATE)) / 365.0, 1) AS reg_years,
       MAX(o.order_date) AS last_order_date,
       SUM(o.pay_amount) AS hist_amt
FROM customers c
LEFT JOIN orders o ON c.customer_id = o.customer_id AND o.status = 'completed'
WHERE MONTH(c.birth_date) = MONTH(CAST(GETDATE() AS DATE))
...
SELECT o.order_id, o.customer_id,
       o.order_date,
       l.last_node_time,
       DATEDIFF(hour, l.last_node_time, GETDATE()) AS stall_hours,
       l.last_node,
       CASE WHEN DATEDIFF(hour, l.last_node_time, GETDATE()) > 72 THEN 'severely_stalled'
            WHEN DATEDIFF(hour, l.last_node_time, GETDATE()) > 48 THEN 'stalled'
            ELSE 'in_transit' END AS logistic_status
FROM orders o
JOIN latest l ON o.order_id = l.order_id
WHERE o.status IN ('shipped', 'delivering')
  AND DATEDIFF(hour, l.last_node_time, GETDATE()) > 24
...
WITH addr AS (
    ...
    FROM orders o
    JOIN shipments s ON o.order_id = s.order_id
    WHERE o.order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
WITH actual AS (
    ...
    LEFT JOIN orders o ON s.store_id = o.store_id
         ...
         AND FORMAT(o.order_date, 'yyyy-MM') = FORMAT(DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1), 'yyyy-MM')
    GROUP BY s.store_id, s.store_name, s.region
),
target AS (
    SELECT store_id, SUM(budget) AS target_amt
    FROM store_targets
    WHERE FORMAT(target_month, 'yyyy-MM') = FORMAT(DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1), 'yyyy-MM')
    GROUP BY store_id
)
...
WITH daily AS (
    ...
    FROM orders
    WHERE status = 'completed'
      AND order_date >= DATEADD(day, -120, CAST(GETDATE() AS DATE))
    GROUP BY CAST(order_date AS DATE)
),
ma AS (
    SELECT dt, gmv,
...
WITH d AS (
    ...
    FROM user_events
    WHERE event_time >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
    GROUP BY CAST(event_time AS DATE)
),
m AS (
    SELECT FORMAT(event_time, 'yyyy-MM') AS ym, COUNT(DISTINCT customer_id) AS mau
    FROM user_events
    WHERE event_time >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
FROM d
JOIN m ON FORMAT(d.dt, 'yyyy-MM') = m.ym
ORDER BY d.dt;
GO
...
WITH reg AS (
    ...
    FROM customers
    WHERE register_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
),
first_order AS (
    SELECT customer_id, MIN(order_date) AS first_order_date
    FROM orders
...
SELECT FORMAT(r.register_date, 'yyyy-MM') AS reg_ym,
       COUNT(*)                                        AS reg_cnt,
       COUNT(fo.customer_id)                           AS converted_cnt,
       ROUND(COUNT(fo.customer_id) / NULLIF(COUNT(*), 0) * 100, 2) AS conv_pct,
       ROUND(AVG(DATEDIFF(day, r.register_date, fo.first_order_date)), 2) AS avg_conv_days,
       SUM(CASE WHEN DATEDIFF(day, r.register_date, fo.first_order_date) <= 1 THEN 1 ELSE 0 END) AS within_1d,
       SUM(CASE WHEN DATEDIFF(day, r.register_date, fo.first_order_date) <= 7 THEN 1 ELSE 0 END) AS within_7d
FROM reg r
LEFT JOIN first_order fo ON r.customer_id = fo.customer_id
GROUP BY FORMAT(r.register_date, 'yyyy-MM')
ORDER BY reg_ym;
GO
...
WITH freq AS (
    ...
    FROM user_events
    WHERE event_time >= DATEADD(day, -30, CAST(GETDATE() AS DATE))
    GROUP BY customer_id
)
...
WITH seq AS (
    ...
    WHERE event_type = 'view'
      AND event_time >= DATEADD(day, -7, CAST(GETDATE() AS DATE))
)
SELECT session_id, customer_id, total_steps,
       STRING_AGG(CONCAT(CONCAT(CAST(step_no AS VARCHAR(10)), ':'), page_url), '>') WITHIN GROUP (ORDER BY step_no) AS path,
...
WITH ev AS (
    ...
    WHERE product_id IS NOT NULL
      AND event_time >= DATEADD(day, -30, CAST(GETDATE() AS DATE))
    GROUP BY product_id
)
...
WITH ev AS (
    ...
    FROM user_events
    WHERE event_time >= DATEADD(day, -7, CAST(GETDATE() AS DATE))
      AND product_id IS NOT NULL
)
SELECT ev.product_id, p.product_name,
       COUNT(*) AS view_events,
       ROUND(AVG(DATEDIFF(minute, event_time, next_time)), 2) AS avg_stay_minutes,
       MAX(DATEDIFF(minute, event_time, next_time))            AS max_stay_minutes,
       SUM(CASE WHEN next_type = 'add_cart' THEN 1 ELSE 0 END) AS led_to_cart
FROM ev
JOIN products p ON ev.product_id = p.product_id
WHERE next_time IS NOT NULL
  AND DATEDIFF(minute, event_time, next_time) BETWEEN 0 AND 60
...
flag AS (
    SELECT customer_id, event_time, event_type,
           CASE WHEN prev_time IS NULL
                     OR DATEDIFF(minute, prev_time, event_time) > 30 THEN 1 ELSE 0 END AS is_new
    FROM ev
),
sess AS (
    SELECT customer_id, event_time, event_type,
...
WITH nc AS (
    ...
    FROM customers
    WHERE register_date >= DATEADD(day, -60, CAST(GETDATE() AS DATE))
),
beh AS (
    ...
    LEFT JOIN user_events e ON n.customer_id = e.customer_id
         AND e.event_time BETWEEN n.register_date AND DATEADD(day, 7, n.register_date)
...
SELECT p.product_name,
       ...
       ROUND(AVG(DATEDIFF(day, rep.first_dt, rep.last_dt)), 1)  AS avg_cycle_days,
...
-- ------------------------------------------------------------------------------
WITH cust_price AS (
    SELECT o.customer_id, FORMAT(o.order_date, 'yyyy-MM') AS ym,
           AVG(p.price) AS avg_price, SUM(o.pay_amount) AS amt
    FROM orders o
    JOIN order_items oi ON o.order_id = oi.order_id
    JOIN products    p  ON oi.product_id = p.product_id
    WHERE o.status = 'completed'
    GROUP BY o.customer_id, FORMAT(o.order_date, 'yyyy-MM')
),
band AS (
...
WITH cm AS (
    SELECT p.category_id,
           SUM(CASE WHEN YEAR(o.order_date) = YEAR(CAST(GETDATE() AS DATE))   THEN oi.amount ELSE 0 END) AS amt_cur,
           SUM(CASE WHEN YEAR(o.order_date) = YEAR(CAST(GETDATE() AS DATE)) - 1 THEN oi.amount ELSE 0 END) AS amt_prev
    FROM order_items oi
    JOIN products p ON oi.product_id = p.product_id
    JOIN orders   o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT oi.product_id, FORMAT(o.order_date, 'yyyy-MM') AS ym, SUM(oi.quantity) AS qty
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
      AND o.order_date >= DATEADD(day, -120, CAST(GETDATE() AS DATE))
...
piv AS (
    SELECT product_id,
           SUM(CASE WHEN ym = FORMAT(DATEADD(month, -2, DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1)), 'yyyy-MM') THEN qty ELSE 0 END) AS q1,
           SUM(CASE WHEN ym = FORMAT(DATEADD(month, -1, DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1)), 'yyyy-MM') THEN qty ELSE 0 END) AS q2,
           SUM(CASE WHEN ym = FORMAT(DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1), 'yyyy-MM') THEN qty ELSE 0 END) AS q3
...
OFFSET 0 ROWS FETCH NEXT 150 ROWS ONLY;
GO
...
promo_days AS (
    SELECT DISTINCT CAST(DATEADD(day, 0, d.dt) AS DATE) AS dt
    FROM daily d
...
retention AS (
    ...
    LEFT JOIN orders o ON f.customer_id = o.customer_id
         AND o.order_date > DATEADD(day, -0, CAST(GETDATE() AS DATE))
    WHERE f.rn = 1
...
FROM shipments s
WHERE s.ship_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
GROUP BY s.carrier
...
SELECT reason,
       ...
       ROUND(AVG(DATEDIFF(day, order_date, refund_date)), 1)  AS avg_refund_lag_days,
       MAX(DATEDIFF(day, order_date, refund_date))            AS max_lag_days,
...
FROM payments p
WHERE p.pay_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
WITH dp AS (
    ...
    FROM payments
    WHERE pay_date >= DATEADD(day, -60, CAST(GETDATE() AS DATE))
...
SELECT o.order_id, o.customer_id, ROUND(o.pay_amount, 2) AS pay_amount, o.order_date,
       ...
       DATEDIFF(day, h.first_order, o.order_date)   AS days_since_first,
...
WITH d AS (
    ...
    FROM orders
    WHERE status = 'completed'
      AND order_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY CAST(order_date AS DATE)
),
stat AS (
    SELECT dt, gmv,
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN (DATEPART(weekday, o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END AS day_type,
       (DATEPART(weekday, o.order_date) - 1) AS dow,
       COUNT(*)                    AS order_cnt,
       COUNT(DISTINCT o.customer_id) AS uv,
...
WHERE o.status = 'completed'
  AND o.order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
GROUP BY CASE WHEN (DATEPART(weekday, o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END,
         (DATEPART(weekday, o.order_date) - 1)
ORDER BY dow;
GO
...
recent AS (
    ...
    FROM customers c
    JOIN orders o ON c.customer_id = o.customer_id
    WHERE o.status = 'completed' AND o.order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
    GROUP BY c.province
)
...
WITH pm AS (
    SELECT s.store_id, s.store_name, s.region,
           SUM(CASE WHEN o.order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN o.pay_amount ELSE 0 END) AS gmv_90d,
           SUM(CASE WHEN o.order_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
                     AND o.order_date <  DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN o.pay_amount ELSE 0 END) AS gmv_prev_90d,
           COUNT(CASE WHEN o.order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE)) THEN 1 END) AS orders_90d,
           COUNT(CASE WHEN o.order_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
                       AND o.order_date <  DATEADD(day, -90, CAST(GETDATE() AS DATE)) THEN 1 END) AS orders_prev_90d
    FROM stores s
    LEFT JOIN orders o ON s.store_id = o.store_id AND o.status = 'completed'
    GROUP BY s.store_id, s.store_name, s.region
)
...
WITH amt AS (
    SELECT customer_id,
           SUM(CASE WHEN order_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN pay_amount ELSE 0 END) AS amt_recent,
           SUM(CASE WHEN order_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
                     AND order_date <  DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN pay_amount ELSE 0 END) AS amt_prev
    FROM orders
    WHERE status = 'completed'
    GROUP BY customer_id
),
...
SELECT CASE WHEN pd.view_times = 1 THEN 'view_1'
       ...
       ROUND(AVG(DATEDIFF(minute, pd.first_view, pd.last_view)), 2)     AS avg_dwell_minutes
FROM pd
...
WITH base AS (
    SELECT c.customer_id, c.customer_name, c.city, c.level, c.gender,
           DATEDIFF(day, c.birth_date, CAST(GETDATE() AS DATE)) / 365 AS age
    FROM customers c
),
beh AS (
    SELECT o.customer_id,
...
SELECT b.customer_id, b.customer_name, b.city,
       ...
       DATEDIFF(day, beh.last_order, CAST(GETDATE() AS DATE)) AS idle_days,
...
stage AS (
    SELECT customer_id, order_cnt, total_amt,
           DATEDIFF(day, first_dt, CAST(GETDATE() AS DATE)) AS days_since_first,
           DATEDIFF(day, last_dt, CAST(GETDATE() AS DATE))  AS days_since_last
    FROM cust
)
...
SELECT ca.category_name AS category_a, cb.category_name AS category_b,
       ...
       ROUND(pr.both_cnt / NULLIF(CASE WHEN sa.cust_cnt < sb.cust_cnt THEN sa.cust_cnt ELSE sb.cust_cnt END, 0) * 100, 2) AS max_opportunity_pct
FROM pair pr
JOIN categories ca ON pr.cat_a = ca.category_id
JOIN categories cb ON pr.cat_b = cb.category_id
...
WITH np AS (
    ...
    FROM products p
    WHERE p.launch_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
),
perf AS (
    SELECT n.product_id,
    ...
    LEFT JOIN orders o ON oi.order_id = o.order_id AND o.status = 'completed'
         ...
         AND o.order_date <= DATEADD(day, 90, n.launch_date)
...
-- ------------------------------------------------------------------------------
WITH mr AS (
    SELECT product_id, FORMAT(created_at, 'yyyy-MM') AS ym,
           AVG(rating) AS avg_rating, COUNT(*) AS review_cnt
    FROM reviews
    GROUP BY product_id, FORMAT(created_at, 'yyyy-MM')
),
cmp AS (
...
SELECT channel,
       COUNT(*)                                                       AS order_cnt,
       ROUND(AVG(DATEDIFF(day, order_date, first_pay_date)), 2)          AS avg_pay_lag_days,
       MAX(DATEDIFF(day, order_date, first_pay_date))                    AS max_pay_lag_days,
       SUM(CASE WHEN DATEDIFF(day, order_date, first_pay_date) <= 0 THEN 1 ELSE 0 END) AS same_day_paid,
       ROUND(SUM(CASE WHEN DATEDIFF(day, order_date, first_pay_date) <= 0 THEN 1 ELSE 0 END)
...
WITH reg AS (
    ...
    FROM customers
    WHERE register_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
WITH big AS (
    ...
    WHERE t.txn_date >= DATEADD(day, -30, CAST(GETDATE() AS DATE))
      AND t.amount >= 100000
)
...
WITH acct_stat AS (
    SELECT account_id,
           ...
           COUNT(*)    AS txn_cnt
    FROM transactions
    WHERE txn_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
    GROUP BY account_id
    HAVING COUNT(*) >= 10
),
tagged AS (
    ...
    WHERE t.txn_date >= DATEADD(day, -7, CAST(GETDATE() AS DATE))
)
...
WITH hourly AS (
    SELECT account_id,
           FORMAT(txn_date, 'yyyy-MM-dd hh') AS hh,
           COUNT(*)      AS cnt,
           SUM(amount)   AS amt
    FROM transactions
    WHERE txn_date >= DATEADD(day, -7, CAST(GETDATE() AS DATE))
    GROUP BY account_id, FORMAT(txn_date, 'yyyy-MM-dd hh')
),
agg AS (
    SELECT account_id,
           SUM(cnt) AS txn_cnt,
...
WITH cp AS (
    ...
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
...
SELECT cust_id, account_id, txn_id,
       ...
       txn_date, next_date,
       DATEDIFF(hour, txn_date, next_date) AS gap_hours,
       ROUND(CASE WHEN amount < next_amount THEN amount ELSE next_amount END / NULLIF(CASE WHEN amount > next_amount THEN amount ELSE next_amount END, 0) * 100, 2) AS match_pct,
       CASE WHEN txn_type = 'transfer_out' AND next_type = 'transfer_in'
                 AND DATEDIFF(hour, txn_date, next_date) <= 24
                 AND CASE WHEN amount < next_amount THEN amount ELSE next_amount END / NULLIF(CASE WHEN amount > next_amount THEN amount ELSE next_amount END, 0) >= 0.9
...
SELECT a.account_type,
       COUNT(*)                                                   AS account_cnt,
       SUM(CASE WHEN DATEDIFF(day, lt.last_dt, CAST(GETDATE() AS DATE)) <= 30  THEN 1 ELSE 0 END) AS active_30d,
       SUM(CASE WHEN DATEDIFF(day, lt.last_dt, CAST(GETDATE() AS DATE)) BETWEEN 31 AND 90 THEN 1 ELSE 0 END) AS low_active,
       SUM(CASE WHEN DATEDIFF(day, lt.last_dt, CAST(GETDATE() AS DATE)) BETWEEN 91 AND 365 THEN 1 ELSE 0 END) AS dormant,
       SUM(CASE WHEN lt.account_id IS NULL
                     OR DATEDIFF(day, lt.last_dt, CAST(GETDATE() AS DATE)) > 365 THEN 1 ELSE 0 END) AS inactive,
       ROUND(AVG(DATEDIFF(day, lt.last_dt, CAST(GETDATE() AS DATE))), 1) AS avg_idle_days,
       ROUND(SUM(CASE WHEN DATEDIFF(day, lt.last_dt, CAST(GETDATE() AS DATE)) <= 30 THEN 1 ELSE 0 END)
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN DATEDIFF(month, start_date, maturity_date) <= 3  THEN 'demand_like'
            WHEN DATEDIFF(month, start_date, maturity_date) <= 12 THEN 'short_term'
            WHEN DATEDIFF(month, start_date, maturity_date) <= 36 THEN 'medium_term'
...
FROM deposits
WHERE status = 'active'
GROUP BY CASE WHEN DATEDIFF(month, start_date, maturity_date) <= 3  THEN 'demand_like'
              WHEN DATEDIFF(month, start_date, maturity_date) <= 12 THEN 'short_term'
              WHEN DATEDIFF(month, start_date, maturity_date) <= 36 THEN 'medium_term'
              ELSE 'long_term' END
ORDER BY total_amt DESC;
...
WITH mat AS (
    SELECT d.deposit_id, d.cust_id, d.amount, d.rate, d.maturity_date,
           DATEDIFF(day, CAST(GETDATE() AS DATE), d.maturity_date) AS days_to_maturity
    FROM deposits d
...
-- ------------------------------------------------------------------------------
SELECT t.channel,
       FORMAT(t.txn_date, 'yyyy-MM') AS ym,
       COUNT(*)                    AS txn_cnt,
       ROUND(SUM(t.amount), 2)     AS total_amt,
       ROUND(AVG(t.amount), 2)     AS avg_amt,
       COUNT(DISTINCT a.cust_id)   AS cust_cnt,
       ROUND(SUM(t.amount) / NULLIF(SUM(SUM(t.amount)) OVER (PARTITION BY FORMAT(t.txn_date, 'yyyy-MM')), 0) * 100, 2) AS channel_share_pct,
       ROUND(SUM(t.amount) - LAG(SUM(t.amount)) OVER (PARTITION BY t.channel ORDER BY FORMAT(t.txn_date, 'yyyy-MM')), 2) AS mom_delta
FROM transactions t
JOIN accounts a ON t.account_id = a.account_id
WHERE t.txn_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
GROUP BY t.channel, FORMAT(t.txn_date, 'yyyy-MM')
...
WITH t AS (
    SELECT t.txn_id, t.account_id, t.amount, t.txn_date, t.channel,
           a.cust_id,
           DATEPART(hour, t.txn_date) AS txn_hour,
           (DATEPART(weekday, t.txn_date) - 1)  AS txn_dow
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
)
SELECT cust_id,
       COUNT(*)                                                    AS total_txn,
...
pairs AS (
    ...
    WHERE a1.cust_id < a2.cust_id
      AND t.txn_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
...
WITH day_txn AS (
    ...
    WHERE txn_type = 'transfer_out'
      AND txn_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
cycle2 AS (
    SELECT e1.from_acct AS a, e1.to_acct AS b, e2.to_acct AS c,
           CASE WHEN e1.total_amt < e2.total_amt THEN e1.total_amt ELSE e2.total_amt END AS min_amt
    FROM edges e1
...
FROM risk_events
WHERE event_time >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
...
WITH evt AS (
    ...
    FROM risk_events
    WHERE event_time >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY cust_id
),
score AS (
...
FROM (
    ...
    FROM credit_scores
    WHERE score_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
    GROUP BY cust_id
) s
...
WITH suspicious AS (
    SELECT t.account_id, a.cust_id, t.txn_id, t.amount, t.txn_date, t.txn_type, t.city,
           ...
           CASE WHEN DATEPART(hour, t.txn_date) >= 20 OR DATEPART(hour, t.txn_date) < 8 THEN 2 ELSE 0 END AS time_score
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATEADD(day, -30, CAST(GETDATE() AS DATE))
),
...
SELECT s.cust_id, c.cust_name, c.risk_level,
       ...
       STRING_AGG(CAST(txn_id AS VARCHAR(20)), ',') WITHIN GROUP (ORDER BY txn_id) AS txn_list,
...
evt_hist AS (
    SELECT cust_id,
           FORMAT(event_time, 'yyyy-MM') AS ym,
           MAX(CASE WHEN risk_level = 'high'   THEN 3
                    WHEN risk_level = 'medium' THEN 2
                    ELSE 1 END) AS level_num
    FROM risk_events
    WHERE event_time >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
    GROUP BY cust_id, FORMAT(event_time, 'yyyy-MM')
),
trend AS (
    SELECT cust_id,
...
SELECT a.account_type,
       FORMAT(a.open_date, 'yyyy-MM') AS open_ym,
       COUNT(*)                                                     AS opened_cnt,
       COUNT(ft.account_id)                                         AS activated_cnt,
       ROUND(COUNT(ft.account_id) / NULLIF(COUNT(*), 0) * 100, 2)   AS activation_pct,
       ROUND(AVG(DATEDIFF(day, a.open_date, ft.first_dt)), 2)          AS avg_activation_days,
       SUM(CASE WHEN DATEDIFF(day, a.open_date, ft.first_dt) <= 7 THEN 1 ELSE 0 END) AS within_7d,
       SUM(CASE WHEN DATEDIFF(day, a.open_date, ft.first_dt) <= 30 THEN 1 ELSE 0 END) AS within_30d
FROM accounts a
LEFT JOIN first_txn ft ON a.account_id = ft.account_id
WHERE a.open_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
GROUP BY a.account_type, FORMAT(a.open_date, 'yyyy-MM')
...
-- ------------------------------------------------------------------------------
WITH act AS (
    SELECT DISTINCT a.cust_id, FORMAT(t.txn_date, 'yyyy-MM') AS ym
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
),
seq AS (
    SELECT cust_id, ym,
...
tenure AS (
    SELECT cust_id,
           DATEDIFF(day, MIN(open_date), CAST(GETDATE() AS DATE)) / 365.0 AS tenure_years
...
WITH act AS (
    SELECT a.cust_id,
           SUM(CASE WHEN t.txn_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN 1 ELSE 0 END) AS cnt_90d,
           SUM(CASE WHEN t.txn_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
                     AND t.txn_date <  DATEADD(day, -90, CAST(GETDATE() AS DATE))  THEN 1 ELSE 0 END) AS cnt_prev_90d,
...
SELECT c.cust_id, c.cust_name, c.cust_type,
       ...
       DATEDIFF(day, a.last_txn, CAST(GETDATE() AS DATE)) AS idle_days,
       ROUND(b.cur_balance, 2)           AS cur_balance,
       ROUND((a.cnt_90d - a.cnt_prev_90d) / NULLIF(a.cnt_prev_90d, 0) * 100, 2) AS activity_change_pct,
       CASE WHEN DATEDIFF(day, a.last_txn, CAST(GETDATE() AS DATE)) > 180 THEN 'churned'
            WHEN DATEDIFF(day, a.last_txn, CAST(GETDATE() AS DATE)) > 90
...
WHERE DATEDIFF(day, a.last_txn, CAST(GETDATE() AS DATE)) > 60
...
SELECT c.cust_id, c.cust_name, c.risk_level,
       ...
       ROUND((f.inflow - f.outflow)
             / NULLIF(CASE WHEN f.inflow > f.outflow THEN f.inflow ELSE f.outflow END, 0) * 100, 2) AS net_flow_pct,
...
WITH r AS (
    ...
    FROM fin_reports
    WHERE period >= FORMAT(DATEADD(day, -365, CAST(GETDATE() AS DATE)), 'yyyy-MM')
    GROUP BY branch_id
)
SELECT b.branch_name, b.city, b.region,
...
WITH card_usage AS (
    ...
    LEFT JOIN card_txns t ON c.card_id = t.card_id
         AND t.txn_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
...
WITH t AS (
    SELECT ct.card_id, ct.txn_id, ct.amount, ct.txn_date, ct.merchant, ct.mcc,
           (FLOOR(ct.amount) % 1000) AS mod_1000,
           COUNT(*) OVER (PARTITION BY ct.merchant) AS merchant_txn_all
    FROM card_txns ct
    WHERE ct.txn_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
)
SELECT card_id,
       COUNT(*)                                                    AS txn_cnt,
...
WITH t AS (
    SELECT a.cust_id, t.amount, t.txn_date, t.channel, t.txn_type,
           DATEPART(hour, t.txn_date) AS hr
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
)
...
WITH e1 AS (
    ...
    JOIN accounts a2 ON t.counterparty = CAST(a2.account_id AS VARCHAR(20))
    WHERE t.txn_type = 'transfer_out'
      AND t.txn_date >= DATEADD(day, -60, CAST(GETDATE() AS DATE))
    GROUP BY t.account_id, a2.account_id
),
e2 AS (
    ...
    WHERE t2.txn_type = 'transfer_out'
      AND t2.txn_date >= DATEADD(day, -60, CAST(GETDATE() AS DATE))
)
...
SELECT CAST(t.txn_date AS DATE) AS dt,
       ...
       SUM(CASE WHEN DATEPART(hour, t.txn_date) >= 20
                  OR DATEPART(hour, t.txn_date) < 8 THEN 1 ELSE 0 END)    AS offhour_cnt,
...
-- ------------------------------------------------------------------------------
WITH stage AS (
    SELECT r.loan_id, FORMAT(r.due_date, 'yyyy-MM') AS ym,
           MAX(r.overdue_days) AS max_od
    FROM repayments r
    GROUP BY r.loan_id, FORMAT(r.due_date, 'yyyy-MM')
),
bucketed AS (
    SELECT loan_id, ym,
...
SELECT CASE WHEN term_months <= 6   THEN 'short_0_6m'
       ...
       SUM(CASE WHEN DATEDIFF(month, CAST(GETDATE() AS DATE), end_date) <= 12 THEN 1 ELSE 0 END) AS maturing_12m,
       ROUND(SUM(CASE WHEN DATEDIFF(month, CAST(GETDATE() AS DATE), end_date) <= 12 THEN loan_amount ELSE 0 END), 2) AS repricing_amt,
       ROUND(SUM(CASE WHEN DATEDIFF(month, CAST(GETDATE() AS DATE), end_date) <= 12 THEN loan_amount ELSE 0 END)
...
WITH rb AS (
    SELECT l.loan_id, l.cust_id,
           ...
           AVG(DATEDIFF(day, r.due_date, r.repay_date))                           AS avg_pay_gap,
           MAX(r.overdue_days)                                                 AS max_od,
...
SELECT product_type,
       ...
       ROUND(AVG(DATEDIFF(month, start_date, last_pay)), 1)               AS avg_actual_months
FROM rp
GROUP BY product_type
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT FORMAT(start_date, 'yyyy-MM') AS ym, product_type,
           COUNT(*)          AS loan_cnt,
           SUM(loan_amount)  AS loan_amt
    FROM loans
    WHERE start_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
    GROUP BY FORMAT(start_date, 'yyyy-MM'), product_type
)
...
SELECT od_stage,
       ...
       ROUND(AVG(DATEDIFF(day, due_date, repay_date)), 1)            AS avg_recovery_days,
...
WITH orig AS (
    SELECT loan_id, cust_id, product_type, loan_amount, term_months,
           start_date, end_date,
           DATEDIFF(month, start_date, end_date) AS actual_months
    FROM loans
)
SELECT product_type,
       COUNT(*)                                                            AS loan_cnt,
...
-- ------------------------------------------------------------------------------
SELECT FORMAT(l.start_date, 'yyyy-MM') AS vintage,
       COUNT(*)                                                        AS loan_cnt,
...
FROM loans l
WHERE l.start_date >= DATEADD(day, -1095, CAST(GETDATE() AS DATE))
...
WITH n AS (
    ...
    FROM fund_nav
    WHERE nav_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
),
dd AS (
...
SELECT f.fund_name, f.fund_type, f.risk_level,
       ...
       ROUND(CAST(PERCENT_RANK() OVER (PARTITION BY f.fund_type ORDER BY c.nav_change) * 100 AS DECIMAL(20,8)), 1) AS pct_in_type,
...
SELECT c.cust_id, c.cust_name,
       ...
       ROUND(AVG(DATEDIFF(day, h.purchase_date, CAST(GETDATE() AS DATE))), 0)              AS avg_holding_days,
       MAX(DATEDIFF(day, h.purchase_date, CAST(GETDATE() AS DATE)))                        AS max_holding_days,
...
monthly AS (
    SELECT fund_id, FORMAT(purchase_date, 'yyyy-MM') AS ym,
           COUNT(*)          AS subscribe_cnt,
           SUM(cost_amount)  AS subscribe_amt
    FROM h GROUP BY fund_id, FORMAT(purchase_date, 'yyyy-MM')
)
...
perf AS (
    ...
    FROM fund_nav n
    WHERE n.nav_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY n.fund_id
)
SELECT m.manager_id,
       COUNT(*)                                 AS fund_cnt,
...
SELECT f.fund_name,
       ...
       ROUND(AVG(DATEDIFF(day, purchase_date, CAST(GETDATE() AS DATE))), 0)               AS avg_holding_days
FROM pos
...
WITH dep AS (
    SELECT d.deposit_id, d.cust_id, d.amount, d.rate, d.start_date, d.maturity_date,
           DATEDIFF(day, d.start_date, d.maturity_date)  AS days_held,
           DATEDIFF(month, d.start_date, d.maturity_date) AS months_held
    FROM deposits d
)
SELECT CASE WHEN months_held <= 3  THEN 'demand_like'
       ...
       SUM(CASE WHEN maturity_date <= CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END)     AS matured_cnt,
       ROUND(SUM(CASE WHEN maturity_date <= CAST(GETDATE() AS DATE) THEN amount * rate * days_held / 365 ELSE 0 END), 2) AS matured_interest
FROM dep
...
WITH y AS (
    SELECT branch_id,
           SUBSTRING(period, 1, 4) AS yr,
           SUM(revenue) AS revenue,
           SUM(cost)    AS cost,
           SUM(profit)  AS profit
    FROM fin_reports
    GROUP BY branch_id, SUBSTRING(period, 1, 4)
)
...
WITH r AS (
    ...
    FROM fx_rates
    WHERE rate_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
)
SELECT currency,
       COUNT(*)                                          AS quote_days,
...
SELECT b.branch_name, b.region,
       ...
       ROUND(STDEV(c.total_cost) / NULLIF(AVG(c.total_cost), 0) * 100, 2) AS cost_volatility_pct,
...
WITH br AS (
    ...
    FROM fin_reports r
    WHERE r.period >= FORMAT(DATEADD(day, -365, CAST(GETDATE() AS DATE)), 'yyyy-MM')
...
WITH rev AS (
    SELECT r.branch_id,
           SUM(r.revenue) AS total_revenue,
           SUM(CASE WHEN r.period >= FORMAT(DATEFROMPARTS(YEAR(CAST(GETDATE() AS DATE)), MONTH(CAST(GETDATE() AS DATE)), 1), 'yyyy-MM')
...
WITH e AS (
    SELECT emp_id, dept_id, job_level, status,
           DATEDIFF(day, hire_date, COALESCE(leave_date, CAST(GETDATE() AS DATE))) / 365.0 AS tenure_years
    FROM employees
)
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 26 THEN 'gen_z_below26'
            WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 36 THEN 'millennial_26_35'
            WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 46 THEN 'gen_x_36_45'
            WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 56 THEN 'boomer_46_55'
            ELSE 'near_retirement_56plus' END AS age_band,
       COUNT(*)                                                   AS emp_cnt,
       ROUND(AVG(DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365.0), 1)   AS avg_age,
       SUM(CASE WHEN gender = 'M' THEN 1 ELSE 0 END)              AS male_cnt,
       SUM(CASE WHEN gender = 'F' THEN 1 ELSE 0 END)              AS female_cnt,
       ROUND(AVG(DATEDIFF(day, hire_date, CAST(GETDATE() AS DATE)) / 365.0), 2)    AS avg_tenure,
       ROUND(AVG(salary), 2)                                      AS avg_salary,
       ROUND(COUNT(*) / NULLIF(SUM(COUNT(*)) OVER (), 0) * 100, 2) AS emp_share_pct
FROM employees
WHERE status = 'active'
GROUP BY CASE WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 26 THEN 'gen_z_below26'
              WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 36 THEN 'millennial_26_35'
              WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 46 THEN 'gen_x_36_45'
              WHEN DATEDIFF(day, birth_date, CAST(GETDATE() AS DATE)) / 365 < 56 THEN 'boomer_46_55'
...
-- ------------------------------------------------------------------------------
WITH hires AS (
    SELECT FORMAT(hire_date, 'yyyy-MM') AS ym, COUNT(*) AS hire_cnt
    FROM employees
    WHERE hire_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
    GROUP BY FORMAT(hire_date, 'yyyy-MM')
),
leaves AS (
    SELECT FORMAT(leave_date, 'yyyy-MM') AS ym, COUNT(*) AS leave_cnt
    FROM employees
    WHERE leave_date IS NOT NULL
      AND leave_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
    GROUP BY FORMAT(leave_date, 'yyyy-MM')
),
yms AS (
...
-- ------------------------------------------------------------------------------
WITH base AS (
    SELECT FORMAT(hire_date, 'yyyy-MM') AS ym,
           COUNT(*) AS cohort_size,
           SUM(CASE WHEN leave_date IS NOT NULL THEN 1 ELSE 0 END) AS left_cnt,
           AVG(DATEDIFF(day, hire_date, leave_date)) AS avg_days_to_leave
    FROM employees
    GROUP BY FORMAT(hire_date, 'yyyy-MM')
)
...
WITH nh AS (
    SELECT emp_id, hire_date, dept_id,
           leave_date,
           DATEDIFF(day, hire_date, leave_date) AS days_to_leave
    FROM employees
    WHERE hire_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
)
SELECT FORMAT(hire_date, 'yyyy-MM') AS hire_ym,
       COUNT(*)                                                        AS hired_cnt,
...
recent_left AS (
    ...
    FROM employees
    WHERE leave_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
    GROUP BY dept_id
)
SELECT d.dept_name,
       ds.active_cnt,
...
SELECT d.dept_name, d.cost_center,
       ...
       SUM(CASE WHEN e.leave_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE)) THEN 1 ELSE 0 END) AS left_1y,
       ROUND(SUM(CASE WHEN e.leave_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE)) THEN 1 ELSE 0 END)
             / NULLIF(SUM(CASE WHEN e.status = 'active' THEN 1 ELSE 0 END), 0) * 100, 2) AS annual_turnover_pct,
       ROUND(AVG(DATEDIFF(day, e.hire_date, CAST(GETDATE() AS DATE)) / 365.0), 2)        AS avg_tenure,
       ROUND(AVG(e.salary), 2)                                          AS avg_salary,
       RANK() OVER (ORDER BY SUM(CASE WHEN e.leave_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE)) THEN 1 ELSE 0 END)
...
SELECT c.change_type,
       FORMAT(c.change_date, 'yyyy-MM') AS change_ym,
       COUNT(*)                                            AS change_cnt,
...
FROM emp_changes c
WHERE c.change_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
GROUP BY c.change_type, FORMAT(c.change_date, 'yyyy-MM')
...
tenure AS (
    SELECT emp_id, DATEDIFF(day, hire_date, CAST(GETDATE() AS DATE)) / 365.0 AS tenure_years
    FROM employees
)
...
FROM payroll p
WHERE p.pay_month >= FORMAT(DATEADD(day, -730, CAST(GETDATE() AS DATE)), 'yyyy-MM')
...
WHERE p.pay_month >= FORMAT(DATEADD(day, -365, CAST(GETDATE() AS DATE)), 'yyyy-MM')
GROUP BY d.dept_name, d.cost_center
...
WITH lv AS (
    SELECT e.emp_id, e.job_level, e.position, e.gender, e.salary, e.dept_id,
           DATEDIFF(day, e.hire_date, CAST(GETDATE() AS DATE)) / 365.0 AS tenure_years,
...
WITH sal AS (
    SELECT s.emp_id, s.effective_date, s.total_salary,
           ...
           YEAR(s.effective_date) AS adj_year
    FROM salaries s
)
SELECT adj_year,
...
SELECT emp_id, emp_name, job_level,
       ...
       ROUND(CAST(pct_in_dept * 100 AS DECIMAL(20,8)), 1)              AS percentile_in_dept,
...
WITH ot AS (
    SELECT a.emp_id, e.dept_id, e.job_level,
           MONTH(a.att_date) AS mon,
           SUM(a.overtime_hours)                                   AS total_ot,
           COUNT(*)                                                AS work_days,
           SUM(CASE WHEN a.overtime_hours > 0 THEN 1 ELSE 0 END)   AS ot_days
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY a.emp_id, e.dept_id, e.job_level, MONTH(a.att_date)
)
SELECT job_level,
...
WITH a AS (
    ...
    FROM attendance
    WHERE att_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
    GROUP BY emp_id
)
...
WITH late AS (
    ...
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
...
grp AS (
    SELECT emp_id, att_date, status, is_abnormal,
           DATEDIFF(day, CAST('2000-01-01' AS DATE), att_date)
...
WHERE l.start_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
...
WITH used AS (
    ...
    WHERE status = 'approved'
      AND YEAR(start_date) = YEAR(CAST(GETDATE() AS DATE))
...
SELECT e.emp_id, e.emp_name, d.dept_name,
       ...
       DATEDIFF(day, CAST(GETDATE() AS DATE), CAST('2026-12-31' AS DATE))                 AS days_to_expire,
...
-- ------------------------------------------------------------------------------
SELECT d.dept_name, l.leave_type, MONTH(l.start_date) AS mon,
       COUNT(*)                    AS request_cnt,
       SUM(l.days)                 AS total_days,
       ROUND(AVG(l.days), 2)       AS avg_days,
       COUNT(DISTINCT l.emp_id)    AS emp_cnt,
...
JOIN departments d ON e.dept_id = d.dept_id
WHERE l.start_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
GROUP BY d.dept_name, l.leave_type, MONTH(l.start_date)
...
-- ------------------------------------------------------------------------------
SELECT FORMAT(a.att_date, 'yyyy-MM') AS ym,
       COUNT(DISTINCT a.emp_id)                                         AS emp_cnt,
       ...
       ROUND(SUM(a.overtime_hours) - LAG(SUM(a.overtime_hours))
             OVER (ORDER BY FORMAT(a.att_date, 'yyyy-MM')), 1)             AS ot_mom_delta
FROM attendance a
WHERE a.att_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
GROUP BY FORMAT(a.att_date, 'yyyy-MM')
ORDER BY ym DESC;
GO
...
att AS (
    ...
    FROM attendance
    WHERE att_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
    GROUP BY emp_id
),
tr AS (
...
SELECT e.emp_id, e.emp_name, e.position, e.job_level, d.dept_name,
       ...
       ROUND(DATEDIFF(day, e.hire_date, CAST(GETDATE() AS DATE)) / 365.0, 2) AS tenure_years,
       e.salary,
...
WITH team AS (
    SELECT e.manager_id,
           ...
           AVG(DATEDIFF(day, e.hire_date, CAST(GETDATE() AS DATE)) / 365.0)             AS avg_team_tenure,
...
WITH hc AS (
    SELECT COUNT(*) AS total_emp,
           ...
           SUM(CASE WHEN leave_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE)) THEN 1 ELSE 0 END) AS left_1y,
           ROUND(AVG(salary), 2) AS avg_salary,
           ROUND(AVG(DATEDIFF(day, hire_date, CAST(GETDATE() AS DATE)) / 365.0), 2) AS avg_tenure
    FROM employees
),
cost AS (
    ...
    FROM payroll
    WHERE pay_month >= FORMAT(DATEADD(day, -365, CAST(GETDATE() AS DATE)), 'yyyy-MM')
),
perf AS (
...
WITH req AS (
    ...
    FROM recruitment r
    WHERE r.open_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY r.dept_id
)
SELECT d.dept_name,
...
-- ------------------------------------------------------------------------------
SELECT FORMAT(r.open_date, 'yyyy-MM') AS open_ym,
       r.position,
       COUNT(*)                                                    AS req_cnt,
       ...
       ROUND(AVG(DATEDIFF(day, r.open_date, COALESCE(r.close_date, CAST(GETDATE() AS DATE)))), 1) AS avg_days_open,
       ROUND(MAX(DATEDIFF(day, r.open_date, COALESCE(r.close_date, CAST(GETDATE() AS DATE)))), 1) AS max_days_open,
       SUM(CASE WHEN r.status = 'open'
                 AND DATEDIFF(day, r.open_date, CAST(GETDATE() AS DATE)) > 60 THEN 1 ELSE 0 END) AS backlog_over_60d,
       SUM(r.hired_cnt)                                            AS hired_cnt
FROM recruitment r
WHERE r.open_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
GROUP BY FORMAT(r.open_date, 'yyyy-MM'), r.position
...
WITH nh AS (
    SELECT e.emp_id, e.dept_id, e.hire_date,
           DATEDIFF(day, e.hire_date, CAST(GETDATE() AS DATE)) AS days_since_hire,
           p.avg_score
    FROM employees e
    LEFT JOIN (SELECT emp_id, AVG(score) AS avg_score FROM performance GROUP BY emp_id) p
           ON e.emp_id = p.emp_id
    WHERE e.hire_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
)
SELECT d.dept_name,
       COUNT(*)                                                        AS new_hires,
...
WITH tr AS (
    ...
    WHERE t.train_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY t.emp_id
),
dept AS (
...
SELECT p.project_name, p.status, d.dept_name,
       ...
       ROUND(DATEDIFF(day, p.start_date, COALESCE(p.end_date, CAST(GETDATE() AS DATE))) / 30.0, 1) AS duration_months,
...
WITH emp_load AS (
    SELECT pa.emp_id,
           ...
           STRING_AGG(CAST(pa.project_id AS VARCHAR(50)), ',') WITHIN GROUP (ORDER BY pa.project_id)  AS project_list
    FROM project_assignments pa
    JOIN projects p ON pa.project_id = p.project_id
...
WITH pm AS (
    SELECT p.project_id, p.project_name, p.dept_id, p.status,
           DATEDIFF(day, p.start_date, COALESCE(p.end_date, CAST(GETDATE() AS DATE))) AS duration_days,
    ...
    FROM projects p
    WHERE p.start_date >= DATEADD(day, -730, CAST(GETDATE() AS DATE))
)
...
WITH chg AS (
    ...
    FROM emp_changes c
    WHERE c.change_date >= DATEADD(day, -1095, CAST(GETDATE() AS DATE))
)
SELECT d.dept_name,
...
gap AS (
    SELECT emp_id, promo_seq,
           DATEDIFF(day, prev_promo_date, change_date) / 365.0 AS years_between,
...
WITH lv AS (
    SELECT e.emp_id, e.dept_id, e.job_level, e.position,
           DATEDIFF(day, e.hire_date, COALESCE(e.leave_date, CAST(GETDATE() AS DATE))) / 365.0 AS tenure_at_exit,
           c.change_type,
           p.avg_score,
           a.abnormal_rate
    FROM employees e
...
WITH demo AS (
    SELECT e.dept_id,
           ...
           CASE WHEN DATEDIFF(year, e.birth_date, CAST(GETDATE() AS DATE)) < 26 THEN 'gen_z'
                WHEN DATEDIFF(year, e.birth_date, CAST(GETDATE() AS DATE)) < 36 THEN 'millennial'
                WHEN DATEDIFF(year, e.birth_date, CAST(GETDATE() AS DATE)) < 46 THEN 'gen_x'
...
SELECT d.dept_name,
       ...
       ROUND(AVG(DATEDIFF(year, a.birth_date, CAST(GETDATE() AS DATE))), 1)             AS avg_age,
...
WITH sig AS (
    ...
    LEFT JOIN (SELECT emp_id, SUM(days) AS leave_days FROM leaves
               WHERE start_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
...
SELECT d.dept_name,
       ...
       ROUND((AVG(s.attendance_rate) * 0.3 + AVG(s.perf_score) * 0.4
              + CASE WHEN AVG(s.train_cnt) * 10.0 < 100.0 THEN AVG(s.train_cnt) * 10.0 ELSE 100.0 END * 0.2
              + CASE WHEN AVG(s.leave_days) * 2.0 < 100.0 THEN AVG(s.leave_days) * 2.0 ELSE 100.0 END * 0.1), 2)          AS engagement_index,
       RANK() OVER (ORDER BY (AVG(s.attendance_rate) * 0.3 + AVG(s.perf_score) * 0.4
                              + CASE WHEN AVG(s.train_cnt) * 10.0 < 100.0 THEN AVG(s.train_cnt) * 10.0 ELSE 100.0 END * 0.2
                              + CASE WHEN AVG(s.leave_days) * 2.0 < 100.0 THEN AVG(s.leave_days) * 2.0 ELSE 100.0 END * 0.1) DESC) AS engagement_rank
FROM sig s
...
WITH act AS (
    SELECT e.dept_id,
           FORMAT(p.pay_date, 'yyyy-MM')                                   AS pay_ym,
           SUM(p.gross_pay)                                             AS actual_cost,
           COUNT(DISTINCT p.emp_id)                                     AS paid_headcount
    FROM payroll p
    JOIN employees e ON p.emp_id = e.emp_id
    WHERE p.pay_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY e.dept_id, FORMAT(p.pay_date, 'yyyy-MM')
),
bud AS (
...
WITH y AS (
    ...
    FROM payroll p
    JOIN employees e ON p.emp_id = e.emp_id
    WHERE p.pay_date >= DATEADD(day, -1095, CAST(GETDATE() AS DATE))
    GROUP BY YEAR(p.pay_date), e.dept_id
)
...
SELECT d.dept_name,
       ...
       ROUND(STDEV(s.direct_reports), 2)                             AS span_stddev,
...
WITH emp_val AS (
    ...
    LEFT JOIN (SELECT emp_id, SUM(gross_pay) AS total_gross FROM payroll
               WHERE pay_month >= FORMAT(DATEADD(day, -1095, CAST(GETDATE() AS DATE)), 'yyyy-MM')
...
WITH pos AS (
    SELECT e.dept_id, e.position, e.job_level,
           ...
           SUM(CASE WHEN e.leave_date IS NOT NULL
                     AND e.leave_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE)) THEN 1 ELSE 0 END) AS recent_leavers
    FROM employees e
...
WITH risk AS (
    ...
    LEFT JOIN (SELECT emp_id,
               ...
               GROUP BY emp_id) a ON e.emp_id = a.emp_id
    LEFT JOIN (SELECT emp_id, SUM(days) AS leave_days FROM leaves
               WHERE start_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE)) GROUP BY emp_id) lv
...
at AS (
    ...
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= DATEADD(day, -90, CAST(GETDATE() AS DATE))
    GROUP BY e.dept_id
),
cost AS (
    ...
    WHERE py.pay_month >= FORMAT(DATEADD(day, -365, CAST(GETDATE() AS DATE)), 'yyyy-MM')
    GROUP BY e.dept_id
)
SELECT d.dept_name,
...
SELECT 'customers'                                                       AS table_name,
       ...
       SUM(CASE WHEN c.register_date > CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END)      AS future_date_rows,
       SUM(CASE WHEN DATEDIFF(year, c.register_date, CAST(GETDATE() AS DATE)) > 50 THEN 1 ELSE 0 END) AS implausible_rows
FROM customers c
UNION ALL
SELECT 'orders',
       COUNT(*),
       ...
       ROUND(SUM(CASE WHEN o.customer_id IS NULL THEN 1 ELSE 0 END) * 100.0
             / NULLIF(COUNT(*), 0), 2),
       SUM(CASE WHEN o.order_date > CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END),
...
SELECT 'employees',
       ...
       ROUND(SUM(CASE WHEN e.emp_name IS NULL THEN 1 ELSE 0 END) * 100.0
             / NULLIF(COUNT(*), 0), 2),
       SUM(CASE WHEN e.hire_date > CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END),
...
SELECT 'transactions',
       ...
       ROUND(SUM(CASE WHEN t.account_id IS NULL THEN 1 ELSE 0 END) * 100.0
             / NULLIF(COUNT(*), 0), 2),
       SUM(CASE WHEN t.txn_date > CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END),
...
WITH dup_cust AS (
    SELECT c.customer_name, c.city,
           ...
           STRING_AGG(CAST(c.customer_id AS VARCHAR(50)), ',') WITHIN GROUP (ORDER BY c.customer_id)             AS id_list
    FROM customers c
...
dup_emp AS (
    SELECT e.emp_name, e.dept_id,
           ...
           STRING_AGG(CAST(e.emp_id AS VARCHAR(50)), ',') WITHIN GROUP (ORDER BY e.emp_id)                       AS id_list
    FROM employees e
...
SELECT FORMAT(c.order_date, 'yyyy-MM')                                      AS recon_ym,
       COUNT(*)                                                          AS order_cnt,
...
WITH eco AS (
    SELECT o.customer_id,
           ...
           MAX(o.order_date)                                             AS last_order_date
    FROM orders o
    WHERE o.order_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
      AND o.status = 'completed'
    GROUP BY o.customer_id
),
fin AS (
...
target AS (
    ...
    FROM store_targets
    WHERE FORMAT(target_month, 'yyyy-MM') >= FORMAT(DATEADD(day, -365, CAST(GETDATE() AS DATE)), 'yyyy-MM')
    GROUP BY store_id
)
...
WITH base AS (
    SELECT s.city,
           c.category_name,
           FORMAT(o.order_date, 'yyyy-MM')                                  AS ym,
    ...
    WHERE o.status = 'completed'
      AND o.order_date >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY s.city, c.category_name, FORMAT(o.order_date, 'yyyy-MM')
)
...
WITH daily AS (
    SELECT CAST(o.order_date AS DATE)                                       AS dt,
           ...
           COUNT(*)                                                      AS order_cnt
    FROM orders o
    WHERE o.order_date >= DATEADD(day, -180, CAST(GETDATE() AS DATE))
      AND o.status = 'completed'
    GROUP BY CAST(o.order_date AS DATE)
),
stat AS (
    SELECT d.dt, d.gmv, d.order_cnt,
           ...
           STDEV(d.gmv) OVER (ORDER BY d.dt ROWS BETWEEN 29 PRECEDING AND 1 PRECEDING) AS sd30
    FROM daily d
)
SELECT s.dt,
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT FORMAT(o.order_date, 'yyyy-MM')                                  AS ym,
           SUM(o.pay_amount)                                             AS gmv,
           COUNT(*)                                                      AS order_cnt,
           COUNT(DISTINCT o.customer_id)                                 AS active_cust
    FROM orders o
    WHERE o.status = 'completed'
      AND o.order_date >= DATEADD(day, -1095, CAST(GETDATE() AS DATE))
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT DATEFROMPARTS(YEAR(o.order_date), MONTH(o.order_date), 1)                                     AS ym,
           SUM(o.pay_amount)                                             AS gmv,
           COUNT(DISTINCT o.customer_id)                                 AS cust_cnt
    FROM orders o
    WHERE o.status = 'completed'
...
SELECT FORMAT(w.ym, 'yyyy-MM')                                              AS ym,
...
cmp AS (
    SELECT t.store_id,
           FORMAT(t.target_month, 'yyyy-MM')                                AS ym,
           SUM(t.target_amt)                                             AS target_amt
    FROM store_targets t
    WHERE t.target_month >= DATEADD(day, -365, CAST(GETDATE() AS DATE))
    GROUP BY t.store_id, FORMAT(t.target_month, 'yyyy-MM')
)
...
WITH kpi AS (
    ...
    FROM orders o
    WHERE o.status = 'completed' AND o.order_date >= DATEADD(day, -30, CAST(GETDATE() AS DATE))
    UNION ALL
    ...
    FROM payroll p
    WHERE p.pay_month = FORMAT(CAST(GETDATE() AS DATE), 'yyyy-MM')
    UNION ALL
