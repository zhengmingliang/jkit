...
-- ------------------------------------------------------------------------------
WITH pm AS (
    SELECT oi.product_id, DATE_FORMAT(o.order_date, '%Y-%m') AS ym,
           SUM(oi.amount) AS amt
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
      AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 120 DAY)
...
FROM rk cur
LEFT JOIN rk prev ON cur.product_id = prev.product_id AND prev.ym = DATE_FORMAT(DATE_SUB(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), INTERVAL 1 MONTH), '%Y-%m')
JOIN products p ON cur.product_id = p.product_id
WHERE cur.ym = DATE_FORMAT(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), '%Y-%m')
...
rf AS (
    ...
    JOIN order_items oi ON o.order_id = oi.order_id
    JOIN products p ON oi.product_id = p.product_id
    WHERE r.refund_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
    GROUP BY p.category_id
)
...
SELECT HOUR(o.order_date) AS hour_of_day,
       CASE WHEN (DAYOFWEEK(o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END AS day_type,
       COUNT(*)                  AS order_cnt,
       SUM(o.pay_amount)         AS gmv,
...
GROUP BY HOUR(o.order_date),
         CASE WHEN (DAYOFWEEK(o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END
...
WITH dur AS (
    SELECT o.order_id, s.province, s.carrier,
           DATEDIFF(s.ship_date, o.order_date) AS deliver_days
    FROM orders o
...
grp AS (
    SELECT customer_id, dt,
           DATEDIFF(dt, CAST('2000-01-01' AS DATE))
...
SELECT CASE WHEN seq = 1 AND next_order_date IS NOT NULL THEN 'has_second' ELSE 'one_time' END AS cust_kind,
       COUNT(*) AS cnt,
       ROUND(AVG(DATEDIFF(next_order_date, order_date)), 2) AS avg_gap_days,
       MIN(DATEDIFF(next_order_date, order_date)) AS min_gap_days,
       MAX(DATEDIFF(next_order_date, order_date)) AS max_gap_days
FROM ranked
WHERE seq = 1
...
SELECT p.product_id, p.product_name, c.category_name, p.launch_date,
       ...
       DATEDIFF(CURRENT_DATE, COALESCE(ls.last_sold_date, p.launch_date)) AS idle_days,
       CASE WHEN ls.product_id IS NULL THEN 'never_sold'
            WHEN DATEDIFF(CURRENT_DATE, ls.last_sold_date) > 90 THEN 'long_idle'
            ELSE 'active' END AS sale_status
FROM products p
LEFT JOIN last_sale ls ON p.product_id = ls.product_id
LEFT JOIN categories c ON p.category_id = c.category_id
WHERE p.status = 'on_sale'
  AND (ls.product_id IS NULL OR DATEDIFF(CURRENT_DATE, ls.last_sold_date) > 90)
...
-- ------------------------------------------------------------------------------
WITH pm AS (
    SELECT oi.product_id, DATE_FORMAT(o.order_date, '%Y-%m') AS ym, SUM(oi.amount) AS amt
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
    GROUP BY oi.product_id, DATE_FORMAT(o.order_date, '%Y-%m')
...
WITH cart AS (
    ...
    FROM carts
    WHERE add_time >= DATE_SUB(CURRENT_DATE, INTERVAL 30 DAY)
),
agg AS (
    SELECT p.category_id,
           COUNT(*)                                          AS add_cnt,
...
WITH cohort AS (
    SELECT c.customer_id,
           DATE_FORMAT(c.register_date, '%Y-%m') AS cohort_ym,
           CAST(DATE_FORMAT(c.register_date, '%Y-%m-01') AS DATE)    AS cohort_md
    FROM customers c
),
act AS (
    SELECT o.customer_id,
           DATE_FORMAT(o.order_date, '%Y-%m') AS act_ym,
           CAST(DATE_FORMAT(o.order_date, '%Y-%m-01') AS DATE)    AS act_md
    FROM orders o
    WHERE o.status = 'completed'
    GROUP BY o.customer_id, DATE_FORMAT(o.order_date, '%Y-%m'), CAST(DATE_FORMAT(o.order_date, '%Y-%m-01') AS DATE)
),
joined AS (
...
SELECT j.cohort_ym,
       PERIOD_DIFF(DATE_FORMAT(j.act_md, '%Y%m'), DATE_FORMAT(j.cohort_md, '%Y%m')) AS month_diff,
...
hist AS (
    SELECT customer_id,
           COUNT(CASE WHEN order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN 1 END) AS cnt_90d,
           COUNT(CASE WHEN order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
                       AND order_date <  DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN 1 END) AS cnt_90_180d
    FROM orders
    WHERE status = 'completed'
    GROUP BY customer_id
)
SELECT ca.customer_id, c.customer_name, c.level,
       ca.last_order_date,
       DATEDIFF(CURRENT_DATE, ca.last_order_date) AS idle_days,
       ca.order_cnt, ROUND(ca.total_amt, 2) AS total_amt,
       h.cnt_90d, h.cnt_90_180d,
       CASE WHEN DATEDIFF(CURRENT_DATE, ca.last_order_date) > 180 THEN 'churned'
            WHEN DATEDIFF(CURRENT_DATE, ca.last_order_date) > 90  THEN 'high_risk'
...
WHERE DATEDIFF(CURRENT_DATE, ca.last_order_date) > 60
...
with_span AS (
    SELECT customer_id, order_cnt, total_amt,
           PERIOD_DIFF(DATE_FORMAT(last_order, '%Y%m'), DATE_FORMAT(first_order, '%Y%m')) AS life_months,
           DATEDIFF(CURRENT_DATE, first_order)    AS days_since_first
    FROM cust
)
SELECT customer_id, order_cnt, ROUND(total_amt, 2) AS ltv,
       life_months,
       ROUND(total_amt / NULLIF(order_cnt, 0), 2)                     AS avg_order_value,
       ROUND(order_cnt / NULLIF(YEAR(CURRENT_DATE) - 2020 + 1, 0), 2) AS annual_frequency,
...
sleeping AS (
    ...
    FROM act
    WHERE DATEDIFF(CURRENT_DATE, last_dt) BETWEEN 90 AND 365
),
reactivated AS (
...
SELECT CASE WHEN r.customer_id IS NOT NULL THEN 'reactivated' ELSE 'still_sleeping' END AS wake_status,
       COUNT(*)                          AS cust_cnt,
       ROUND(AVG(DATEDIFF(COALESCE(r.wake_date, CURRENT_DATE), s.last_dt)), 1) AS avg_sleep_days,
...
SELECT c.customer_id, c.customer_name,
       ...
       COUNT(*)                   AS order_cnt,
       SUM(o.pay_amount)          AS total_amt,
       GROUP_CONCAT(DISTINCT s.city ORDER BY s.city SEPARATOR ',') AS cities
FROM orders o
JOIN customers c ON o.customer_id = c.customer_id
JOIN stores    s ON o.store_id = s.store_id
WHERE o.status = 'completed'
...
SELECT c.customer_id, c.customer_name, c.birth_date, c.city,
       ...
       ROUND(DATEDIFF(CURRENT_DATE, c.register_date) / 365.0, 1) AS reg_years,
       MAX(o.order_date) AS last_order_date,
       SUM(o.pay_amount) AS hist_amt
FROM customers c
LEFT JOIN orders o ON c.customer_id = o.customer_id AND o.status = 'completed'
WHERE MONTH(c.birth_date) = MONTH(CURRENT_DATE)
...
SELECT o.order_id, o.customer_id,
       o.order_date,
       l.last_node_time,
       TIMESTAMPDIFF(HOUR, l.last_node_time, NOW()) AS stall_hours,
       l.last_node,
       CASE WHEN TIMESTAMPDIFF(HOUR, l.last_node_time, NOW()) > 72 THEN 'severely_stalled'
            WHEN TIMESTAMPDIFF(HOUR, l.last_node_time, NOW()) > 48 THEN 'stalled'
            ELSE 'in_transit' END AS logistic_status
FROM orders o
JOIN latest l ON o.order_id = l.order_id
WHERE o.status IN ('shipped', 'delivering')
  AND TIMESTAMPDIFF(HOUR, l.last_node_time, NOW()) > 24
...
WITH actual AS (
    ...
    LEFT JOIN orders o ON s.store_id = o.store_id
         ...
         AND DATE_FORMAT(o.order_date, '%Y-%m') = DATE_FORMAT(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), '%Y-%m')
    GROUP BY s.store_id, s.store_name, s.region
),
target AS (
    SELECT store_id, SUM(budget) AS target_amt
    FROM store_targets
    WHERE DATE_FORMAT(target_month, '%Y-%m') = DATE_FORMAT(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), '%Y-%m')
    GROUP BY store_id
)
...
WITH daily AS (
    ...
    FROM orders
    WHERE status = 'completed'
      AND order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 120 DAY)
    GROUP BY CAST(order_date AS DATE)
),
ma AS (
    SELECT dt, gmv,
...
m AS (
    SELECT DATE_FORMAT(event_time, '%Y-%m') AS ym, COUNT(DISTINCT customer_id) AS mau
    FROM user_events
    WHERE event_time >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
...
FROM d
JOIN m ON DATE_FORMAT(d.dt, '%Y-%m') = m.ym
ORDER BY d.dt;
...
WITH reg AS (
    ...
    FROM customers
    WHERE register_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
),
first_order AS (
    SELECT customer_id, MIN(order_date) AS first_order_date
    FROM orders
...
SELECT DATE_FORMAT(r.register_date, '%Y-%m') AS reg_ym,
       COUNT(*)                                        AS reg_cnt,
       COUNT(fo.customer_id)                           AS converted_cnt,
       ROUND(COUNT(fo.customer_id) / NULLIF(COUNT(*), 0) * 100, 2) AS conv_pct,
       ROUND(AVG(DATEDIFF(fo.first_order_date, r.register_date)), 2) AS avg_conv_days,
       SUM(CASE WHEN DATEDIFF(fo.first_order_date, r.register_date) <= 1 THEN 1 ELSE 0 END) AS within_1d,
       SUM(CASE WHEN DATEDIFF(fo.first_order_date, r.register_date) <= 7 THEN 1 ELSE 0 END) AS within_7d
FROM reg r
LEFT JOIN first_order fo ON r.customer_id = fo.customer_id
GROUP BY DATE_FORMAT(r.register_date, '%Y-%m')
ORDER BY reg_ym;
...
SELECT session_id, customer_id, total_steps,
       GROUP_CONCAT(CONCAT(CONCAT(CAST(step_no AS CHAR(10)), ':'), page_url) ORDER BY step_no SEPARATOR '>') AS path,
       TIMESTAMPDIFF(MINUTE, MIN(event_time), MAX(event_time)) AS session_minutes
FROM seq
WHERE step_no <= 10
...
SELECT ev.product_id, p.product_name,
       ...
       ROUND(AVG(TIMESTAMPDIFF(MINUTE, event_time, next_time)), 2) AS avg_stay_minutes,
       MAX(TIMESTAMPDIFF(MINUTE, event_time, next_time))            AS max_stay_minutes,
       SUM(CASE WHEN next_type = 'add_cart' THEN 1 ELSE 0 END) AS led_to_cart
FROM ev
JOIN products p ON ev.product_id = p.product_id
WHERE next_time IS NOT NULL
  AND TIMESTAMPDIFF(MINUTE, event_time, next_time) BETWEEN 0 AND 60
...
flag AS (
    SELECT customer_id, event_time, event_type,
           CASE WHEN prev_time IS NULL
                     OR TIMESTAMPDIFF(MINUTE, prev_time, event_time) > 30 THEN 1 ELSE 0 END AS is_new
    FROM ev
),
sess AS (
    SELECT customer_id, event_time, event_type,
...
WITH nc AS (
    ...
    FROM customers
    WHERE register_date >= DATE_SUB(CURRENT_DATE, INTERVAL 60 DAY)
),
beh AS (
    ...
    LEFT JOIN user_events e ON n.customer_id = e.customer_id
         AND e.event_time BETWEEN n.register_date AND DATE_ADD(n.register_date, INTERVAL 7 DAY)
...
SELECT p.product_name,
       ...
       ROUND(AVG(DATEDIFF(rep.last_dt, rep.first_dt)), 1)  AS avg_cycle_days,
...
-- ------------------------------------------------------------------------------
WITH cust_price AS (
    SELECT o.customer_id, DATE_FORMAT(o.order_date, '%Y-%m') AS ym,
...
WITH cm AS (
    SELECT p.category_id,
           SUM(CASE WHEN YEAR(o.order_date) = YEAR(CURRENT_DATE)   THEN oi.amount ELSE 0 END) AS amt_cur,
           SUM(CASE WHEN YEAR(o.order_date) = YEAR(CURRENT_DATE) - 1 THEN oi.amount ELSE 0 END) AS amt_prev
    FROM order_items oi
    JOIN products p ON oi.product_id = p.product_id
    JOIN orders   o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT oi.product_id, DATE_FORMAT(o.order_date, '%Y-%m') AS ym, SUM(oi.quantity) AS qty
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
      AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 120 DAY)
...
piv AS (
    SELECT product_id,
           SUM(CASE WHEN ym = DATE_FORMAT(DATE_SUB(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), INTERVAL 2 MONTH), '%Y-%m') THEN qty ELSE 0 END) AS q1,
           SUM(CASE WHEN ym = DATE_FORMAT(DATE_SUB(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), INTERVAL 1 MONTH), '%Y-%m') THEN qty ELSE 0 END) AS q2,
           SUM(CASE WHEN ym = DATE_FORMAT(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), '%Y-%m') THEN qty ELSE 0 END) AS q3
...
promo_days AS (
    SELECT DISTINCT CAST(DATE_ADD(d.dt, INTERVAL 0 DAY) AS DATE) AS dt
    FROM daily d
...
retention AS (
    ...
    LEFT JOIN orders o ON f.customer_id = o.customer_id
         AND o.order_date > DATE_SUB(CURRENT_DATE, INTERVAL 0 DAY)
    WHERE f.rn = 1
...
FROM shipments s
WHERE s.ship_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
GROUP BY s.carrier
...
SELECT reason,
       ...
       ROUND(AVG(DATEDIFF(refund_date, order_date)), 1)  AS avg_refund_lag_days,
       MAX(DATEDIFF(refund_date, order_date))            AS max_lag_days,
...
FROM payments p
WHERE p.pay_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
...
WITH dp AS (
    ...
    FROM payments
    WHERE pay_date >= DATE_SUB(CURRENT_DATE, INTERVAL 60 DAY)
...
SELECT o.order_id, o.customer_id, ROUND(o.pay_amount, 2) AS pay_amount, o.order_date,
       ...
       DATEDIFF(o.order_date, h.first_order)   AS days_since_first,
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN (DAYOFWEEK(o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END AS day_type,
       (DAYOFWEEK(o.order_date) - 1) AS dow,
       COUNT(*)                    AS order_cnt,
       COUNT(DISTINCT o.customer_id) AS uv,
...
WHERE o.status = 'completed'
  AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
GROUP BY CASE WHEN (DAYOFWEEK(o.order_date) - 1) IN (0, 6) THEN 'weekend' ELSE 'weekday' END,
         (DAYOFWEEK(o.order_date) - 1)
ORDER BY dow;
...
recent AS (
    ...
    FROM customers c
    JOIN orders o ON c.customer_id = o.customer_id
    WHERE o.status = 'completed' AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
    GROUP BY c.province
)
...
WITH pm AS (
    SELECT s.store_id, s.store_name, s.region,
           SUM(CASE WHEN o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN o.pay_amount ELSE 0 END) AS gmv_90d,
           SUM(CASE WHEN o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
                     AND o.order_date <  DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN o.pay_amount ELSE 0 END) AS gmv_prev_90d,
           COUNT(CASE WHEN o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY) THEN 1 END) AS orders_90d,
           COUNT(CASE WHEN o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
                       AND o.order_date <  DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY) THEN 1 END) AS orders_prev_90d
    FROM stores s
    LEFT JOIN orders o ON s.store_id = o.store_id AND o.status = 'completed'
    GROUP BY s.store_id, s.store_name, s.region
)
...
WITH amt AS (
    SELECT customer_id,
           SUM(CASE WHEN order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN pay_amount ELSE 0 END) AS amt_recent,
           SUM(CASE WHEN order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
                     AND order_date <  DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN pay_amount ELSE 0 END) AS amt_prev
    FROM orders
    WHERE status = 'completed'
    GROUP BY customer_id
),
...
SELECT CASE WHEN pd.view_times = 1 THEN 'view_1'
       ...
       ROUND(AVG(TIMESTAMPDIFF(MINUTE, pd.first_view, pd.last_view)), 2)     AS avg_dwell_minutes
FROM pd
...
WITH base AS (
    SELECT c.customer_id, c.customer_name, c.city, c.level, c.gender,
           DATEDIFF(CURRENT_DATE, c.birth_date) / 365 AS age
    FROM customers c
),
beh AS (
    SELECT o.customer_id,
...
SELECT b.customer_id, b.customer_name, b.city,
       ...
       DATEDIFF(CURRENT_DATE, beh.last_order) AS idle_days,
...
stage AS (
    SELECT customer_id, order_cnt, total_amt,
           DATEDIFF(CURRENT_DATE, first_dt) AS days_since_first,
           DATEDIFF(CURRENT_DATE, last_dt)  AS days_since_last
    FROM cust
)
...
WITH np AS (
    ...
    FROM products p
    WHERE p.launch_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
),
perf AS (
    SELECT n.product_id,
    ...
    LEFT JOIN orders o ON oi.order_id = o.order_id AND o.status = 'completed'
         ...
         AND o.order_date <= DATE_ADD(n.launch_date, INTERVAL 90 DAY)
...
-- ------------------------------------------------------------------------------
WITH mr AS (
    SELECT product_id, DATE_FORMAT(created_at, '%Y-%m') AS ym,
...
SELECT channel,
       COUNT(*)                                                       AS order_cnt,
       ROUND(AVG(DATEDIFF(first_pay_date, order_date)), 2)          AS avg_pay_lag_days,
       MAX(DATEDIFF(first_pay_date, order_date))                    AS max_pay_lag_days,
       SUM(CASE WHEN DATEDIFF(first_pay_date, order_date) <= 0 THEN 1 ELSE 0 END) AS same_day_paid,
       ROUND(SUM(CASE WHEN DATEDIFF(first_pay_date, order_date) <= 0 THEN 1 ELSE 0 END)
...
WITH reg AS (
    ...
    FROM customers
    WHERE register_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
...
WITH acct_stat AS (
    SELECT account_id,
           ...
           COUNT(*)    AS txn_cnt
    FROM transactions
    WHERE txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
    GROUP BY account_id
    HAVING COUNT(*) >= 10
),
tagged AS (
    ...
    WHERE t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY)
)
...
WITH hourly AS (
    SELECT account_id,
           DATE_FORMAT(txn_date, '%Y-%m-%d %H') AS hh,
           COUNT(*)      AS cnt,
           SUM(amount)   AS amt
    FROM transactions
    WHERE txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY)
    GROUP BY account_id, DATE_FORMAT(txn_date, '%Y-%m-%d %H')
),
agg AS (
    SELECT account_id,
           SUM(cnt) AS txn_cnt,
...
WITH cp AS (
    ...
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
...
SELECT cust_id, account_id, txn_id,
       ...
       txn_date, next_date,
       TIMESTAMPDIFF(HOUR, txn_date, next_date) AS gap_hours,
       ...
       CASE WHEN txn_type = 'transfer_out' AND next_type = 'transfer_in'
                 AND TIMESTAMPDIFF(HOUR, txn_date, next_date) <= 24
...
SELECT a.account_type,
       COUNT(*)                                                   AS account_cnt,
       SUM(CASE WHEN DATEDIFF(CURRENT_DATE, lt.last_dt) <= 30  THEN 1 ELSE 0 END) AS active_30d,
       SUM(CASE WHEN DATEDIFF(CURRENT_DATE, lt.last_dt) BETWEEN 31 AND 90 THEN 1 ELSE 0 END) AS low_active,
       SUM(CASE WHEN DATEDIFF(CURRENT_DATE, lt.last_dt) BETWEEN 91 AND 365 THEN 1 ELSE 0 END) AS dormant,
       SUM(CASE WHEN lt.account_id IS NULL
                     OR DATEDIFF(CURRENT_DATE, lt.last_dt) > 365 THEN 1 ELSE 0 END) AS inactive,
       ROUND(AVG(DATEDIFF(CURRENT_DATE, lt.last_dt)), 1) AS avg_idle_days,
       ROUND(SUM(CASE WHEN DATEDIFF(CURRENT_DATE, lt.last_dt) <= 30 THEN 1 ELSE 0 END)
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN PERIOD_DIFF(DATE_FORMAT(maturity_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) <= 3  THEN 'demand_like'
            WHEN PERIOD_DIFF(DATE_FORMAT(maturity_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) <= 12 THEN 'short_term'
            WHEN PERIOD_DIFF(DATE_FORMAT(maturity_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) <= 36 THEN 'medium_term'
...
FROM deposits
WHERE status = 'active'
GROUP BY CASE WHEN PERIOD_DIFF(DATE_FORMAT(maturity_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) <= 3  THEN 'demand_like'
              WHEN PERIOD_DIFF(DATE_FORMAT(maturity_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) <= 12 THEN 'short_term'
              WHEN PERIOD_DIFF(DATE_FORMAT(maturity_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) <= 36 THEN 'medium_term'
              ELSE 'long_term' END
ORDER BY total_amt DESC;
...
WITH mat AS (
    SELECT d.deposit_id, d.cust_id, d.amount, d.rate, d.maturity_date,
           DATEDIFF(d.maturity_date, CURRENT_DATE) AS days_to_maturity
    FROM deposits d
...
SELECT CONCAT(CONCAT(CAST(has_deposit AS CHAR(1)), CAST(has_fund AS CHAR(1))),
              CONCAT(CAST(has_loan AS CHAR(1)), CAST(has_card AS CHAR(1)))) AS product_combo,
       COUNT(*) AS cust_cnt,
...
FROM hold
GROUP BY CONCAT(CONCAT(CAST(has_deposit AS CHAR(1)), CAST(has_fund AS CHAR(1))),
                CONCAT(CAST(has_loan AS CHAR(1)), CAST(has_card AS CHAR(1)))),
         CASE WHEN has_deposit = 1 AND has_fund = 0 THEN 'cross_sell_fund'
              WHEN has_deposit = 1 AND has_card = 0 THEN 'cross_sell_card'
              WHEN has_loan = 1 AND has_deposit = 0 THEN 'cross_sell_deposit'
              ELSE 'none' END
...
SELECT t.channel,
       ...
       ROUND(SUM(t.amount) / NULLIF(SUM(SUM(t.amount)) OVER (PARTITION BY DATE_FORMAT(t.txn_date, '%Y-%m')), 0) * 100, 2) AS channel_share_pct,
       ROUND(SUM(t.amount) - LAG(SUM(t.amount)) OVER (PARTITION BY t.channel ORDER BY DATE_FORMAT(t.txn_date, '%Y-%m')), 2) AS mom_delta
FROM transactions t
JOIN accounts a ON t.account_id = a.account_id
WHERE t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
GROUP BY t.channel, DATE_FORMAT(t.txn_date, '%Y-%m')
...
WITH t AS (
    SELECT t.txn_id, t.account_id, t.amount, t.txn_date, t.channel,
           a.cust_id,
           HOUR(t.txn_date) AS txn_hour,
           (DAYOFWEEK(t.txn_date) - 1)  AS txn_dow
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
...
pairs AS (
    ...
    JOIN cust_acct a2 ON t.counterparty = CAST(a2.account_id AS CHAR(20))
    WHERE a1.cust_id < a2.cust_id
      AND t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
...
WITH edges AS (
    SELECT t.account_id AS from_acct, a2.account_id AS to_acct,
           COUNT(*) AS txn_cnt, SUM(t.amount) AS total_amt
    FROM transactions t
    JOIN accounts a2 ON t.counterparty = CAST(a2.account_id AS CHAR(20))
...
FROM risk_events
WHERE event_time >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
...
WITH evt AS (
    ...
    FROM risk_events
    WHERE event_time >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY cust_id
),
score AS (
...
WITH suspicious AS (
    SELECT t.account_id, a.cust_id, t.txn_id, t.amount, t.txn_date, t.txn_type, t.city,
           ...
           CASE WHEN HOUR(t.txn_date) >= 20 OR HOUR(t.txn_date) < 8 THEN 2 ELSE 0 END AS time_score
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 30 DAY)
),
...
SELECT s.cust_id, c.cust_name, c.risk_level,
       ...
       GROUP_CONCAT(CAST(txn_id AS CHAR(20)) ORDER BY txn_id SEPARATOR ',') AS txn_list,
...
evt_hist AS (
    SELECT cust_id,
           DATE_FORMAT(event_time, '%Y-%m') AS ym,
           MAX(CASE WHEN risk_level = 'high'   THEN 3
                    WHEN risk_level = 'medium' THEN 2
                    ELSE 1 END) AS level_num
    FROM risk_events
    WHERE event_time >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
    GROUP BY cust_id, DATE_FORMAT(event_time, '%Y-%m')
),
trend AS (
    SELECT cust_id,
...
SELECT a.account_type,
       DATE_FORMAT(a.open_date, '%Y-%m') AS open_ym,
       COUNT(*)                                                     AS opened_cnt,
       COUNT(ft.account_id)                                         AS activated_cnt,
       ROUND(COUNT(ft.account_id) / NULLIF(COUNT(*), 0) * 100, 2)   AS activation_pct,
       ROUND(AVG(DATEDIFF(ft.first_dt, a.open_date)), 2)          AS avg_activation_days,
       SUM(CASE WHEN DATEDIFF(ft.first_dt, a.open_date) <= 7 THEN 1 ELSE 0 END) AS within_7d,
       SUM(CASE WHEN DATEDIFF(ft.first_dt, a.open_date) <= 30 THEN 1 ELSE 0 END) AS within_30d
FROM accounts a
LEFT JOIN first_txn ft ON a.account_id = ft.account_id
WHERE a.open_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
GROUP BY a.account_type, DATE_FORMAT(a.open_date, '%Y-%m')
...
-- ------------------------------------------------------------------------------
WITH act AS (
    SELECT DISTINCT a.cust_id, DATE_FORMAT(t.txn_date, '%Y-%m') AS ym
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
),
seq AS (
    SELECT cust_id, ym,
...
tenure AS (
    SELECT cust_id,
           DATEDIFF(CURRENT_DATE, MIN(open_date)) / 365.0 AS tenure_years
...
WITH act AS (
    SELECT a.cust_id,
           SUM(CASE WHEN t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN 1 ELSE 0 END) AS cnt_90d,
           SUM(CASE WHEN t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
                     AND t.txn_date <  DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)  THEN 1 ELSE 0 END) AS cnt_prev_90d,
...
SELECT c.cust_id, c.cust_name, c.cust_type,
       ...
       DATEDIFF(CURRENT_DATE, a.last_txn) AS idle_days,
       ROUND(b.cur_balance, 2)           AS cur_balance,
       ROUND((a.cnt_90d - a.cnt_prev_90d) / NULLIF(a.cnt_prev_90d, 0) * 100, 2) AS activity_change_pct,
       CASE WHEN DATEDIFF(CURRENT_DATE, a.last_txn) > 180 THEN 'churned'
            WHEN DATEDIFF(CURRENT_DATE, a.last_txn) > 90
...
WHERE DATEDIFF(CURRENT_DATE, a.last_txn) > 60
...
WITH r AS (
    ...
    FROM fin_reports
    WHERE period >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY), '%Y-%m')
    GROUP BY branch_id
)
SELECT b.branch_name, b.city, b.region,
...
WITH t AS (
    ...
    FROM card_txns ct
    WHERE ct.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
)
SELECT card_id,
       COUNT(*)                                                    AS txn_cnt,
...
WITH e1 AS (
    ...
    JOIN accounts a2 ON t.counterparty = CAST(a2.account_id AS CHAR(20))
    WHERE t.txn_type = 'transfer_out'
      AND t.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 60 DAY)
    GROUP BY t.account_id, a2.account_id
),
e2 AS (
    ...
    WHERE t2.txn_type = 'transfer_out'
      AND t2.txn_date >= DATE_SUB(CURRENT_DATE, INTERVAL 60 DAY)
)
...
SELECT CAST(t.txn_date AS DATE) AS dt,
       ...
       SUM(CASE WHEN HOUR(t.txn_date) >= 20
                  OR HOUR(t.txn_date) < 8 THEN 1 ELSE 0 END)    AS offhour_cnt,
...
-- ------------------------------------------------------------------------------
WITH stage AS (
    SELECT r.loan_id, DATE_FORMAT(r.due_date, '%Y-%m') AS ym,
           MAX(r.overdue_days) AS max_od
    FROM repayments r
    GROUP BY r.loan_id, DATE_FORMAT(r.due_date, '%Y-%m')
),
bucketed AS (
    SELECT loan_id, ym,
...
SELECT CASE WHEN term_months <= 6   THEN 'short_0_6m'
       ...
       SUM(CASE WHEN PERIOD_DIFF(DATE_FORMAT(end_date, '%Y%m'), DATE_FORMAT(CURRENT_DATE, '%Y%m')) <= 12 THEN 1 ELSE 0 END) AS maturing_12m,
       ROUND(SUM(CASE WHEN PERIOD_DIFF(DATE_FORMAT(end_date, '%Y%m'), DATE_FORMAT(CURRENT_DATE, '%Y%m')) <= 12 THEN loan_amount ELSE 0 END), 2) AS repricing_amt,
       ROUND(SUM(CASE WHEN PERIOD_DIFF(DATE_FORMAT(end_date, '%Y%m'), DATE_FORMAT(CURRENT_DATE, '%Y%m')) <= 12 THEN loan_amount ELSE 0 END)
...
WITH rb AS (
    SELECT l.loan_id, l.cust_id,
           ...
           AVG(DATEDIFF(r.repay_date, r.due_date))                           AS avg_pay_gap,
           MAX(r.overdue_days)                                                 AS max_od,
...
SELECT product_type,
       ...
       ROUND(AVG(PERIOD_DIFF(DATE_FORMAT(last_pay, '%Y%m'), DATE_FORMAT(start_date, '%Y%m'))), 1)               AS avg_actual_months
FROM rp
GROUP BY product_type
...
by_dim AS (
    ...
    UNION ALL
    SELECT 'branch', CAST(branch_id AS CHAR(10)), SUM(loan_amount)
    FROM loans WHERE status IN ('active', 'overdue') GROUP BY branch_id
    UNION ALL
    SELECT 'cust', CAST(cust_id AS CHAR(20)), SUM(loan_amount)
    FROM loans WHERE status IN ('active', 'overdue') GROUP BY cust_id
)
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT DATE_FORMAT(start_date, '%Y-%m') AS ym, product_type,
           COUNT(*)          AS loan_cnt,
           SUM(loan_amount)  AS loan_amt
    FROM loans
    WHERE start_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
    GROUP BY DATE_FORMAT(start_date, '%Y-%m'), product_type
)
...
SELECT od_stage,
       ...
       ROUND(AVG(DATEDIFF(repay_date, due_date)), 1)            AS avg_recovery_days,
...
WITH orig AS (
    SELECT loan_id, cust_id, product_type, loan_amount, term_months,
           start_date, end_date,
           PERIOD_DIFF(DATE_FORMAT(end_date, '%Y%m'), DATE_FORMAT(start_date, '%Y%m')) AS actual_months
    FROM loans
)
SELECT product_type,
       COUNT(*)                                                            AS loan_cnt,
...
-- ------------------------------------------------------------------------------
SELECT DATE_FORMAT(l.start_date, '%Y-%m') AS vintage,
       COUNT(*)                                                        AS loan_cnt,
...
FROM loans l
WHERE l.start_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1095 DAY)
...
WITH n AS (
    ...
    FROM fund_nav
    WHERE nav_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
),
dd AS (
...
SELECT c.cust_id, c.cust_name,
       ...
       ROUND(AVG(DATEDIFF(CURRENT_DATE, h.purchase_date)), 0)              AS avg_holding_days,
       MAX(DATEDIFF(CURRENT_DATE, h.purchase_date))                        AS max_holding_days,
...
monthly AS (
    SELECT fund_id, DATE_FORMAT(purchase_date, '%Y-%m') AS ym,
           COUNT(*)          AS subscribe_cnt,
           SUM(cost_amount)  AS subscribe_amt
    FROM h GROUP BY fund_id, DATE_FORMAT(purchase_date, '%Y-%m')
)
...
perf AS (
    ...
    FROM fund_nav n
    WHERE n.nav_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY n.fund_id
)
SELECT m.manager_id,
       COUNT(*)                                 AS fund_cnt,
...
SELECT f.fund_name,
       ...
       ROUND(AVG(DATEDIFF(CURRENT_DATE, purchase_date)), 0)               AS avg_holding_days
FROM pos
...
WITH dep AS (
    SELECT d.deposit_id, d.cust_id, d.amount, d.rate, d.start_date, d.maturity_date,
           DATEDIFF(d.maturity_date, d.start_date)  AS days_held,
           PERIOD_DIFF(DATE_FORMAT(d.maturity_date, '%Y%m'), DATE_FORMAT(d.start_date, '%Y%m')) AS months_held
    FROM deposits d
)
SELECT CASE WHEN months_held <= 3  THEN 'demand_like'
       ...
       SUM(CASE WHEN maturity_date <= CURRENT_DATE THEN 1 ELSE 0 END)     AS matured_cnt,
       ROUND(SUM(CASE WHEN maturity_date <= CURRENT_DATE THEN amount * rate * days_held / 365 ELSE 0 END), 2) AS matured_interest
FROM dep
...
WITH r AS (
    ...
    FROM fx_rates
    WHERE rate_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
)
SELECT currency,
       COUNT(*)                                          AS quote_days,
...
WITH br AS (
    ...
    FROM fin_reports r
    WHERE r.period >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY), '%Y-%m')
...
WITH rev AS (
    SELECT r.branch_id,
           SUM(r.revenue) AS total_revenue,
           SUM(CASE WHEN r.period >= DATE_FORMAT(CAST(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AS DATE), '%Y-%m')
...
WITH RECURSIVE org_tree AS (
    SELECT dept_id, dept_name, parent_dept_id, 1 AS lvl,
           CAST(dept_id AS CHAR(200)) AS path
    FROM departments
    WHERE parent_dept_id IS NULL
    UNION ALL
    SELECT d.dept_id, d.dept_name, d.parent_dept_id, t.lvl + 1,
           CAST(CONCAT(CONCAT(t.path, '>'), CAST(d.dept_id AS CHAR(200))) AS CHAR(200))
    FROM departments d
...
WITH RECURSIVE chain AS (
    SELECT emp_id, emp_name, manager_id, 1 AS depth,
           CAST(emp_id AS CHAR(200)) AS chain_path, CAST(emp_name AS CHAR(200)) AS chain_names
    FROM employees
    WHERE manager_id IS NULL
    UNION ALL
    SELECT e.emp_id, e.emp_name, e.manager_id, c.depth + 1,
           CAST(CONCAT(CONCAT(c.chain_path, '>'), CAST(e.emp_id AS CHAR(200))) AS CHAR(200)),
           CAST(CONCAT(CONCAT(c.chain_names, '>'), e.emp_name) AS CHAR(200))
    FROM employees e
...
WITH e AS (
    SELECT emp_id, dept_id, job_level, status,
           DATEDIFF(COALESCE(leave_date, CURRENT_DATE), hire_date) / 365.0 AS tenure_years
    FROM employees
)
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 26 THEN 'gen_z_below26'
            WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 36 THEN 'millennial_26_35'
            WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 46 THEN 'gen_x_36_45'
            WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 56 THEN 'boomer_46_55'
            ELSE 'near_retirement_56plus' END AS age_band,
       COUNT(*)                                                   AS emp_cnt,
       ROUND(AVG(DATEDIFF(CURRENT_DATE, birth_date) / 365.0), 1)   AS avg_age,
       SUM(CASE WHEN gender = 'M' THEN 1 ELSE 0 END)              AS male_cnt,
       SUM(CASE WHEN gender = 'F' THEN 1 ELSE 0 END)              AS female_cnt,
       ROUND(AVG(DATEDIFF(CURRENT_DATE, hire_date) / 365.0), 2)    AS avg_tenure,
       ROUND(AVG(salary), 2)                                      AS avg_salary,
       ROUND(COUNT(*) / NULLIF(SUM(COUNT(*)) OVER (), 0) * 100, 2) AS emp_share_pct
FROM employees
WHERE status = 'active'
GROUP BY CASE WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 26 THEN 'gen_z_below26'
              WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 36 THEN 'millennial_26_35'
              WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 46 THEN 'gen_x_36_45'
              WHEN DATEDIFF(CURRENT_DATE, birth_date) / 365 < 56 THEN 'boomer_46_55'
...
-- ------------------------------------------------------------------------------
WITH hires AS (
    SELECT DATE_FORMAT(hire_date, '%Y-%m') AS ym, COUNT(*) AS hire_cnt
    FROM employees
    WHERE hire_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
    GROUP BY DATE_FORMAT(hire_date, '%Y-%m')
),
leaves AS (
    SELECT DATE_FORMAT(leave_date, '%Y-%m') AS ym, COUNT(*) AS leave_cnt
    FROM employees
    WHERE leave_date IS NOT NULL
      AND leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
...
-- ------------------------------------------------------------------------------
WITH base AS (
    SELECT DATE_FORMAT(hire_date, '%Y-%m') AS ym,
           COUNT(*) AS cohort_size,
           SUM(CASE WHEN leave_date IS NOT NULL THEN 1 ELSE 0 END) AS left_cnt,
           AVG(DATEDIFF(leave_date, hire_date)) AS avg_days_to_leave
    FROM employees
    GROUP BY DATE_FORMAT(hire_date, '%Y-%m')
)
...
WITH nh AS (
    SELECT emp_id, hire_date, dept_id,
           leave_date,
           DATEDIFF(leave_date, hire_date) AS days_to_leave
    FROM employees
    WHERE hire_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
)
SELECT DATE_FORMAT(hire_date, '%Y-%m') AS hire_ym,
       COUNT(*)                                                        AS hired_cnt,
...
recent_left AS (
    ...
    FROM employees
    WHERE leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
    GROUP BY dept_id
)
SELECT d.dept_name,
       ds.active_cnt,
...
SELECT d.dept_name, d.cost_center,
       ...
       SUM(CASE WHEN e.leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY) THEN 1 ELSE 0 END) AS left_1y,
       ROUND(SUM(CASE WHEN e.leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY) THEN 1 ELSE 0 END)
             / NULLIF(SUM(CASE WHEN e.status = 'active' THEN 1 ELSE 0 END), 0) * 100, 2) AS annual_turnover_pct,
       ROUND(AVG(DATEDIFF(CURRENT_DATE, e.hire_date) / 365.0), 2)        AS avg_tenure,
       ROUND(AVG(e.salary), 2)                                          AS avg_salary,
       RANK() OVER (ORDER BY SUM(CASE WHEN e.leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY) THEN 1 ELSE 0 END)
...
SELECT c.change_type,
       DATE_FORMAT(c.change_date, '%Y-%m') AS change_ym,
       COUNT(*)                                            AS change_cnt,
...
FROM emp_changes c
WHERE c.change_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
GROUP BY c.change_type, DATE_FORMAT(c.change_date, '%Y-%m')
...
tenure AS (
    SELECT emp_id, DATEDIFF(CURRENT_DATE, hire_date) / 365.0 AS tenure_years
    FROM employees
)
...
FROM payroll p
WHERE p.pay_month >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY), '%Y-%m')
...
WHERE p.pay_month >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY), '%Y-%m')
GROUP BY d.dept_name, d.cost_center
...
WITH lv AS (
    SELECT e.emp_id, e.job_level, e.position, e.gender, e.salary, e.dept_id,
           DATEDIFF(CURRENT_DATE, e.hire_date) / 365.0 AS tenure_years,
...
WITH ot AS (
    ...
    WHERE a.att_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
...
WITH a AS (
    ...
    FROM attendance
    WHERE att_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
    GROUP BY emp_id
)
...
WITH late AS (
    ...
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
...
grp AS (
    SELECT emp_id, att_date, status, is_abnormal,
           DATEDIFF(att_date, CAST('2000-01-01' AS DATE))
...
WHERE l.start_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
...
SELECT e.emp_id, e.emp_name, d.dept_name,
       ...
       DATEDIFF(CAST('2026-12-31' AS DATE), CURRENT_DATE)                 AS days_to_expire,
...
-- ------------------------------------------------------------------------------
SELECT DATE_FORMAT(a.att_date, '%Y-%m') AS ym,
       COUNT(DISTINCT a.emp_id)                                         AS emp_cnt,
       ...
       ROUND(SUM(a.overtime_hours) - LAG(SUM(a.overtime_hours))
             OVER (ORDER BY DATE_FORMAT(a.att_date, '%Y-%m')), 1)             AS ot_mom_delta
FROM attendance a
WHERE a.att_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
GROUP BY DATE_FORMAT(a.att_date, '%Y-%m')
ORDER BY ym DESC;
...
att AS (
    ...
    FROM attendance
    WHERE att_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY)
    GROUP BY emp_id
),
tr AS (
...
SELECT e.emp_id, e.emp_name, e.position, e.job_level, d.dept_name,
       ...
       ROUND(DATEDIFF(CURRENT_DATE, e.hire_date) / 365.0, 2) AS tenure_years,
       e.salary,
...
WITH team AS (
    SELECT e.manager_id,
           ...
           AVG(DATEDIFF(CURRENT_DATE, e.hire_date) / 365.0)             AS avg_team_tenure,
...
WITH hc AS (
    SELECT COUNT(*) AS total_emp,
           ...
           SUM(CASE WHEN leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY) THEN 1 ELSE 0 END) AS left_1y,
           ROUND(AVG(salary), 2) AS avg_salary,
           ROUND(AVG(DATEDIFF(CURRENT_DATE, hire_date) / 365.0), 2) AS avg_tenure
    FROM employees
),
cost AS (
    ...
    FROM payroll
    WHERE pay_month >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY), '%Y-%m')
),
perf AS (
...
WITH req AS (
    ...
    FROM recruitment r
    WHERE r.open_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY r.dept_id
)
SELECT d.dept_name,
...
-- ------------------------------------------------------------------------------
SELECT DATE_FORMAT(r.open_date, '%Y-%m') AS open_ym,
       r.position,
       COUNT(*)                                                    AS req_cnt,
       ...
       ROUND(AVG(DATEDIFF(COALESCE(r.close_date, CURRENT_DATE), r.open_date)), 1) AS avg_days_open,
       ROUND(MAX(DATEDIFF(COALESCE(r.close_date, CURRENT_DATE), r.open_date)), 1) AS max_days_open,
       SUM(CASE WHEN r.status = 'open'
                 AND DATEDIFF(CURRENT_DATE, r.open_date) > 60 THEN 1 ELSE 0 END) AS backlog_over_60d,
       SUM(r.hired_cnt)                                            AS hired_cnt
FROM recruitment r
WHERE r.open_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
GROUP BY DATE_FORMAT(r.open_date, '%Y-%m'), r.position
...
WITH tr AS (
    ...
    WHERE t.train_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY t.emp_id
),
dept AS (
...
SELECT p.project_name, p.status, d.dept_name,
       ...
       ROUND(DATEDIFF(COALESCE(p.end_date, CURRENT_DATE), p.start_date) / 30.0, 1) AS duration_months,
...
WITH emp_load AS (
    SELECT pa.emp_id,
           ...
           GROUP_CONCAT(CAST(pa.project_id AS CHAR(50)) ORDER BY pa.project_id SEPARATOR ',')  AS project_list
    FROM project_assignments pa
    JOIN projects p ON pa.project_id = p.project_id
...
WITH pm AS (
    ...
    FROM projects p
    WHERE p.start_date >= DATE_SUB(CURRENT_DATE, INTERVAL 730 DAY)
)
...
WITH chg AS (
    ...
    FROM emp_changes c
    WHERE c.change_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1095 DAY)
)
SELECT d.dept_name,
...
gap AS (
    SELECT emp_id, promo_seq,
           DATEDIFF(change_date, prev_promo_date) / 365.0 AS years_between,
...
WITH lv AS (
    SELECT e.emp_id, e.dept_id, e.job_level, e.position,
           DATEDIFF(COALESCE(e.leave_date, CURRENT_DATE), e.hire_date) / 365.0 AS tenure_at_exit,
           c.change_type,
           p.avg_score,
           a.abnormal_rate
    FROM employees e
...
WITH demo AS (
    SELECT e.dept_id,
           ...
           CASE WHEN (YEAR(CURRENT_DATE) - YEAR(e.birth_date)) < 26 THEN 'gen_z'
                WHEN (YEAR(CURRENT_DATE) - YEAR(e.birth_date)) < 36 THEN 'millennial'
                WHEN (YEAR(CURRENT_DATE) - YEAR(e.birth_date)) < 46 THEN 'gen_x'
...
SELECT d.dept_name,
       ...
       ROUND(AVG((YEAR(CURRENT_DATE) - YEAR(a.birth_date))), 1)             AS avg_age,
...
WITH sig AS (
    ...
    LEFT JOIN (SELECT emp_id, SUM(days) AS leave_days FROM leaves
               WHERE start_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
...
WITH act AS (
    SELECT e.dept_id,
           DATE_FORMAT(p.pay_date, '%Y-%m')                                   AS pay_ym,
           SUM(p.gross_pay)                                             AS actual_cost,
           COUNT(DISTINCT p.emp_id)                                     AS paid_headcount
    FROM payroll p
    JOIN employees e ON p.emp_id = e.emp_id
    WHERE p.pay_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY e.dept_id, DATE_FORMAT(p.pay_date, '%Y-%m')
),
bud AS (
...
WITH y AS (
    ...
    FROM payroll p
    JOIN employees e ON p.emp_id = e.emp_id
    WHERE p.pay_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1095 DAY)
...
WITH emp_val AS (
    ...
    LEFT JOIN (SELECT emp_id, SUM(gross_pay) AS total_gross FROM payroll
               WHERE pay_month >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 1095 DAY), '%Y-%m')
...
WITH pos AS (
    SELECT e.dept_id, e.position, e.job_level,
           ...
           SUM(CASE WHEN e.leave_date IS NOT NULL
                     AND e.leave_date >= DATE_SUB(CURRENT_DATE, INTERVAL 180 DAY) THEN 1 ELSE 0 END) AS recent_leavers
    FROM employees e
...
WITH risk AS (
    ...
    LEFT JOIN (SELECT emp_id,
               ...
               GROUP BY emp_id) a ON e.emp_id = a.emp_id
    LEFT JOIN (SELECT emp_id, SUM(days) AS leave_days FROM leaves
               WHERE start_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY) GROUP BY emp_id) lv
...
at AS (
    ...
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
    GROUP BY e.dept_id
),
cost AS (
    ...
    WHERE py.pay_month >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY), '%Y-%m')
    GROUP BY e.dept_id
)
SELECT d.dept_name,
...
SELECT 'customers'                                                       AS table_name,
       ...
       SUM(CASE WHEN c.register_date > CURRENT_DATE THEN 1 ELSE 0 END)      AS future_date_rows,
       SUM(CASE WHEN (YEAR(CURRENT_DATE) - YEAR(c.register_date)) > 50 THEN 1 ELSE 0 END) AS implausible_rows
FROM customers c
UNION ALL
SELECT 'orders',
       COUNT(*),
       ...
       ROUND(SUM(CASE WHEN o.customer_id IS NULL THEN 1 ELSE 0 END) * 100.0
             / NULLIF(COUNT(*), 0), 2),
       SUM(CASE WHEN o.order_date > CURRENT_DATE THEN 1 ELSE 0 END),
...
SELECT 'employees',
       ...
       ROUND(SUM(CASE WHEN e.emp_name IS NULL THEN 1 ELSE 0 END) * 100.0
             / NULLIF(COUNT(*), 0), 2),
       SUM(CASE WHEN e.hire_date > CURRENT_DATE THEN 1 ELSE 0 END),
...
SELECT 'transactions',
       ...
       ROUND(SUM(CASE WHEN t.account_id IS NULL THEN 1 ELSE 0 END) * 100.0
             / NULLIF(COUNT(*), 0), 2),
       SUM(CASE WHEN t.txn_date > CURRENT_DATE THEN 1 ELSE 0 END),
...
WITH dup_cust AS (
    SELECT c.customer_name, c.city,
           ...
           GROUP_CONCAT(CAST(c.customer_id AS CHAR(50)) ORDER BY c.customer_id SEPARATOR ',')             AS id_list
    FROM customers c
...
dup_emp AS (
    SELECT e.emp_name, e.dept_id,
           ...
           GROUP_CONCAT(CAST(e.emp_id AS CHAR(50)) ORDER BY e.emp_id SEPARATOR ',')                       AS id_list
    FROM employees e
...
SELECT 'customers'                                                       AS entity,
       ...
       CAST(d.keep_id AS CHAR(50))                                              AS suggested_keep_id,
       d.dup_cnt, d.id_list,
...
SELECT DATE_FORMAT(c.order_date, '%Y-%m')                                      AS recon_ym,
       COUNT(*)                                                          AS order_cnt,
...
FROM cmp c
GROUP BY DATE_FORMAT(c.order_date, '%Y-%m')
...
target AS (
    ...
    FROM store_targets
    WHERE DATE_FORMAT(target_month, '%Y-%m') >= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY), '%Y-%m')
    GROUP BY store_id
)
...
WITH base AS (
    ...
    WHERE o.status = 'completed'
      AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY s.city, c.category_name, DATE_FORMAT(o.order_date, '%Y-%m')
)
SELECT 'city'                                                            AS dim_level,
       CAST(b.city AS CHAR(100))                                                AS dim_value,
       NULL                                                              AS dim_value2,
...
GROUP BY b.city
UNION ALL
SELECT 'category', NULL, CAST(b.category_name AS CHAR(100)),
...
UNION ALL
SELECT 'city_category', CAST(b.city AS CHAR(100)), CAST(b.category_name AS CHAR(100)),
       SUM(b.sales_amount), SUM(b.qty), SUM(b.order_cnt),
       ROUND(SUM(b.sales_amount) * 100.0
             / NULLIF(SUM(SUM(b.sales_amount)) OVER (), 0), 2),
       RANK() OVER (ORDER BY SUM(b.sales_amount) DESC)
...
WITH m AS (
    ...
    FROM orders o
    WHERE o.status = 'completed'
      AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1095 DAY)
...
-- ------------------------------------------------------------------------------
WITH m AS (
    SELECT CAST(DATE_FORMAT(o.order_date, '%Y-%m-01') AS DATE)                                     AS ym,
           SUM(o.pay_amount)                                             AS gmv,
           COUNT(DISTINCT o.customer_id)                                 AS cust_cnt
    FROM orders o
    WHERE o.status = 'completed'
...
SELECT DATE_FORMAT(w.ym, '%Y-%m')                                              AS ym,
...
WITH act AS (
    ...
    WHERE o.status = 'completed'
      AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY o.store_id, DATE_FORMAT(o.order_date, '%Y-%m')
),
cmp AS (
    SELECT t.store_id,
           DATE_FORMAT(t.target_month, '%Y-%m')                                AS ym,
           SUM(t.target_amt)                                             AS target_amt
    FROM store_targets t
    WHERE t.target_month >= DATE_SUB(CURRENT_DATE, INTERVAL 365 DAY)
    GROUP BY t.store_id, DATE_FORMAT(t.target_month, '%Y-%m')
)
...
WITH kpi AS (
    ...
    FROM orders o
    WHERE o.status = 'completed' AND o.order_date >= DATE_SUB(CURRENT_DATE, INTERVAL 30 DAY)
    UNION ALL
    ...
    FROM payroll p
    WHERE p.pay_month = DATE_FORMAT(CURRENT_DATE, '%Y-%m')
    UNION ALL
