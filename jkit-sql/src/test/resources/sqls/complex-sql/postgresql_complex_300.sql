...
WITH pm AS (
    ...
    JOIN orders o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
      AND o.order_date >= (CURRENT_DATE - INTERVAL '120 days')
...
FROM rk cur
LEFT JOIN rk prev ON cur.product_id = prev.product_id AND prev.ym = TO_CHAR((DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 months'), 'YYYY-MM')
JOIN products p ON cur.product_id = p.product_id
WHERE cur.ym = TO_CHAR(DATE_TRUNC('month', CURRENT_DATE), 'YYYY-MM')
...
rf AS (
    ...
    JOIN order_items oi ON o.order_id = oi.order_id
    JOIN products p ON oi.product_id = p.product_id
    WHERE r.refund_date >= (CURRENT_DATE - INTERVAL '180 days')
    GROUP BY p.category_id
)
...
GROUP BY EXTRACT(HOUR FROM o.order_date),
         CASE WHEN EXTRACT(DOW FROM o.order_date) IN (0, 6) THEN 'weekend' ELSE 'weekday' END
...
WITH dur AS (
    SELECT o.order_id, s.province, s.carrier,
           (s.ship_date::date - o.order_date::date) AS deliver_days
    FROM orders o
...
WITH RECURSIVE date_seq AS (
    ...
    UNION ALL
    SELECT CAST((dt + INTERVAL '1 days') AS DATE) FROM date_seq WHERE dt < DATE '2024-03-31'
),
daily AS (
...
SELECT CASE WHEN seq = 1 AND next_order_date IS NOT NULL THEN 'has_second' ELSE 'one_time' END AS cust_kind,
       COUNT(*) AS cnt,
       ROUND(AVG((next_order_date::date - order_date::date)), 2) AS avg_gap_days,
       MIN((next_order_date::date - order_date::date)) AS min_gap_days,
       MAX((next_order_date::date - order_date::date)) AS max_gap_days
FROM ranked
WHERE seq = 1
...
SELECT p.product_id, p.product_name, c.category_name, p.launch_date,
       ...
       (CURRENT_DATE::date - COALESCE(ls.last_sold_date, p.launch_date)::date) AS idle_days,
       CASE WHEN ls.product_id IS NULL THEN 'never_sold'
            WHEN (CURRENT_DATE::date - ls.last_sold_date::date) > 90 THEN 'long_idle'
            ELSE 'active' END AS sale_status
FROM products p
LEFT JOIN last_sale ls ON p.product_id = ls.product_id
LEFT JOIN categories c ON p.category_id = c.category_id
WHERE p.status = 'on_sale'
  AND (ls.product_id IS NULL OR (CURRENT_DATE::date - ls.last_sold_date::date) > 90)
...
WITH cart AS (
    ...
    FROM carts
    WHERE add_time >= (CURRENT_DATE - INTERVAL '30 days')
),
agg AS (
    SELECT p.category_id,
           COUNT(*)                                          AS add_cnt,
...
WITH rfm AS (
    SELECT o.customer_id,
           (CURRENT_DATE::date - MAX(o.order_date)::date)  AS recency,
...
WITH cohort AS (
    SELECT c.customer_id,
           ...
           DATE_TRUNC('month', c.register_date)    AS cohort_md
    FROM customers c
),
act AS (
    SELECT o.customer_id,
           TO_CHAR(o.order_date, 'YYYY-MM') AS act_ym,
           DATE_TRUNC('month', o.order_date)    AS act_md
    FROM orders o
    WHERE o.status = 'completed'
    GROUP BY o.customer_id, TO_CHAR(o.order_date, 'YYYY-MM'), DATE_TRUNC('month', o.order_date)
),
joined AS (
...
SELECT j.cohort_ym,
       ((EXTRACT(YEAR FROM j.act_md) - EXTRACT(YEAR FROM j.cohort_md)) * 12 + (EXTRACT(MONTH FROM j.act_md) - EXTRACT(MONTH FROM j.cohort_md))) AS month_diff,
...
hist AS (
    SELECT customer_id,
           COUNT(CASE WHEN order_date >= (CURRENT_DATE - INTERVAL '90 days')  THEN 1 END) AS cnt_90d,
           COUNT(CASE WHEN order_date >= (CURRENT_DATE - INTERVAL '180 days')
                       AND order_date <  (CURRENT_DATE - INTERVAL '90 days')  THEN 1 END) AS cnt_90_180d
    FROM orders
    WHERE status = 'completed'
    GROUP BY customer_id
)
SELECT ca.customer_id, c.customer_name, c.level,
       ca.last_order_date,
       (CURRENT_DATE::date - ca.last_order_date::date) AS idle_days,
       ca.order_cnt, ROUND(ca.total_amt, 2) AS total_amt,
       h.cnt_90d, h.cnt_90_180d,
       CASE WHEN (CURRENT_DATE::date - ca.last_order_date::date) > 180 THEN 'churned'
            WHEN (CURRENT_DATE::date - ca.last_order_date::date) > 90  THEN 'high_risk'
...
WHERE (CURRENT_DATE::date - ca.last_order_date::date) > 60
...
with_span AS (
    SELECT customer_id, order_cnt, total_amt,
           ((EXTRACT(YEAR FROM last_order) - EXTRACT(YEAR FROM first_order)) * 12 + (EXTRACT(MONTH FROM last_order) - EXTRACT(MONTH FROM first_order))) AS life_months,
           (CURRENT_DATE::date - first_order::date)    AS days_since_first
    FROM cust
)
SELECT customer_id, order_cnt, ROUND(total_amt, 2) AS ltv,
       life_months,
       ROUND(total_amt / NULLIF(order_cnt, 0), 2)                     AS avg_order_value,
       ROUND(order_cnt / NULLIF(EXTRACT(YEAR FROM CURRENT_DATE) - 2020 + 1, 0), 2) AS annual_frequency,
...
sleeping AS (
    ...
    FROM act
    WHERE (CURRENT_DATE::date - last_dt::date) BETWEEN 90 AND 365
),
reactivated AS (
...
SELECT CASE WHEN r.customer_id IS NOT NULL THEN 'reactivated' ELSE 'still_sleeping' END AS wake_status,
       COUNT(*)                          AS cust_cnt,
       ROUND(AVG((COALESCE(r.wake_date, CURRENT_DATE)::date - s.last_dt::date)), 1) AS avg_sleep_days,
...
SELECT c.customer_id, c.customer_name, c.birth_date, c.city,
       ...
       ROUND((CURRENT_DATE::date - c.register_date::date) / 365.0, 1) AS reg_years,
...
SELECT o.order_id, o.customer_id,
       o.order_date,
       l.last_node_time,
       (EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - l.last_node_time)) / 3600) AS stall_hours,
       l.last_node,
       CASE WHEN (EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - l.last_node_time)) / 3600) > 72 THEN 'severely_stalled'
            WHEN (EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - l.last_node_time)) / 3600) > 48 THEN 'stalled'
            ELSE 'in_transit' END AS logistic_status
FROM orders o
JOIN latest l ON o.order_id = l.order_id
WHERE o.status IN ('shipped', 'delivering')
  AND (EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - l.last_node_time)) / 3600) > 24
...
WITH actual AS (
    ...
    LEFT JOIN orders o ON s.store_id = o.store_id
         ...
         AND TO_CHAR(o.order_date, 'YYYY-MM') = TO_CHAR(DATE_TRUNC('month', CURRENT_DATE), 'YYYY-MM')
    GROUP BY s.store_id, s.store_name, s.region
),
target AS (
    SELECT store_id, SUM(budget) AS target_amt
    FROM store_targets
    WHERE TO_CHAR(target_month, 'YYYY-MM') = TO_CHAR(DATE_TRUNC('month', CURRENT_DATE), 'YYYY-MM')
    GROUP BY store_id
)
...
WITH daily AS (
    ...
    FROM orders
    WHERE status = 'completed'
      AND order_date >= (CURRENT_DATE - INTERVAL '120 days')
    GROUP BY CAST(order_date AS DATE)
),
ma AS (
    SELECT dt, gmv,
...
WITH reg AS (
    ...
    FROM customers
    WHERE register_date >= (CURRENT_DATE - INTERVAL '180 days')
),
first_order AS (
    SELECT customer_id, MIN(order_date) AS first_order_date
    FROM orders
...
SELECT TO_CHAR(r.register_date, 'YYYY-MM') AS reg_ym,
       ...
       ROUND(AVG((fo.first_order_date::date - r.register_date::date)), 2) AS avg_conv_days,
       SUM(CASE WHEN (fo.first_order_date::date - r.register_date::date) <= 1 THEN 1 ELSE 0 END) AS within_1d,
       SUM(CASE WHEN (fo.first_order_date::date - r.register_date::date) <= 7 THEN 1 ELSE 0 END) AS within_7d
FROM reg r
...
SELECT session_id, customer_id, total_steps,
       STRING_AGG(CONCAT(CONCAT(CAST(step_no AS VARCHAR(10)), ':'), page_url), '>' ORDER BY step_no) AS path,
...
SELECT ev.product_id, p.product_name,
       ...
       ROUND(AVG((EXTRACT(EPOCH FROM (next_time - event_time)) / 60)), 2) AS avg_stay_minutes,
       MAX((EXTRACT(EPOCH FROM (next_time - event_time)) / 60))            AS max_stay_minutes,
       SUM(CASE WHEN next_type = 'add_cart' THEN 1 ELSE 0 END) AS led_to_cart
FROM ev
JOIN products p ON ev.product_id = p.product_id
WHERE next_time IS NOT NULL
  AND (EXTRACT(EPOCH FROM (next_time - event_time)) / 60) BETWEEN 0 AND 60
...
flag AS (
    SELECT customer_id, event_time, event_type,
           CASE WHEN prev_time IS NULL
                     OR (EXTRACT(EPOCH FROM (event_time - prev_time)) / 60) > 30 THEN 1 ELSE 0 END AS is_new
    FROM ev
),
sess AS (
    SELECT customer_id, event_time, event_type,
...
WITH nc AS (
    ...
    FROM customers
    WHERE register_date >= (CURRENT_DATE - INTERVAL '60 days')
),
beh AS (
    ...
    LEFT JOIN user_events e ON n.customer_id = e.customer_id
         AND e.event_time BETWEEN n.register_date AND (n.register_date + INTERVAL '7 days')
...
SELECT p.product_name,
       ...
       ROUND(AVG((rep.last_dt::date - rep.first_dt::date)), 1)  AS avg_cycle_days,
...
WITH cm AS (
    SELECT p.category_id,
           SUM(CASE WHEN EXTRACT(YEAR FROM o.order_date) = EXTRACT(YEAR FROM CURRENT_DATE)   THEN oi.amount ELSE 0 END) AS amt_cur,
           SUM(CASE WHEN EXTRACT(YEAR FROM o.order_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN oi.amount ELSE 0 END) AS amt_prev
    FROM order_items oi
    JOIN products p ON oi.product_id = p.product_id
    JOIN orders   o ON oi.order_id = o.order_id
    WHERE o.status = 'completed'
...
piv AS (
    SELECT product_id,
           SUM(CASE WHEN ym = TO_CHAR((DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '2 months'), 'YYYY-MM') THEN qty ELSE 0 END) AS q1,
           SUM(CASE WHEN ym = TO_CHAR((DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 months'), 'YYYY-MM') THEN qty ELSE 0 END) AS q2,
           SUM(CASE WHEN ym = TO_CHAR(DATE_TRUNC('month', CURRENT_DATE), 'YYYY-MM') THEN qty ELSE 0 END) AS q3
...
promo_days AS (
    SELECT DISTINCT CAST((d.dt + INTERVAL '0 days') AS DATE) AS dt
    FROM daily d
...
retention AS (
    ...
    LEFT JOIN orders o ON f.customer_id = o.customer_id
         AND o.order_date > (CURRENT_DATE - INTERVAL '0 days')
    WHERE f.rn = 1
...
FROM shipments s
WHERE s.ship_date >= (CURRENT_DATE - INTERVAL '180 days')
GROUP BY s.carrier
...
SELECT reason,
       ...
       ROUND(AVG((refund_date::date - order_date::date)), 1)  AS avg_refund_lag_days,
       MAX((refund_date::date - order_date::date))            AS max_lag_days,
...
FROM payments p
WHERE p.pay_date >= (CURRENT_DATE - INTERVAL '90 days')
...
WITH dp AS (
    ...
    FROM payments
    WHERE pay_date >= (CURRENT_DATE - INTERVAL '60 days')
...
SELECT o.order_id, o.customer_id, ROUND(o.pay_amount, 2) AS pay_amount, o.order_date,
       ...
       (o.order_date::date - h.first_order::date)   AS days_since_first,
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN EXTRACT(DOW FROM o.order_date) IN (0, 6) THEN 'weekend' ELSE 'weekday' END AS day_type,
...
WITH pm AS (
    SELECT s.store_id, s.store_name, s.region,
           SUM(CASE WHEN o.order_date >= (CURRENT_DATE - INTERVAL '90 days')  THEN o.pay_amount ELSE 0 END) AS gmv_90d,
           SUM(CASE WHEN o.order_date >= (CURRENT_DATE - INTERVAL '180 days')
                     AND o.order_date <  (CURRENT_DATE - INTERVAL '90 days')  THEN o.pay_amount ELSE 0 END) AS gmv_prev_90d,
           COUNT(CASE WHEN o.order_date >= (CURRENT_DATE - INTERVAL '90 days') THEN 1 END) AS orders_90d,
           COUNT(CASE WHEN o.order_date >= (CURRENT_DATE - INTERVAL '180 days')
                       AND o.order_date <  (CURRENT_DATE - INTERVAL '90 days') THEN 1 END) AS orders_prev_90d
    FROM stores s
    LEFT JOIN orders o ON s.store_id = o.store_id AND o.status = 'completed'
    GROUP BY s.store_id, s.store_name, s.region
)
...
WITH amt AS (
    SELECT customer_id,
           SUM(CASE WHEN order_date >= (CURRENT_DATE - INTERVAL '90 days')  THEN pay_amount ELSE 0 END) AS amt_recent,
           SUM(CASE WHEN order_date >= (CURRENT_DATE - INTERVAL '180 days')
                     AND order_date <  (CURRENT_DATE - INTERVAL '90 days')  THEN pay_amount ELSE 0 END) AS amt_prev
    FROM orders
    WHERE status = 'completed'
    GROUP BY customer_id
),
...
SELECT CASE WHEN pd.view_times = 1 THEN 'view_1'
       ...
       ROUND(AVG((EXTRACT(EPOCH FROM (pd.last_view - pd.first_view)) / 60)), 2)     AS avg_dwell_minutes
FROM pd
...
WITH base AS (
    SELECT c.customer_id, c.customer_name, c.city, c.level, c.gender,
           (CURRENT_DATE::date - c.birth_date::date) / 365 AS age
    FROM customers c
),
beh AS (
    SELECT o.customer_id,
...
SELECT b.customer_id, b.customer_name, b.city,
       ...
       (CURRENT_DATE::date - beh.last_order::date) AS idle_days,
...
stage AS (
    SELECT customer_id, order_cnt, total_amt,
           (CURRENT_DATE::date - first_dt::date) AS days_since_first,
           (CURRENT_DATE::date - last_dt::date)  AS days_since_last
    FROM cust
)
...
perf AS (
    ...
    LEFT JOIN orders o ON oi.order_id = o.order_id AND o.status = 'completed'
         ...
         AND o.order_date <= (n.launch_date + INTERVAL '90 days')
...
SELECT channel,
       COUNT(*)                                                       AS order_cnt,
       ROUND(AVG((first_pay_date::date - order_date::date)), 2)          AS avg_pay_lag_days,
       MAX((first_pay_date::date - order_date::date))                    AS max_pay_lag_days,
       SUM(CASE WHEN (first_pay_date::date - order_date::date) <= 0 THEN 1 ELSE 0 END) AS same_day_paid,
       ROUND(SUM(CASE WHEN (first_pay_date::date - order_date::date) <= 0 THEN 1 ELSE 0 END)
...
WITH reg AS (
    ...
    FROM customers
    WHERE register_date >= (CURRENT_DATE - INTERVAL '90 days')
...
WITH acct_stat AS (
    SELECT account_id,
           ...
           COUNT(*)    AS txn_cnt
    FROM transactions
    WHERE txn_date >= (CURRENT_DATE - INTERVAL '180 days')
    GROUP BY account_id
    HAVING COUNT(*) >= 10
),
tagged AS (
    ...
    WHERE t.txn_date >= (CURRENT_DATE - INTERVAL '7 days')
)
...
WITH cp AS (
    ...
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= (CURRENT_DATE - INTERVAL '180 days')
...
SELECT cust_id, account_id, txn_id,
       ...
       txn_date, next_date,
       (EXTRACT(EPOCH FROM (next_date - txn_date)) / 3600) AS gap_hours,
       ...
       CASE WHEN txn_type = 'transfer_out' AND next_type = 'transfer_in'
                 AND (EXTRACT(EPOCH FROM (next_date - txn_date)) / 3600) <= 24
...
SELECT a.account_type,
       COUNT(*)                                                   AS account_cnt,
       SUM(CASE WHEN (CURRENT_DATE::date - lt.last_dt::date) <= 30  THEN 1 ELSE 0 END) AS active_30d,
       SUM(CASE WHEN (CURRENT_DATE::date - lt.last_dt::date) BETWEEN 31 AND 90 THEN 1 ELSE 0 END) AS low_active,
       SUM(CASE WHEN (CURRENT_DATE::date - lt.last_dt::date) BETWEEN 91 AND 365 THEN 1 ELSE 0 END) AS dormant,
       SUM(CASE WHEN lt.account_id IS NULL
                     OR (CURRENT_DATE::date - lt.last_dt::date) > 365 THEN 1 ELSE 0 END) AS inactive,
       ROUND(AVG((CURRENT_DATE::date - lt.last_dt::date)), 1) AS avg_idle_days,
       ROUND(SUM(CASE WHEN (CURRENT_DATE::date - lt.last_dt::date) <= 30 THEN 1 ELSE 0 END)
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN ((EXTRACT(YEAR FROM maturity_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM maturity_date) - EXTRACT(MONTH FROM start_date))) <= 3  THEN 'demand_like'
            WHEN ((EXTRACT(YEAR FROM maturity_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM maturity_date) - EXTRACT(MONTH FROM start_date))) <= 12 THEN 'short_term'
            WHEN ((EXTRACT(YEAR FROM maturity_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM maturity_date) - EXTRACT(MONTH FROM start_date))) <= 36 THEN 'medium_term'
...
FROM deposits
WHERE status = 'active'
GROUP BY CASE WHEN ((EXTRACT(YEAR FROM maturity_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM maturity_date) - EXTRACT(MONTH FROM start_date))) <= 3  THEN 'demand_like'
              WHEN ((EXTRACT(YEAR FROM maturity_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM maturity_date) - EXTRACT(MONTH FROM start_date))) <= 12 THEN 'short_term'
              WHEN ((EXTRACT(YEAR FROM maturity_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM maturity_date) - EXTRACT(MONTH FROM start_date))) <= 36 THEN 'medium_term'
              ELSE 'long_term' END
ORDER BY total_amt DESC;
...
WITH mat AS (
    SELECT d.deposit_id, d.cust_id, d.amount, d.rate, d.maturity_date,
           (d.maturity_date::date - CURRENT_DATE::date) AS days_to_maturity
    FROM deposits d
...
WITH t AS (
    SELECT t.txn_id, t.account_id, t.amount, t.txn_date, t.channel,
           a.cust_id,
           EXTRACT(HOUR FROM t.txn_date) AS txn_hour,
           EXTRACT(DOW FROM t.txn_date)  AS txn_dow
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
...
pairs AS (
    ...
    WHERE a1.cust_id < a2.cust_id
      AND t.txn_date >= (CURRENT_DATE - INTERVAL '180 days')
...
WITH suspicious AS (
    SELECT t.account_id, a.cust_id, t.txn_id, t.amount, t.txn_date, t.txn_type, t.city,
           ...
           CASE WHEN EXTRACT(HOUR FROM t.txn_date) >= 20 OR EXTRACT(HOUR FROM t.txn_date) < 8 THEN 2 ELSE 0 END AS time_score
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= (CURRENT_DATE - INTERVAL '30 days')
),
...
SELECT s.cust_id, c.cust_name, c.risk_level,
       ...
       STRING_AGG(CAST(txn_id AS VARCHAR(20)), ',' ORDER BY txn_id) AS txn_list,
...
SELECT a.account_type,
       ...
       ROUND(AVG((ft.first_dt::date - a.open_date::date)), 2)          AS avg_activation_days,
       SUM(CASE WHEN (ft.first_dt::date - a.open_date::date) <= 7 THEN 1 ELSE 0 END) AS within_7d,
       SUM(CASE WHEN (ft.first_dt::date - a.open_date::date) <= 30 THEN 1 ELSE 0 END) AS within_30d
FROM accounts a
LEFT JOIN first_txn ft ON a.account_id = ft.account_id
WHERE a.open_date >= (CURRENT_DATE - INTERVAL '365 days')
...
WITH act AS (
    ...
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= (CURRENT_DATE - INTERVAL '365 days')
),
seq AS (
    SELECT cust_id, ym,
...
tenure AS (
    SELECT cust_id,
           (CURRENT_DATE::date - MIN(open_date)::date) / 365.0 AS tenure_years
...
WITH act AS (
    SELECT a.cust_id,
           SUM(CASE WHEN t.txn_date >= (CURRENT_DATE - INTERVAL '90 days')  THEN 1 ELSE 0 END) AS cnt_90d,
           SUM(CASE WHEN t.txn_date >= (CURRENT_DATE - INTERVAL '180 days')
                     AND t.txn_date <  (CURRENT_DATE - INTERVAL '90 days')  THEN 1 ELSE 0 END) AS cnt_prev_90d,
...
SELECT c.cust_id, c.cust_name, c.cust_type,
       ...
       (CURRENT_DATE::date - a.last_txn::date) AS idle_days,
       ROUND(b.cur_balance, 2)           AS cur_balance,
       ROUND((a.cnt_90d - a.cnt_prev_90d) / NULLIF(a.cnt_prev_90d, 0) * 100, 2) AS activity_change_pct,
       CASE WHEN (CURRENT_DATE::date - a.last_txn::date) > 180 THEN 'churned'
            WHEN (CURRENT_DATE::date - a.last_txn::date) > 90
...
WHERE (CURRENT_DATE::date - a.last_txn::date) > 60
...
WITH r AS (
    ...
    FROM fin_reports
    WHERE period >= TO_CHAR((CURRENT_DATE - INTERVAL '365 days'), 'YYYY-MM')
    GROUP BY branch_id
)
SELECT b.branch_name, b.city, b.region,
...
WITH t AS (
    ...
    FROM card_txns ct
    WHERE ct.txn_date >= (CURRENT_DATE - INTERVAL '90 days')
)
SELECT card_id,
       COUNT(*)                                                    AS txn_cnt,
...
WITH t AS (
    SELECT a.cust_id, t.amount, t.txn_date, t.channel, t.txn_type,
           EXTRACT(HOUR FROM t.txn_date) AS hr
    FROM transactions t
    JOIN accounts a ON t.account_id = a.account_id
    WHERE t.txn_date >= (CURRENT_DATE - INTERVAL '180 days')
)
...
WITH e1 AS (
    ...
    JOIN accounts a2 ON t.counterparty = CAST(a2.account_id AS VARCHAR(20))
    WHERE t.txn_type = 'transfer_out'
      AND t.txn_date >= (CURRENT_DATE - INTERVAL '60 days')
    GROUP BY t.account_id, a2.account_id
),
e2 AS (
    ...
    WHERE t2.txn_type = 'transfer_out'
      AND t2.txn_date >= (CURRENT_DATE - INTERVAL '60 days')
)
...
SELECT CAST(t.txn_date AS DATE) AS dt,
       ...
       SUM(CASE WHEN EXTRACT(HOUR FROM t.txn_date) >= 20
                  OR EXTRACT(HOUR FROM t.txn_date) < 8 THEN 1 ELSE 0 END)    AS offhour_cnt,
...
SELECT CASE WHEN term_months <= 6   THEN 'short_0_6m'
       ...
       SUM(CASE WHEN ((EXTRACT(YEAR FROM end_date) - EXTRACT(YEAR FROM CURRENT_DATE)) * 12 + (EXTRACT(MONTH FROM end_date) - EXTRACT(MONTH FROM CURRENT_DATE))) <= 12 THEN 1 ELSE 0 END) AS maturing_12m,
       ROUND(SUM(CASE WHEN ((EXTRACT(YEAR FROM end_date) - EXTRACT(YEAR FROM CURRENT_DATE)) * 12 + (EXTRACT(MONTH FROM end_date) - EXTRACT(MONTH FROM CURRENT_DATE))) <= 12 THEN loan_amount ELSE 0 END), 2) AS repricing_amt,
       ROUND(SUM(CASE WHEN ((EXTRACT(YEAR FROM end_date) - EXTRACT(YEAR FROM CURRENT_DATE)) * 12 + (EXTRACT(MONTH FROM end_date) - EXTRACT(MONTH FROM CURRENT_DATE))) <= 12 THEN loan_amount ELSE 0 END)
...
WITH rb AS (
    SELECT l.loan_id, l.cust_id,
           ...
           AVG((r.repay_date::date - r.due_date::date))                           AS avg_pay_gap,
           MAX(r.overdue_days)                                                 AS max_od,
...
SELECT product_type,
       ...
       ROUND(AVG(((EXTRACT(YEAR FROM last_pay) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM last_pay) - EXTRACT(MONTH FROM start_date)))), 1)               AS avg_actual_months
FROM rp
GROUP BY product_type
...
WITH m AS (
    SELECT TO_CHAR(start_date, 'YYYY-MM') AS ym, product_type,
           ...
           SUM(loan_amount)  AS loan_amt
    FROM loans
    WHERE start_date >= (CURRENT_DATE - INTERVAL '730 days')
...
SELECT od_stage,
       ...
       ROUND(AVG((repay_date::date - due_date::date)), 1)            AS avg_recovery_days,
...
WITH orig AS (
    SELECT loan_id, cust_id, product_type, loan_amount, term_months,
           start_date, end_date,
           ((EXTRACT(YEAR FROM end_date) - EXTRACT(YEAR FROM start_date)) * 12 + (EXTRACT(MONTH FROM end_date) - EXTRACT(MONTH FROM start_date))) AS actual_months
    FROM loans
)
SELECT product_type,
       COUNT(*)                                                            AS loan_cnt,
...
FROM loans l
WHERE l.start_date >= (CURRENT_DATE - INTERVAL '1095 days')
...
WITH n AS (
    ...
    FROM fund_nav
    WHERE nav_date >= (CURRENT_DATE - INTERVAL '730 days')
),
dd AS (
...
SELECT f.fund_name, f.fund_type, f.risk_level,
       ...
       ROUND(CAST(PERCENT_RANK() OVER (PARTITION BY f.fund_type ORDER BY c.nav_change) * 100 AS NUMERIC), 1) AS pct_in_type,
...
SELECT c.cust_id, c.cust_name,
       ...
       ROUND(AVG((CURRENT_DATE::date - h.purchase_date::date)), 0)              AS avg_holding_days,
       MAX((CURRENT_DATE::date - h.purchase_date::date))                        AS max_holding_days,
...
perf AS (
    ...
    FROM fund_nav n
    WHERE n.nav_date >= (CURRENT_DATE - INTERVAL '365 days')
    GROUP BY n.fund_id
)
SELECT m.manager_id,
       COUNT(*)                                 AS fund_cnt,
...
SELECT f.fund_name,
       ...
       ROUND(AVG((CURRENT_DATE::date - purchase_date::date)), 0)               AS avg_holding_days
FROM pos
...
WITH dep AS (
    SELECT d.deposit_id, d.cust_id, d.amount, d.rate, d.start_date, d.maturity_date,
           (d.maturity_date::date - d.start_date::date)  AS days_held,
           ((EXTRACT(YEAR FROM d.maturity_date) - EXTRACT(YEAR FROM d.start_date)) * 12 + (EXTRACT(MONTH FROM d.maturity_date) - EXTRACT(MONTH FROM d.start_date))) AS months_held
    FROM deposits d
)
...
WITH r AS (
    ...
    FROM fx_rates
    WHERE rate_date >= (CURRENT_DATE - INTERVAL '365 days')
)
SELECT currency,
       COUNT(*)                                          AS quote_days,
...
WITH br AS (
    ...
    FROM fin_reports r
    WHERE r.period >= TO_CHAR((CURRENT_DATE - INTERVAL '365 days'), 'YYYY-MM')
...
WITH rev AS (
    SELECT r.branch_id,
           SUM(r.revenue) AS total_revenue,
           SUM(CASE WHEN r.period >= TO_CHAR(DATE_TRUNC('month', CURRENT_DATE), 'YYYY-MM')
...
WITH e AS (
    SELECT emp_id, dept_id, job_level, status,
           (COALESCE(leave_date, CURRENT_DATE)::date - hire_date::date) / 365.0 AS tenure_years
    FROM employees
)
...
-- ------------------------------------------------------------------------------
SELECT CASE WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 26 THEN 'gen_z_below26'
            WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 36 THEN 'millennial_26_35'
            WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 46 THEN 'gen_x_36_45'
            WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 56 THEN 'boomer_46_55'
            ELSE 'near_retirement_56plus' END AS age_band,
       COUNT(*)                                                   AS emp_cnt,
       ROUND(AVG((CURRENT_DATE::date - birth_date::date) / 365.0), 1)   AS avg_age,
       SUM(CASE WHEN gender = 'M' THEN 1 ELSE 0 END)              AS male_cnt,
       SUM(CASE WHEN gender = 'F' THEN 1 ELSE 0 END)              AS female_cnt,
       ROUND(AVG((CURRENT_DATE::date - hire_date::date) / 365.0), 2)    AS avg_tenure,
       ROUND(AVG(salary), 2)                                      AS avg_salary,
       ROUND(COUNT(*) / NULLIF(SUM(COUNT(*)) OVER (), 0) * 100, 2) AS emp_share_pct
FROM employees
WHERE status = 'active'
GROUP BY CASE WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 26 THEN 'gen_z_below26'
              WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 36 THEN 'millennial_26_35'
              WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 46 THEN 'gen_x_36_45'
              WHEN (CURRENT_DATE::date - birth_date::date) / 365 < 56 THEN 'boomer_46_55'
...
WITH hires AS (
    ...
    FROM employees
    WHERE hire_date >= (CURRENT_DATE - INTERVAL '730 days')
...
leaves AS (
    ...
    WHERE leave_date IS NOT NULL
      AND leave_date >= (CURRENT_DATE - INTERVAL '730 days')
...
WITH base AS (
    SELECT TO_CHAR(hire_date, 'YYYY-MM') AS ym,
           ...
           AVG((leave_date::date - hire_date::date)) AS avg_days_to_leave
    FROM employees
    GROUP BY TO_CHAR(hire_date, 'YYYY-MM')
)
...
WITH nh AS (
    SELECT emp_id, hire_date, dept_id,
           leave_date,
           (leave_date::date - hire_date::date) AS days_to_leave
    FROM employees
    WHERE hire_date >= (CURRENT_DATE - INTERVAL '730 days')
)
...
recent_left AS (
    ...
    FROM employees
    WHERE leave_date >= (CURRENT_DATE - INTERVAL '180 days')
    GROUP BY dept_id
)
SELECT d.dept_name,
       ds.active_cnt,
...
SELECT d.dept_name, d.cost_center,
       ...
       SUM(CASE WHEN e.leave_date >= (CURRENT_DATE - INTERVAL '365 days') THEN 1 ELSE 0 END) AS left_1y,
       ROUND(SUM(CASE WHEN e.leave_date >= (CURRENT_DATE - INTERVAL '365 days') THEN 1 ELSE 0 END)
             / NULLIF(SUM(CASE WHEN e.status = 'active' THEN 1 ELSE 0 END), 0) * 100, 2) AS annual_turnover_pct,
       ROUND(AVG((CURRENT_DATE::date - e.hire_date::date) / 365.0), 2)        AS avg_tenure,
       ROUND(AVG(e.salary), 2)                                          AS avg_salary,
       RANK() OVER (ORDER BY SUM(CASE WHEN e.leave_date >= (CURRENT_DATE - INTERVAL '365 days') THEN 1 ELSE 0 END)
...
FROM emp_changes c
WHERE c.change_date >= (CURRENT_DATE - INTERVAL '730 days')
...
tenure AS (
    SELECT emp_id, (CURRENT_DATE::date - hire_date::date) / 365.0 AS tenure_years
    FROM employees
)
...
FROM payroll p
WHERE p.pay_month >= TO_CHAR((CURRENT_DATE - INTERVAL '730 days'), 'YYYY-MM')
...
WHERE p.pay_month >= TO_CHAR((CURRENT_DATE - INTERVAL '365 days'), 'YYYY-MM')
GROUP BY d.dept_name, d.cost_center
...
WITH lv AS (
    SELECT e.emp_id, e.job_level, e.position, e.gender, e.salary, e.dept_id,
           (CURRENT_DATE::date - e.hire_date::date) / 365.0 AS tenure_years,
...
SELECT emp_id, emp_name, job_level,
       ...
       ROUND(CAST(pct_in_dept * 100 AS NUMERIC), 1)              AS percentile_in_dept,
...
WITH ot AS (
    ...
    WHERE a.att_date >= (CURRENT_DATE - INTERVAL '365 days')
...
WITH a AS (
    ...
    FROM attendance
    WHERE att_date >= (CURRENT_DATE - INTERVAL '90 days')
    GROUP BY emp_id
)
...
WITH late AS (
    ...
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= (CURRENT_DATE - INTERVAL '180 days')
...
grp AS (
    SELECT emp_id, att_date, status, is_abnormal,
           (att_date::date - DATE '2000-01-01'::date)
...
WHERE l.start_date >= (CURRENT_DATE - INTERVAL '365 days')
...
SELECT e.emp_id, e.emp_name, d.dept_name,
       ...
       (DATE '2026-12-31'::date - CURRENT_DATE::date)                 AS days_to_expire,
...
att AS (
    ...
    FROM attendance
    WHERE att_date >= (CURRENT_DATE - INTERVAL '180 days')
    GROUP BY emp_id
),
tr AS (
...
SELECT e.emp_id, e.emp_name, e.position, e.job_level, d.dept_name,
       ...
       ROUND((CURRENT_DATE::date - e.hire_date::date) / 365.0, 2) AS tenure_years,
       e.salary,
...
WITH team AS (
    SELECT e.manager_id,
           ...
           AVG((CURRENT_DATE::date - e.hire_date::date) / 365.0)             AS avg_team_tenure,
...
WITH hc AS (
    SELECT COUNT(*) AS total_emp,
           ...
           SUM(CASE WHEN leave_date >= (CURRENT_DATE - INTERVAL '365 days') THEN 1 ELSE 0 END) AS left_1y,
           ROUND(AVG(salary), 2) AS avg_salary,
           ROUND(AVG((CURRENT_DATE::date - hire_date::date) / 365.0), 2) AS avg_tenure
    FROM employees
),
cost AS (
    ...
    FROM payroll
    WHERE pay_month >= TO_CHAR((CURRENT_DATE - INTERVAL '365 days'), 'YYYY-MM')
),
perf AS (
...
WITH req AS (
    ...
    FROM recruitment r
    WHERE r.open_date >= (CURRENT_DATE - INTERVAL '365 days')
    GROUP BY r.dept_id
)
SELECT d.dept_name,
...
SELECT TO_CHAR(r.open_date, 'YYYY-MM') AS open_ym,
       ...
       ROUND(AVG((COALESCE(r.close_date, CURRENT_DATE)::date - r.open_date::date)), 1) AS avg_days_open,
       ROUND(MAX((COALESCE(r.close_date, CURRENT_DATE)::date - r.open_date::date)), 1) AS max_days_open,
       SUM(CASE WHEN r.status = 'open'
                 AND (CURRENT_DATE::date - r.open_date::date) > 60 THEN 1 ELSE 0 END) AS backlog_over_60d,
       SUM(r.hired_cnt)                                            AS hired_cnt
FROM recruitment r
WHERE r.open_date >= (CURRENT_DATE - INTERVAL '730 days')
...
WITH nh AS (
    SELECT e.emp_id, e.dept_id, e.hire_date,
           (CURRENT_DATE::date - e.hire_date::date) AS days_since_hire,
           p.avg_score
    FROM employees e
    LEFT JOIN (SELECT emp_id, AVG(score) AS avg_score FROM performance GROUP BY emp_id) p
...
WITH tr AS (
    ...
    WHERE t.train_date >= (CURRENT_DATE - INTERVAL '365 days')
    GROUP BY t.emp_id
),
dept AS (
...
SELECT p.project_name, p.status, d.dept_name,
       ...
       ROUND((COALESCE(p.end_date, CURRENT_DATE)::date - p.start_date::date) / 30.0, 1) AS duration_months,
...
WITH emp_load AS (
    SELECT pa.emp_id,
           ...
           STRING_AGG(CAST(pa.project_id AS VARCHAR(50)), ',' ORDER BY pa.project_id)  AS project_list
    FROM project_assignments pa
    JOIN projects p ON pa.project_id = p.project_id
...
WITH pm AS (
    SELECT p.project_id, p.project_name, p.dept_id, p.status,
           (COALESCE(p.end_date, CURRENT_DATE)::date - p.start_date::date) AS duration_days,
    ...
    FROM projects p
    WHERE p.start_date >= (CURRENT_DATE - INTERVAL '730 days')
)
...
WITH chg AS (
    ...
    FROM emp_changes c
    WHERE c.change_date >= (CURRENT_DATE - INTERVAL '1095 days')
)
SELECT d.dept_name,
...
gap AS (
    SELECT emp_id, promo_seq,
           (change_date::date - prev_promo_date::date) / 365.0 AS years_between,
...
WITH lv AS (
    SELECT e.emp_id, e.dept_id, e.job_level, e.position,
           (COALESCE(e.leave_date, CURRENT_DATE)::date - e.hire_date::date) / 365.0 AS tenure_at_exit,
           c.change_type,
           p.avg_score,
           a.abnormal_rate
    FROM employees e
...
WITH demo AS (
    SELECT e.dept_id,
           ...
           CASE WHEN (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM e.birth_date)) < 26 THEN 'gen_z'
                WHEN (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM e.birth_date)) < 36 THEN 'millennial'
                WHEN (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM e.birth_date)) < 46 THEN 'gen_x'
...
SELECT d.dept_name,
       ...
       ROUND(AVG((EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM a.birth_date))), 1)             AS avg_age,
...
WITH act AS (
    ...
    FROM payroll p
    JOIN employees e ON p.emp_id = e.emp_id
    WHERE p.pay_date >= (CURRENT_DATE - INTERVAL '365 days')
...
WITH y AS (
    ...
    FROM payroll p
    JOIN employees e ON p.emp_id = e.emp_id
    WHERE p.pay_date >= (CURRENT_DATE - INTERVAL '1095 days')
...
WITH emp_val AS (
    ...
    LEFT JOIN (SELECT emp_id, SUM(gross_pay) AS total_gross FROM payroll
               WHERE pay_month >= TO_CHAR((CURRENT_DATE - INTERVAL '1095 days'), 'YYYY-MM')
...
WITH pos AS (
    SELECT e.dept_id, e.position, e.job_level,
           ...
           SUM(CASE WHEN e.leave_date IS NOT NULL
                     AND e.leave_date >= (CURRENT_DATE - INTERVAL '180 days') THEN 1 ELSE 0 END) AS recent_leavers
    FROM employees e
...
WITH risk AS (
    ...
    LEFT JOIN (SELECT emp_id,
               ...
               GROUP BY emp_id) a ON e.emp_id = a.emp_id
    LEFT JOIN (SELECT emp_id, SUM(days) AS leave_days FROM leaves
               WHERE start_date >= (CURRENT_DATE - INTERVAL '365 days') GROUP BY emp_id) lv
...
at AS (
    ...
    FROM attendance a
    JOIN employees e ON a.emp_id = e.emp_id
    WHERE a.att_date >= (CURRENT_DATE - INTERVAL '90 days')
    GROUP BY e.dept_id
),
cost AS (
    ...
    WHERE py.pay_month >= TO_CHAR((CURRENT_DATE - INTERVAL '365 days'), 'YYYY-MM')
    GROUP BY e.dept_id
)
SELECT d.dept_name,
...
SELECT 'customers'                                                       AS table_name,
       ...
       SUM(CASE WHEN (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM c.register_date)) > 50 THEN 1 ELSE 0 END) AS implausible_rows
FROM customers c
UNION ALL
SELECT 'orders',
       COUNT(*),
...
WITH dup_cust AS (
    SELECT c.customer_name, c.city,
           ...
           STRING_AGG(CAST(c.customer_id AS VARCHAR(50)), ',' ORDER BY c.customer_id)             AS id_list
    FROM customers c
...
dup_emp AS (
    SELECT e.emp_name, e.dept_id,
           ...
           STRING_AGG(CAST(e.emp_id AS VARCHAR(50)), ',' ORDER BY e.emp_id)                       AS id_list
    FROM employees e
...
target AS (
    ...
    FROM store_targets
    WHERE TO_CHAR(target_month, 'YYYY-MM') >= TO_CHAR((CURRENT_DATE - INTERVAL '365 days'), 'YYYY-MM')
    GROUP BY store_id
)
...
-- ------------------------------------------------------------------------------
WITH daily AS (
    SELECT DATE_TRUNC('day', o.order_date)                                       AS dt,
           SUM(o.pay_amount)                                             AS gmv,
           COUNT(*)                                                      AS order_cnt
    FROM orders o
...
WITH m AS (
    ...
    FROM orders o
    WHERE o.status = 'completed'
      AND o.order_date >= (CURRENT_DATE - INTERVAL '1095 days')
...
cmp AS (
    ...
    WHERE t.target_month >= (CURRENT_DATE - INTERVAL '365 days')
...
WITH kpi AS (
    ...
    FROM payroll p
    WHERE p.pay_month = TO_CHAR(CURRENT_DATE, 'YYYY-MM')
    UNION ALL
