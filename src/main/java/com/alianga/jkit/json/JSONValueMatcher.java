...
class JSONValueMatcher<T> {
    ...
    static final class PlhvImpl<T> extends JSONValueMatcher<T> {
        ...
        public final T matchValue(CharSource source, char[] buf, int offset, int endToken,
            ...
            if ((ch = buf[i]) != endToken) {
                ...
                result = parseContext.strictMode ? valueMapForChars.getValue(buf, offset, i, hashValue) :
                        valueMapForChars.getValueByHash(hashValue);
            }
            parseContext.endIndex = i;
            return result;
        }

        public final T matchValue(CharSource source, byte[] buf, int offset, int endToken,
                                  JSONParseContext parseContext) {
            int i = offset;
            byte b;
            T result = null;
            if ((b = buf[i]) != endToken) {
                ...
                result = parseContext.strictMode ? valueMapForBytes.getValue(buf, offset, i, hashValue) :
                        valueMapForBytes.getValueByHash(hashValue);
            }
            parseContext.endIndex = i;
            return result;
        }
...
//                }
//                return new JSONOneNodeEqualMatcher(valueMapForChars, key.length(), keyCharValue, keyByteValue,
//                originalMap.get(key));
//            }
