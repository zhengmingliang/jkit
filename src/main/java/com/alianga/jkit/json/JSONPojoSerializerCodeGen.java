...
import com.alianga.jkit.common.idgenerate.IdGenerator;
...
final class JSONPojoSerializerCodeGen {
    static final String IMPORT_CODE_TEXT =
            "import com.alianga.jkit.json.JSONConfig;\n" +
                    "import com.alianga.jkit.json.JSONPojoFieldSerializer;\n" +
                    "import com.alianga.jkit.json.JSONPojoStructure;\n" +
    ...
    static JavaSourceObject generateJavaCodeSource(JSONPojoStructure jsonPojoStructure, boolean printSource,
                                                   boolean runtime) {
        ...
        codeBuilder.append("public final class ").append(genClassName)
                .append(" extends com.alianga.jkit.json.JSONPojoSerializer<").append(canonicalName).append("> {\n\n");
        // fieldset
        ...
        if (!jsonPojoStructure.isForceUseFields()) {
            ...
            fmatOutHeaderBuilder.append(
                    "\t\t\tsuper.serializePojoFormatOut(entity, writer, jsonConfig, indentLevel);\n");
            fmatOutHeaderBuilder.append("\t\t\treturn;\n");
            fmatOutHeaderBuilder.append("\t\t}\n");
        }
        compactHeaderBuilder.append(
                "\t\tboolean isEmptyFlag = !checkWriteClassName(jsonConfig.isWriteClassName(), writer, pojoClass, " +
                        "false, indentLevel, jsonConfig);\n");

        fmatOutHeaderBuilder.append(
                "\t\tboolean isEmptyFlag = !checkWriteClassName(jsonConfig.isWriteClassName(), writer, pojoClass, " +
                        "true, indentLevel, jsonConfig);\n");
        ...
        if (fieldSerializerUseMethods.length > 0) {
            ...
            for (JSONPojoFieldSerializer fieldSerializer : fieldSerializerUseMethods) {
                ...
                if (useUnsafe) {
                    ...
                    longs = isBoolean ? JSONMemoryHandle.getCharLongs(fieldNameTempBuilder.substring(1)) :
                            longsFormatOut;
                ...
                if (accessFlag) {
                    compactBodyBuilder.append("\t\t" + getCanonicalName(returnType) + " " + valueVar + " = entity." +
                            getterInfo.generateCode() + ";\n");
                    fmatOutBodyBuilder.append("\t\t" + getCanonicalName(returnType) + " " + valueVar + " = entity." +
                            getterInfo.generateCode() + ";\n");

                    // generate format code
                    generateSerializeFieldFormatOutCode(ensureNotEmptyFlag, primitive, fieldSerializerName, fieldKey,
                            valueVar, nameEqualUnderlineName, underlineName, useUnsafe, longsFormatOut, intsFormatOut,
                            fieldNameFormatOutTokenLength, returnType, fieldSerializer, runtime, fmatOutBodyBuilder);
                    ...
                    if (primitive) {
                        if (firstFlag) {
                            if (isBoolean) {
                                ...
                                if (nameEqualUnderlineName) {
                                    ...
                                    compactBodyBuilder.append(
                                            "\t\t\t\twriter.write(\"\\\"" + underlineName + "\\\":true\");\n");
                                    compactBodyBuilder.append("\t\t\t}\n");
                                }
                                compactBodyBuilder.append("\t\t} else {\n");
                                if (nameEqualUnderlineName) {
                                    compactBodyBuilder.append(
                                            "\t\t\twriter.write(\"\\\"" + fieldKey + "\\\":false\");\n");
                                } else {
                                    compactBodyBuilder.append("\t\t\tif (unCamelCaseToUnderline) {\n");
                                    compactBodyBuilder.append(
                                            "\t\t\t\twriter.write(\"\\\"" + fieldKey + "\\\":false\");\n");
                                    compactBodyBuilder.append("\t\t\t} else {\n");
                                    compactBodyBuilder.append(
                                            "\t\t\t\twriter.write(\"\\\"" + underlineName + "\\\":false\");\n");
                                    compactBodyBuilder.append("\t\t\t}\n");
                                }
                                compactBodyBuilder.append("\t\t}\n");
                            } else {
                                    ...
                                    if (useUnsafe /*&& (total & 3) != 1*/) {
                                        ...
                                        for (int i = 0; i < arrLength; ++i) {
                                            if (i + 1 < arrLength) {
                                                ...
                                                int i1 = ints[i];
                                                int i2 = ints[i + 1];
                                                long l3 = EnvUtils.BIG_ENDIAN ? ((long) i1) << 32 | i2 :
                                                        ((long) i2) << 32 | i1;
                                                compactBodyBuilder.append(
                                                        "\t\twriteMemory(writer, " + l1 + "L, " + l2 + "L, " + l3 +
                                                                "L, " + len + ");\n");
                                                total -= 8;
                                                ++i;
                                            } else {
                                                ...
                                                compactBodyBuilder.append(
                                                        "\t\twriteMemory(writer, " + longs[i] + "L, " + ints[i] + ", " +
                                                                len + ");\n");
                                                total -= 4;
                                            }
                                        }
                                    ...
                                    if (nameEqualUnderlineName) {
                                        ...
                                        if (useUnsafe /*&& (total & 3) != 1*/) {
                                            ...
                                            for (int i = 0; i < arrLength; ++i) {
                                                if (i + 1 < arrLength) {
                                                    int len = total > 8 ? 8 : total;
                                                    long l1 = longsWithComma[i];
                                                    long l2 = longsWithComma[i + 1];
                                                    int i1 = intsWithComma[i];
                                                    int i2 = intsWithComma[i + 1];
                                                    long l3 = EnvUtils.BIG_ENDIAN ? ((long) i1) << 32 | i2 :
                                                            ((long) i2) << 32 | i1;
                                                    compactBodyBuilder.append(
                                                            "\t\t\twriteMemory(writer, " + l1 + "L, " + l2 + "L, " +
                                                                    l3 + "L, " + len + ");\n");
                                                    total -= 8;
                                                    ++i;
                                                } else {
                                                    int len = total > 4 ? 4 : total;
                                                    compactBodyBuilder.append(
                                                            "\t\t\twriteMemory(writer, " + longsWithComma[i] + "L, " +
                                                                    intsWithComma[i] + ", " + len + ");\n");
                                                    total -= 4;
                                                }
                                            }
                                    ...
                                    if (nameEqualUnderlineName) {
                                        ...
                                        if (useUnsafe /*&& (total & 3) != 1*/) {
                                            ...
                                            for (int i = 0; i < arrLength; ++i) {
                                                if (i + 1 < arrLength) {
                                                    int len = total > 8 ? 8 : total;
                                                    long l1 = longsWithCommaBoolFalse[i];
                                                    long l2 =
                                                            longsWithCommaBoolFalse[i + 1];
                                                    int i1 = intsWithCommaBoolFalse[i];
                                                    int i2 =
                                                            intsWithCommaBoolFalse[i + 1];
                                                    long l3 = EnvUtils.BIG_ENDIAN ? ((long) i1) << 32 | i2 :
                                                            ((long) i2) << 32 | i1;
                                                    compactBodyBuilder.append(
                                                            "\t\t\twriteMemory(writer, " + l1 + "L, " + l2 + "L, " +
                                                    ...
                                                    int len = total > 4 ? 4 : total;
                                                    compactBodyBuilder.append(
                                                            "\t\t\twriteMemory(writer, " + longsWithCommaBoolFalse[i] +
                                                                    "L, " + intsWithCommaBoolFalse[i] + ", " + len +
                                                                    ");\n");
                                                    total -= 4;
                                                }
                                            }
                                        ...
                                        if (useUnsafe /*&& (total & 3) != 1*/) {
                                            ...
                                            for (int i = 0; i < arrLength; ++i) {
                                                if (i + 1 < arrLength) {
                                                    ...
                                                    int len = total > 4 ? 4 : total;
                                                    compactBodyBuilder.append("\t\t\t\twriteMemory(writer, " +
                                                            longsWithCommaBoolFalse[i] + "L, " +
                                                            intsWithCommaBoolFalse[i] + ", " + len + ");\n");
                                                    total -= 4;
                                                }
                                            }
                                            if (remOne) {
                        ...
                        if (fieldSerializer.isCustomSerialize()) {
                            ...
                            if (runtime) {
                                if (EnvUtils.JDK_9_PLUS) {
                                    compactBodyBuilder.append("\t\t\twriter.writeJSONStringBytes(" + valueVar +
                                            ", (byte[]) getStringValue(" + valueVar + "));\n");
                                } else {
                                    compactBodyBuilder.append(
                                            "\t\t\twriter.writeJSONChars(getChars(" + valueVar + "));\n");
                                }
                            } else {
                            ...
                            compactBodyBuilder.append("\t\t\twriter.writeLong(" + valueVar + ", jsonConfig);\n");
                        } else if (returnType == Integer.class || returnType == Short.class ||
                                returnType == Byte.class) {
                            compactBodyBuilder.append("\t\t\twriter.writeInt(" + valueVar + ");\n");
                        } else if (returnType == BigDecimal.class) {
                            compactBodyBuilder.append(
                                    "\t\t\twriter.writeLatinString(" + valueVar + ".toString(), jsonConfig);\n");
                            ...
                            if (propertyDefinition == null || propertyDefinition.pattern().isEmpty() ||
                                    propertyDefinition.pattern().trim().equalsIgnoreCase("yyyy-MM-dd HH:mm:ss")) {
                                if (asTimestamp) {
                                    compactBodyBuilder.append("\t\t\twriter.writeLong(" + valueVar + ".getTime());\n");
                                } else {
                                    // use default pattern
                                    compactBodyBuilder.append(
                                            "\t\t\twriter.writeDate(" + valueVar + ", jsonConfig);\n");
                                }
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.ZoneId".equals(returnTypeName) ||
                                "java.time.ZoneOffset".equals(returnTypeName)) {
                            compactBodyBuilder.append("\t\t\twriter.writeJSONString(" + valueVar + ".getId());\n");
                        } else if ("java.time.Duration".equals(returnTypeName)) {
                            compactBodyBuilder.append(
                                    "\t\t\twriter.writeJSONDuration(" + valueVar + ".getSeconds(), " + valueVar +
                                            ".getNano());\n");
                        } else if ("java.time.Period".equals(returnTypeName)) {
                            compactBodyBuilder.append(
                                    "\t\t\twriter.writeJSONPeriod(" + valueVar + ".getYears(), " + valueVar +
                                            ".getMonths(), " + valueVar + ".getDays());\n");
                        } else if ("java.time.MonthDay".equals(returnTypeName)) {
                            if (fieldSerializer.getPropertyDefinition() == null ||
                                    fieldSerializer.getPropertyDefinition().pattern().length() == 0) {
                                compactBodyBuilder.append(
                                        "\t\t\twriter.writeJSONDefaultMonthDay(" + valueVar + ".getMonthValue(), " +
                                                valueVar + ".getDayOfMonth());\n");
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.YearMonth".equals(returnTypeName)) {
                            if (fieldSerializer.getPropertyDefinition() == null ||
                                    fieldSerializer.getPropertyDefinition().pattern().length() == 0) {
                                compactBodyBuilder.append(
                                        "\t\t\twriter.writeJSONYearMonth(" + valueVar + ".getYear(), " + valueVar +
                                                ".getMonthValue());\n");
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.Year".equals(returnTypeName)) {
                            compactBodyBuilder.append("\t\t\twriter.writeInt(" + valueVar + ".getValue());\n");
                        } else if ("java.time.Instant".equals(returnTypeName)) {
                            JSONPropertyDefinition propertyDefinition = fieldSerializer.getPropertyDefinition();
                            if (propertyDefinition == null ||
                                    (propertyDefinition.pattern().length() == 0 && !propertyDefinition.asTimestamp())) {
                                // use default pattern
                                compactBodyBuilder.append(
                                        "\t\t\twriter.writeJSONInstant(" + valueVar + ".getEpochSecond(), " + valueVar +
                                                ".getNano());\n");
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.LocalTime".equals(returnTypeName)) {
                            if (fieldSerializer.getPropertyDefinition() == null ||
                                    fieldSerializer.getPropertyDefinition().pattern().length() == 0) {
                                compactBodyBuilder.append(
                                        "\t\t\twriter.writeJSONTimeWithNano(" + valueVar + ".getHour(), " + valueVar +
                                                ".getMinute(), " + valueVar + ".getSecond(), " + valueVar +
                                                ".getNano());\n");
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.LocalDate".equals(returnTypeName)) {
                            if (fieldSerializer.getPropertyDefinition() == null ||
                                    fieldSerializer.getPropertyDefinition().pattern().length() == 0) {
                                compactBodyBuilder.append(
                                        "\t\t\twriter.writeJSONLocalDate(" + valueVar + ".getYear(), " + valueVar +
                                                ".getMonthValue(), " + valueVar + ".getDayOfMonth());\n");
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.LocalDateTime".equals(returnTypeName)) {
                            JSONPropertyDefinition propertyDefinition = fieldSerializer.getPropertyDefinition();
                            if (propertyDefinition == null ||
                                    (propertyDefinition.pattern().length() == 0 && !propertyDefinition.asTimestamp())) {
                                compactBodyBuilder.append(
                                        "\t\t\twriter.writeJSONLocalDateTime(" + valueVar + ".getYear(), " + valueVar +
                                                ".getMonthValue(), " + valueVar + ".getDayOfMonth(), " + valueVar +
                                                ".getHour(), " + valueVar + ".getMinute(), " + valueVar +
                                                ".getSecond(), " + valueVar + ".getNano(), \"\");\n");
                            } else {
                                useSerializerInvokeFlag = true;
                            }
                        } else if ("java.time.ZonedDateTime".equals(returnTypeName) ||
                                "java.time.OffsetDateTime".equals(returnTypeName)) {
                            if (fieldSerializer.getPropertyDefinition() == null ||
                                    fieldSerializer.getPropertyDefinition().pattern().length() == 0) {
                                if ("java.time.ZonedDateTime".equals(returnTypeName)) {
                                    compactBodyBuilder.append(
                                            "\t\t\twriter.writeJSONLocalDateTime(" + valueVar + ".getYear(), " +
                                                    valueVar + ".getMonthValue(), " + valueVar + ".getDayOfMonth(), " +
                                                    valueVar + ".getHour(), " + valueVar + ".getMinute(), " + valueVar +
                                                    ".getSecond(), " + valueVar + ".getNano(), " + valueVar +
                                                    ".getZone().getId());\n");
                                } else {
                                    compactBodyBuilder.append(
                                            "\t\t\twriter.writeJSONLocalDateTime(" + valueVar + ".getYear(), " +
                        ...
                        if (useSerializerInvokeFlag) {
                            if (!appendFieldSerializersFlag) {
                                ...
                                fieldSetBuilder.append(
                                        "\t\tJSONPojoFieldSerializer[] fieldSerializerUseMethods = pojoStructure" +
                                                ".getFieldSerializers(false);\n");
                            }
                            // use unsafe & field offset
                            fieldsDefinitionBuilder.append("\tfinal JSONPojoFieldSerializer ")
                                    .append(fieldSerializerName).append(";\n");
                            fieldSetBuilder.append("\t\tthis.").append(fieldSerializerName).append(" = ")
                                    .append("fieldSerializerUseMethods[").append(fieldIndex).append("];\n");

                            compactBodyBuilder.append(
                                    "\t\t\tdoSerialize(" + fieldSerializerName + ".getSerializer(), " + valueVar +
                                            ", writer, jsonConfig, -1);\n");
                        }
                        compactBodyBuilder.append("\t\t}\n");
                    }
                    ...
                    // use unsafe & field offset
                    fieldsDefinitionBuilder.append("\tfinal JSONPojoFieldSerializer ").append(fieldSerializerName)
                            .append(";\n");
                    fieldSetBuilder.append("\t\tthis.").append(fieldSerializerName).append(" = ")
                            .append("fieldSerializerUseMethods[").append(fieldIndex).append("];\n");

                    compactBodyBuilder.append(
                            "\t\tObject " + valueVar + " = invokeValue(" + fieldSerializerName + ", entity);\n");

                    // generate format code
                    if (Modifier.isPublic(returnType.getModifiers())) {
                        if (EnvUtils.JDK_7_BELOW) {
                            fmatOutBodyBuilder.append(
                                    "\t\t" + getCanonicalName(returnType) + " " + valueVar + " = invokeValue(" +
                                            fieldSerializerName + ", entity, " + getCanonicalName(returnType) +
                                            ".class);\n");
                        } else {
                            fmatOutBodyBuilder.append("\t\t" + getCanonicalName(returnType) + " " + valueVar + " = (" +
                                    getCanonicalName(returnType) + ") invokeValue(" + fieldSerializerName +
                                    ", entity);\n");
                        }
                    } else {
                        fmatOutBodyBuilder.append(
...
//            codeBuilder.append("\t}\n\n");
        ...
        // compact method code
        codeBuilder.append("\tpublic void serializePojoCompact(").append(canonicalName)
                .append(" entity, JSONWriter writer, JSONConfig jsonConfig, int indentLevel) throws Exception {\n");
        codeBuilder.append(compactHeaderBuilder);
        codeBuilder.append(compactBodyBuilder);
        codeBuilder.append("\t}\n\n");

        // formatOut method code
        codeBuilder.append("\tpublic void serializePojoFormatOut(").append(canonicalName)
                .append(" entity, JSONWriter writer, JSONConfig jsonConfig, int indentLevel) throws Exception {\n");
    ...
    private static void generateSerializeFieldFormatOutCode(boolean ensureNotEmptyFlag, boolean primitive,
                                                            String fieldSerializerName, String fieldKey,
                                                            String valueVar, boolean nameEqualUnderlineName,
                                                            String underlineName, boolean useUnsafe,
                                                            long[] longsFormatOut, int[] intsFormatOut,
                                                            int fieldNameFormatOutTokenLength, Class<?> returnType,
                                                            JSONPojoFieldSerializer fieldSerializer, boolean runtime,
                                                            StringBuilder fmatOutBodyBuilder) {
            ...
            if (useUnsafe) {
                ...
                for (int i = 0; i < arrLengthf; ++i) {
                    if (i + 1 < arrLengthf) {
                        ...
                        fmatOutBodyBuilder.append(tabFlag)
                                .append("\t\twriteMemory(writer, " + l1 + "L, " + l2 + "L, " + l3 + "L, " + len +
                                        ");\n");
                        totalf -= 8;
                        ++i;
                    } else {
                        ...
                        fmatOutBodyBuilder.append(tabFlag)
                                .append("\t\twriteMemory(writer, " + longsFormatOut[i] + "L, " + intsFormatOut[i] +
                                        ", " + len + ");\n");
                        totalf -= 4;
                    }
                }
        ...
        if (fieldSerializer.isCustomSerialize()) {
            ...
            if (runtime) {
                if (EnvUtils.JDK_9_PLUS) {
                    fmatOutBodyBuilder.append(tabFlag)
                            .append("\t\twriter.writeJSONStringBytes(" + valueVar + ", (byte[]) getStringValue(" +
                                    valueVar + "));\n");
                } else {
                    fmatOutBodyBuilder.append(tabFlag)
                            .append("\t\twriter.writeJSONChars(getChars(" + valueVar + "));\n");
                }
            } else {
            ...
            fmatOutBodyBuilder.append(tabFlag).append("\t\twriter.writeLong(" + valueVar + ", jsonConfig);\n");
                ...
                returnType == int.class || returnType == short.class || returnType == byte.class) {
            ...
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriter.writeLatinString(" + valueVar + ".toString(), jsonConfig);\n");
        } else if (returnType == BigInteger.class) {
            fmatOutBodyBuilder.append(tabFlag).append("\t\twriter.writeBigInteger(" + valueVar + ", jsonConfig);\n");
        } else if (returnType == String[].class) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriteStringArrayFormatOut(writer, " + valueVar + ", jsonConfig, indentPlus);\n");
        } else if (returnType == double[].class) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriteDoubleArrayFormatOut(writer, " + valueVar + ", jsonConfig, indentPlus);\n");
        } else if (returnType == long[].class) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriteLongArrayFormatOut(writer, " + valueVar + ", jsonConfig, indentPlus);\n");
        } else if (fieldSerializer.isStringCollection()) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriteStringCollectionFormatOut(writer, " + valueVar + ", jsonConfig, indentPlus);\n");
        } else if (returnType == UUID.class) {
        ...
        } else if ("java.time.ZoneId".equals(returnTypeName) || "java.time.ZoneOffset".equals(returnTypeName)) {
            fmatOutBodyBuilder.append(tabFlag).append("\t\twriter.writeJSONString(" + valueVar + ".getId());\n");
        } else if ("java.time.Duration".equals(returnTypeName)) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriter.writeJSONDuration(" + valueVar + ".getSeconds(), " + valueVar +
                            ".getNano());\n");
        } else if ("java.time.Period".equals(returnTypeName)) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\twriter.writeJSONPeriod(" + valueVar + ".getYears(), " + valueVar + ".getMonths(), " +
                            valueVar + ".getDays());\n");
        } else if ("java.time.MonthDay".equals(returnTypeName)) {
            if (fieldSerializer.getPropertyDefinition() == null ||
                    fieldSerializer.getPropertyDefinition().pattern().isEmpty()) {
                fmatOutBodyBuilder.append(tabFlag)
                        .append("\t\twriter.writeJSONDefaultMonthDay(" + valueVar + ".getMonthValue(), " + valueVar +
                                ".getDayOfMonth());\n");
            } else {
                useSerializerInvokeFlag = true;
            }
            ...
            if (fieldSerializer.getPropertyDefinition() == null ||
                    fieldSerializer.getPropertyDefinition().pattern().isEmpty()) {
                fmatOutBodyBuilder.append(tabFlag)
                        .append("\t\twriter.writeJSONYearMonth(" + valueVar + ".getYear(), " + valueVar +
                                ".getMonthValue());\n");
            } else {
                useSerializerInvokeFlag = true;
            }
            ...
            JSONPropertyDefinition propertyDefinition = fieldSerializer.getPropertyDefinition();
            if (propertyDefinition == null ||
                    (propertyDefinition.pattern().isEmpty() && !propertyDefinition.asTimestamp())) {
                // use default pattern
                fmatOutBodyBuilder.append(tabFlag)
                        .append("\t\twriter.writeJSONInstant(" + valueVar + ".getEpochSecond(), " + valueVar +
                                ".getNano());\n");
            } else {
                useSerializerInvokeFlag = true;
            }
            ...
            if (fieldSerializer.getPropertyDefinition() == null ||
                    fieldSerializer.getPropertyDefinition().pattern().isEmpty()) {
                fmatOutBodyBuilder.append(tabFlag)
                        .append("\t\twriter.writeJSONTimeWithNano(" + valueVar + ".getHour(), " + valueVar +
                                ".getMinute(), " + valueVar + ".getSecond(), " + valueVar + ".getNano());\n");
            } else {
                useSerializerInvokeFlag = true;
            }
        } else if ("java.time.LocalDate".equals(returnTypeName)) {
            if (fieldSerializer.getPropertyDefinition() == null ||
                    fieldSerializer.getPropertyDefinition().pattern().isEmpty()) {
                fmatOutBodyBuilder.append(tabFlag)
                        .append("\t\twriter.writeJSONLocalDate(" + valueVar + ".getYear(), " + valueVar +
                                ".getMonthValue(), " + valueVar + ".getDayOfMonth());\n");
            } else {
                useSerializerInvokeFlag = true;
            }
            ...
            if (propertyDefinition == null ||
                    (propertyDefinition.pattern().isEmpty() && !propertyDefinition.asTimestamp())) {
                fmatOutBodyBuilder.append(tabFlag)
                        .append("\t\twriter.writeJSONLocalDateTime(" + valueVar + ".getYear(), " + valueVar +
                                ".getMonthValue(), " + valueVar + ".getDayOfMonth(), " + valueVar + ".getHour(), " +
                                valueVar + ".getMinute(), " + valueVar + ".getSecond(), " + valueVar +
                                ".getNano(), \"\");\n");
            } else {
                useSerializerInvokeFlag = true;
            }
                ...
                if ("java.time.ZonedDateTime".equals(returnTypeName)) {
                    fmatOutBodyBuilder.append(tabFlag)
                            .append("\t\twriter.writeJSONLocalDateTime(" + valueVar + ".getYear(), " + valueVar +
                                    ".getMonthValue(), " + valueVar + ".getDayOfMonth(), " + valueVar + ".getHour(), " +
                                    valueVar + ".getMinute(), " + valueVar + ".getSecond(), " + valueVar +
                                    ".getNano(), " + valueVar + ".getZone().getId());\n");
                } else {
                    fmatOutBodyBuilder.append(tabFlag)
                            .append("\t\twriter.writeJSONLocalDateTime(" + valueVar + ".getYear(), " + valueVar +
                                    ".getMonthValue(), " + valueVar + ".getDayOfMonth(), " + valueVar + ".getHour(), " +
                                    valueVar + ".getMinute(), " + valueVar + ".getSecond(), " + valueVar +
                                    ".getNano(), " + valueVar + ".getOffset().getId());\n");
                }
            } else {
                useSerializerInvokeFlag = true;
            }
        ...
        if (useSerializerInvokeFlag) {
            fmatOutBodyBuilder.append(tabFlag)
                    .append("\t\tdoSerialize(" + fieldSerializerName + ".getSerializer(), " + valueVar +
                            ", writer, jsonConfig, indentPlus);\n");
        }
        if (checkNullFlag) {
            fmatOutBodyBuilder.append("\t\t}\n");
        }
