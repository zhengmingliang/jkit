...
//            new DateTemplate[]{new DateTemplate("yyyy-MM-dd HH:mm:ss.S"), new DateTemplate("yyyy.MM.dd HH:mm:ss.S")
//            , new DateTemplate("yyyy/MM/dd HH:mm:ss.S")};

    // 年月日时分秒-19bits
    private static DateTemplate[] pattern_19bit =
            new DateTemplate[]{new DateTemplate("yyyy.MM.dd HH:mm:ss")};
//            new DateTemplate[]{new DateTemplate("yyyy-MM-dd HH:mm:ss"), new DateTemplate("yyyy.MM.dd HH:mm:ss"),
//            new DateTemplate("yyyy/MM/dd HH:mm:ss")};
...
//            new DateTemplate[]{new DateTemplate("yyyy-MM-dd"), new DateTemplate("yyyy.MM.dd"), new DateTemplate
//            ("yyyy/MM/dd")};
    ...
    /**
     * 按字符串长度自动匹配日期格式并解析为时间戳
     *
     * @param originDate 日期字符串，支持 8、10、19、21-23 位以及 Y.M.d 等宽松格式
     * @return 解析得到的时间戳（毫秒）
     * @throws IllegalArgumentException 所有候选格式都无法解析该字符串时抛出
     */
    ...
    /**
     * 按字符串长度自动匹配日期格式并解析为日期对象
     *
     * @param originDate 日期字符串，支持 8、10、19、21-23 位以及 Y.M.d 等宽松格式
     * @return 解析得到的 {@link GregorianDate} 对象
     * @throws IllegalArgumentException 所有候选格式都无法解析该字符串时抛出
     */
