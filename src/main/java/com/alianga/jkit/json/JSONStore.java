...
import com.alianga.jkit.expression.EvaluateEnvironment;
...
import com.alianga.jkit.json.internal.compiler.JDKCompiler;
 ...
 *
 * @time 2025/12/29 21:05
 */
final class JSONStore {
    static final int CATEGORY_LEN = ReflectConsts.ClassCategory.values().length;
    // Default JSONStore
    ...
    void init() {
        ...
        putTypeSerializer(COLLECTION_SER, ArrayList.class, LinkedList.class, Vector.class, HashSet.class,
                LinkedHashSet.class);
    }
            ...
            switch (classCategory) {
                ...
                case CollectionCategory: {
                    GenericParameterizedType<?> parameterizedType =
                            GenericParameterizedType.collectionOf((Class<? extends Collection>) type);
                    deserializer =
                            new JSONTypeDeserializer.CollectionImpl.CollectionInstanceImpl(this, parameterizedType);
                    break;
                }
                case MapCategory: {
                    GenericParameterizedType<?> parameterizedType =
                            GenericParameterizedType.mapOf((Class<? extends Map>) type);
     ...
     * @return Pojo属性反序列化器
     */
    JSONTypeDeserializer getFieldDeserializer(GenericParameterizedType<?> genericParameterizedType,
        ...
        switch (classCategory) {
            case CollectionCategory:
                ...
                switch (collectionType) {
                    ...
                    default:
                        return new JSONTypeDeserializer.CollectionImpl.CollectionInstanceImpl(this,
            ...
            case DateCategory: {
                ...
                return property == null ? JSONTypeDeserializer.DATE :
                        new JSONTypeDeserializer.DateImpl.DateInstanceImpl(genericParameterizedType, property);
            }
            case ObjectCategory: {
                ...
                if (classStrucWrap.isTemporal()) {
                    ...
                    return JSONTemporalDeserializer.getTemporalDeserializerInstance(classWrapperType,
                            genericParameterizedType, property);
                }
            }
            break;
            default:
    ...
    JSONTypeDeserializer createObjectDeserializer(Class<?> type) {
        ...
        if (classStrucWrap.isRecord()) {
            ...
            if (classStrucWrap.isTemporal()) {
                ClassStrucWrap.ClassWrapperType classWrapperType = classStrucWrap.getClassWrapperType();
                return JSONTemporalDeserializer.getTemporalDeserializerInstance(classWrapperType,
                        GenericParameterizedType.actualType(type), null);
            }
    ...
    JSONTypeDeserializer createEnumDeserializer(Class<?> type) {
        ...
        return enumMatcher.isPlhv() ? new JSONTypeDeserializer.EnumImpl.EnumInstanceOptimizeImpl(values, enumMatcher) :
                new JSONTypeDeserializer.EnumImpl.EnumInstanceImpl(values, enumMatcher);
    }
     ...
     * @return
     */
    JSONTypeDeserializer getPossibleTypesTypeDeserializer(final Class<?> parentType, final Class<?>[] possibleTypes,
                                                          final Expression possibleExpression) {
        ...
        return new JSONTypeDeserializer() {
            @Override
            protected Object deserialize(CharSource charSource, char[] buf, final int fromIndex,
                ...
                if (possibleExpression == null) {
                    for (int deserIndex = 0, len = possibleDeserializers.length; deserIndex < len; ++deserIndex) {
                        ...
                        if (deserializer == null || !deserializer.checkIfSupportedStartsWith(buf[fromIndex])) {
                            continue;
                        }
                        Class<?> actualType =
                                deserIndex < possibleTypes.length ? possibleTypes[deserIndex] : parentType;
                        try {
                            GenericParameterizedType<?> localParameterizedType =
                                    GenericParameterizedType.actualType(actualType);
                            Object result = deserializer.deserialize(charSource, buf, fromIndex, localParameterizedType,
                    ...
                    try {
                        Object context =
                                ANY_DESER.deserialize(charSource, buf, fromIndex, parameterizedType, defaultValue,
                                        endToken, parseContext);
                        ...
                        if (evalResult instanceof Integer) {
                            ...
                            if (index < possibleTypes.length && index > -1) {
                                ...
                                if (deserializer != null) {
                                    return deserializer.deserialize(charSource, buf, fromIndex, localParameterizedType,
                                            defaultValue, endToken, parseContext);
                                }
                            }
                        }
            ...
            @Override
            protected Object deserialize(CharSource charSource, byte[] buf, final int fromIndex,
                                         GenericParameterizedType parameterizedType, Object defaultValue, int endToken,
                                         JSONParseContext parseContext) throws Exception {
                if (possibleExpression == null) {
                    for (int deserIndex = 0, len = possibleDeserializers.length; deserIndex < len; ++deserIndex) {
            ...
            if (classCategory == ReflectConsts.ClassCategory.ObjectCategory) {
                ...
                if (classStrucWrap.isTemporal()) {
                    ...
                    if (enableJIT && pojoStructure.isSupportedJIT()) {
                        try {
                            Class<?> serializerClass = JDKCompiler.compileJavaSource(
                                    JSONPojoSerializer.generateRuntimeJavaCodeSource(pojoStructure));
                            Constructor constructor =
                                    serializerClass.getDeclaredConstructor(new Class[]{JSONPojoStructure.class});
     ...
     * @return
     */
    JSONTypeSerializer getFieldTypeSerializer(ReflectConsts.ClassCategory classCategory, Class<?> type,
    ...
    JSONTypeSerializer getTypeSerializer(ReflectConsts.ClassCategory classCategory,
                                         JSONPropertyDefinition propertyDefinition) {
        ...
        if (classCategory == ReflectConsts.ClassCategory.DateCategory) {
            if (propertyDefinition != null) {
                ...
                if (!pattern.isEmpty()) {
                    ...
                    return new JSONTypeSerializer.DatePatternImpl(DateFormatter.of(pattern),
                            JSONGeneral.getTimeZone(timeZoneId));
                }
            }
        }
    ...
    <E> JSONTypeDeserializer buildDeserializer(final JSONTypeMapper<E> mapper) {
        return new JSONTypeDeserializer() {
            ...
            protected Object deserialize(CharSource charSource, char[] buf, int fromIndex,
                                         ...
                                         int endToken, JSONParseContext jsonParseContext) throws Exception {
                Object value =
                        ANY_DESER.deserialize(charSource, buf, fromIndex, parameterizedType, defaultValue, endToken,
                                jsonParseContext);
                Object result = mapper.readOf(value);
                if (result == null || parameterizedType.getActualType().isInstance(result)) {
                    return result;
                }
                throw new JSONException("Syntax error, at pos " + fromIndex + ", context text by '" +
                        JSONGeneral.createErrorContextText(buf, fromIndex) + result.getClass().getName() +
                        " cannot be cast to " + parameterizedType.getActualType().getName());
            }
            ...
            protected Object deserialize(CharSource charSource, byte[] bytes, int fromIndex,
                                         ...
                                         int endToken, JSONParseContext jsonParseContext) throws Exception {
                Object value =
                        ANY_DESER.deserialize(charSource, bytes, fromIndex, parameterizedType, defaultValue, endToken,
                                jsonParseContext);
                Object result = mapper.readOf(value);
                if (result == null || parameterizedType.getActualType().isInstance(result)) {
                    return result;
                ...
                throw new JSONException("Syntax error, at pos " + fromIndex + ", context text by '" +
                        JSONGeneral.createErrorContextText(bytes, fromIndex) + result.getClass().getName() +
                        " cannot be cast to " + parameterizedType.getActualType().getName());
            }
        };
    }
     ...
     * @return
     */
    static final JSONTypeDeserializer wrapTypeDeserializer(final JSONTypeDeserializer original,
        ...
        return new JSONTypeDeserializer() {
            ...
            protected Object deserialize(CharSource charSource, char[] buf, int fromIndex,
                                         ...
                                         int endToken, JSONParseContext jsonParseContext) throws Exception {
                Object result =
                        original.deserialize(charSource, buf, fromIndex, parameterizedType, defaultValue, endToken,
                                jsonParseContext);
                ...
                throw new JSONException("Syntax error, at pos " + fromIndex + ", context text by '" +
                        JSONGeneral.createErrorContextText(buf, fromIndex) + "...', " + result.getClass().getName() +
                        " cannot be cast to " + targetType.getName());
            }
            ...
            protected Object deserialize(CharSource charSource, byte[] bytes, int fromIndex,
                                         ...
                                         int endToken, JSONParseContext jsonParseContext) throws Exception {
                Object result =
                        original.deserialize(charSource, bytes, fromIndex, parameterizedType, defaultValue, endToken,
                                jsonParseContext);
                if (result == null || targetType.isInstance(result)) {
                    return result;
                }
    ...
    public void setNoneStringMode(boolean noneStringMode) {
        if (noneStringMode == this.noneStringMode) {
            return;
        }
        ...
        if (noneStringMode) {
            ...
            // serializer
            putTypeSerializer(STRING_SER = JSONTypeSerializer.TO_NULL, String.class, CharSequence.class,
                    String[].class);
        } else {
            // deserializer
    ...
    public void updatePojoProperties(Class<?> pojoClass, Map<String, JSONPropertyDefinition> properties) {
        ...
        if (pojoStruc != null) {
            Map<String, JSONPropertyDefinition> targetProperties =
                    new HashMap<String, JSONPropertyDefinition>(pojoStruc.propertyDefinitions);
