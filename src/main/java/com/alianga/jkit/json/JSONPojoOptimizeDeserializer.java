...
/**
 * branch optimization, unpacking, and JIT optimization for commonly used entity classes (JDK9+ has a slightly
 * noticeable effect +5%~20%)
 ...
 *
 * @param <T> the POJO type to deserialize
 */
public abstract class JSONPojoOptimizeDeserializer<T> extends JSONPojoDeserializer {
    /** the structure metadata of the target POJO class, used to create instances and locate fields */
    ...
    abstract static class GenericImpl extends JSONPojoOptimizeDeserializer {
        ...
        final JSONPojoFieldDeserializer matchDeserializer(char[] buf, int offset, int endToken,
                                                          JSONParseContext parseContext) {
            char c;
            if ((c = buf[offset]) != endToken) {
                long hashValue = readPojoFieldHash(c, buf, offset, endToken, parseContext);
                JSONKeyValueMap.EntryNode<JSONPojoFieldDeserializer> entryNode =
                        valueEntryNodes[(int) (hashValue & mask)];
                if (entryNode != null && (!parseContext.strictMode ? entryNode.hash == hashValue :
                        entryNode.equals(buf, offset, parseContext.endIndex - offset))) {
        ...
        final JSONPojoFieldDeserializer matchDeserializer(byte[] buf, int offset, int endToken,
                                                          JSONParseContext parseContext) {
            byte c;
            if ((c = buf[offset]) != endToken) {
                long hashValue = readPojoFieldHash(c, buf, offset, endToken, parseContext);
    ...
    protected final T deserialize(CharSource charSource, char[] buf, int fromIndex,
                                  GenericParameterizedType<?> parameterizedType, Object entity, int endToken,
    ...
    protected final T deserialize(CharSource charSource, byte[] buf, int fromIndex,
                                  GenericParameterizedType<?> parameterizedType, Object entity, int endToken,
                                  JSONParseContext parseContext) throws Exception {
    ...
    abstract JSONPojoFieldDeserializer matchDeserializer(char[] buf, int offset, int endToken,
                                                         JSONParseContext parseContext);

    final JSONPojoFieldDeserializer matchUnquoted(char[] buf, int offset, int beginByte,
                                                  JSONParseContext parseContext) {
    ...
    final Object deserializePojo(CharSource charSource, char[] buf, int fromIndex,
        ...
        final boolean allowLastEndComma = parseContext.allowLastEndComma;
        ...
        for (; ; ) {
            fieldDeserializer = c == DOUBLE_QUOTATION || c == '\'' ? matchDeserializer(buf, i + 1, c, parseContext) :
                    matchUnquoted(buf, i, c, parseContext);
            ...
            if (c == COLON_SIGN) {
                ...
                if (fieldDeserializer != null) {
                    Object value = fieldDeserializer.deserializer.deserialize(charSource, buf, i,
                            fieldDeserializer.genericParameterizedType, null, '}', parseContext);
                ...
                c = buf[i = skipWhiteSpacesOrComment(buf, parseContext.endIndex, allowComment, parseContext)];
                if (c == END_OBJECT || ((isComma = c == COMMA) &&
                        ((c = buf[i = skipWhiteSpacesOrComment(buf, i, allowComment, parseContext)]) == END_OBJECT) &&
                        allowLastEndComma)) {
                    parseContext.endIndex = i;
                    return entity;
                }
    ...
    abstract JSONPojoFieldDeserializer matchDeserializer(byte[] buf, int offset, int endToken,
                                                         JSONParseContext parseContext);

    final JSONPojoFieldDeserializer matchUnquoted(byte[] buf, int offset, int beginByte,
                                                  JSONParseContext parseContext) {
        if (parseContext.allowUnquotedFieldNames) {
            final int from = offset;
            long hashValue = beginByte;
    ...
    final Object deserializePojo(CharSource charSource, byte[] buf, int fromIndex,
                                 GenericParameterizedType<?> parameterizedType, Object entity,
                                 JSONParseContext parseContext) throws Exception {
        if (entity == null) {
            entity = pojoStructure.newInstance();
        ...
        JSONPojoFieldDeserializer fieldDeserializer =
                c == DOUBLE_QUOTATION || c == '\'' ? matchDeserializer(buf, i + 1, c, parseContext) :
                        matchUnquoted(buf, i, c, parseContext);
        c = buf[i = skipWhiteSpacesOrComment(buf, parseContext.endIndex, allowComment, parseContext)];
        if (c == COLON_SIGN) {
            i = skipWhiteSpacesOrComment(buf, i, allowComment, parseContext);
            if (fieldDeserializer != null) {
                Object value = fieldDeserializer.deserializer.deserialize(charSource, buf, i,
                        fieldDeserializer.genericParameterizedType, null, END_OBJECT, parseContext);
                JSON_SECURE_TRUSTED_ACCESS.set(fieldDeserializer.setterInfo, entity, value);
            } else {
    ...
    static final int readPojoFieldHashPlhv(int hashValue, byte[] buf, int offset, int endToken,
                                           JSONParseContext parseContext) {
        int c;
        int c1;
    ...
    static final int readPojoFieldHashPlhv(int hashValue, char[] buf, int offset, int endToken,
                                           JSONParseContext parseContext) {
        int c;
        int c1;
        if ((c = buf[++offset]) != endToken && (c1 = buf[++offset]) != endToken) {
    ...
    static JSONPojoOptimizeDeserializer<?> optimize(JSONPojoStructure pojoStructure) {
        if (pojoStructure.fieldDeserializerMatcher.isPlhv()) {
            final JSONKeyValueMap<JSONPojoFieldDeserializer> fieldDeserializerMap =
                    pojoStructure.fieldDeserializerMatcher.valueMapForChars;
            ...
            if (size == 1) {
                ...
                return new JSONPojoOptimizeDeserializer(pojoStructure) {
                    JSONPojoFieldDeserializer matchDeserializer(char[] buf, int offset, int endToken,
                                                                JSONParseContext parseContext) {
                        int c;
                        if ((c = buf[offset]) != endToken) {
                            int hashValue = readPojoFieldHashPlhv(c, buf, offset, endToken, parseContext);
                            if ((!parseContext.strictMode ? hv == hashValue :
                                    one.equals(buf, offset, parseContext.endIndex - offset))) {
                                return value;
                            }
                            return skipIfNotMatch(buf, endToken, parseContext);
                        }
                    ...
                    JSONPojoFieldDeserializer matchDeserializer(byte[] buf, int offset, int endToken,
                                                                JSONParseContext parseContext) {
                        int c;
                        if ((c = buf[offset]) != endToken) {
                            int hashValue = readPojoFieldHashPlhv(c, buf, offset, endToken, parseContext);
...
//                                if (!parseContext.strictMode || one.equals(buf, offset, parseContext.endIndex -
//                                offset)) {
//                                    return one.value;
//                                }
//                            } else if(hashValue == two.hash) {
//                                if (!parseContext.strictMode || two.equals(buf, offset, parseContext.endIndex -
//                                offset)) {
//                                    return two.value;
//                                }
//                            }
...
//            }
            ...
            return new JSONPojoOptimizeDeserializer(pojoStructure) {
                JSONPojoFieldDeserializer matchDeserializer(char[] buf, int offset, int endToken,
                    ...
                    if ((c = buf[offset]) != endToken) {
                        int hashValue = readPojoFieldHashPlhv(c, buf, offset, endToken, parseContext);
                        JSONKeyValueMap.EntryNode<JSONPojoFieldDeserializer> entryNode =
                                valueEntryNodes[hashValue & mask];
                        if (entryNode != null && (!parseContext.strictMode ? entryNode.hash == hashValue :
                                entryNode.equals(buf, offset, parseContext.endIndex - offset))) {
                            return entryNode.value;
                        }
            ...
            return new GenericImpl(pojoStructure) {
                long readPojoFieldHash(long initValue, byte[] buf, int offset, int endToken,
                                       JSONParseContext parseContext) {
                ...
                long readPojoFieldHash(long initValue, char[] buf, int offset, int endToken,
                                       JSONParseContext parseContext) {
                    long hashValue = initValue;
                    int c;
                    int c1;
