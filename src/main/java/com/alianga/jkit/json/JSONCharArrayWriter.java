...
//    }
    ...
    @Override
    public final void writeJSONLocalDateTime(int year, int month, int day, int hour, int minute, int second, int nano,
...
//    }
    ...
    public final void writeJSONTimeWithNano(int hourOfDay, int minute, int second, int nano) {
        ...
        buf[off++] = '"';
        off += JSONMemoryHandle.putInt(buf, off, TWO_DIGITS_32_BITS[hourOfDay]);
        off += JSONMemoryHandle.putLong(buf, off,
                mergeInt64(minute, ':', ':')); // writeTwoDigitsAndPreSuffix(minute, ':', ':', buf, off);
        off += JSONMemoryHandle.putInt(buf, off, TWO_DIGITS_32_BITS[second]);
        if (nano > 0) {
            off = writeNano(nano, buf, off);
        }
    ...
    public final void writeJSONPeriod(int years, int months, int days) throws IOException {
        ensureCapacity(24 + SECURITY_UNCHECK_SPACE);
        int off = count;
        buf[off++] = '"';
        buf[off++] = 'P';
    ...
    @Override
    public final void writeJSONDuration(long seconds, int nano) {
        ensureCapacity(24 + SECURITY_UNCHECK_SPACE);
        int off = count;
        buf[off++] = '"';
        if (seconds == 0 && nano == 0) {
            off += JSONMemoryHandle.putLong(buf, off, JSONGeneral.DURATION_ZERO_LONG);
            buf[off++] = '"';
            count = off;
            return;
        }
        off += JSONMemoryHandle.putInt(buf, off, JSONGeneral.DURATION_PT_INT); // PT
        final boolean negative = seconds < 0;
        if (negative) {
            seconds = -seconds;
        }
        long hour = seconds /
                3600; // 暂不优化: seconds < 5146971002711999L ? EnvUtils.JDK_AGENT_INSTANCE.multiplyHighKaratsuba
        // (seconds, 0x123456789abce0L) : seconds / 3600 ( // ((2^64 - 3599 * 0x123456789abce0L) / 3584) * 3600 +
        // 3599 * 2 + 1 -> 5146971002711999L)
        if (hour > 0) {
            if (negative) {
                buf[off++] = '-';
            }
    ...
    public void writeJSONYearMonth(int year, int monthValue) throws IOException {
        ...
        buf[off++] = '-';
        off += JSONMemoryHandle.putInt(buf, off, TWO_DIGITS_32_BITS[monthValue]);
        buf[off++] = '"';
        count = off;
    }

    public final void writeJSONDefaultMonthDay(int monthValue, int dayOfMonth) throws IOException {
        ensureCapacity(9 + SECURITY_UNCHECK_SPACE);
        int off = count;
        buf[off++] = '"';
        buf[off++] = '-';
        off += JSONMemoryHandle.putLong(buf, off, mergeInt64(monthValue, '-', '-'));
        off += JSONMemoryHandle.putInt(buf, off, TWO_DIGITS_32_BITS[dayOfMonth]);
        buf[off++] = '"';
        count = off;
    }
    ...
    public void writeJSONChars(char[] chars) throws IOException {
        ...
        if (len < 64) {
            ...
            int count = this.count;
            int beginIndex = 0;
            int i = JSONGeneral.JSON_UTIL.toNoEscapeOffset(chars, 0);
            buf[count++] = '"';
            for (; i < len; ++i) {
                char ch = chars[i];
                String escapeStr;
