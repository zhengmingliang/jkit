...
final class JSONValidator extends JSONGeneral {
    ...
    static boolean validate(CharSource source, char[] chars, ReadOption... readOptions) {
        ...
        try {
            ...
            switch (beginChar) {
                case '{':
                    validate = JSONStore.INSTANCE.MAP_DESER.validate(source, chars, fromIndex, toIndex, (char) 0,
                            parseContext);
                    break;
                case '[':
                    validate = JSONStore.INSTANCE.COLLECTION_DESER.validate(source, chars, fromIndex, toIndex, (char) 0,
                            parseContext);
                    break;
                case '\'':
                case '"':
                    ...
                    try {
                        switch (beginChar) {
                            ...
                            default: {
                                ...
                                JSONTypeDeserializer.NUMBER_SKIPPER.deserialize(source, numBuf, 0,
    ...
    static boolean validate(CharSource source, byte[] bytes, ReadOption... readOptions) {
        ...
        try {
            ...
            switch (beginByte) {
                case '{':
                    validate =
                            JSONStore.INSTANCE.MAP_DESER.validate(source, bytes, fromIndex, toIndex, JSONGeneral.ZERO,
                                    parseContext);
                    break;
                case '[':
                    validate = JSONStore.INSTANCE.COLLECTION_DESER.validate(source, bytes, fromIndex, toIndex,
                            JSONGeneral.ZERO, parseContext);
                    break;
                case '\'':
                case '"':
