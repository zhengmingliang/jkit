...
//    }
    ...
    public void writeLatinJSONString(String value, byte[] bytes) throws IOException {
        ...
        if (len < 32) {
            ...
            do {
                if (len > 15) {
                    long v1 = JSONMemoryHandle.getLong(bytes, 0);
                    long v2 = JSONMemoryHandle.getLong(bytes, 8);
            ...
            // len >= 32
            int beginIndex = 0;
            int i = JSONGeneral.JSON_UTIL.toNoEscapeOffset(bytes, 0);
            for (; i < len; ++i) {
                byte b = bytes[i];
                String escapeStr;
        ...
        if (EnvUtils.JDK_9_PLUS) {
            ...
            if (len <= 32) {
                ...
                if (len >= 8) {
                    ...
                    JSONMemoryHandle.putLong(buf, count, JSONMemoryHandle.getLong(bytes, 0));
                    if (len > 8) {
                        JSONMemoryHandle.putLong(buf, count + (remOff = len - 8),
                                JSONMemoryHandle.getLong(bytes, remOff));
                    }
                    this.count = count + len;
                    return;
                }
                if (len >= 4) {
                    ...
                    if (len > 4) {
                        JSONMemoryHandle.putInt(buf, count + (remOff = len - 4),
                                JSONMemoryHandle.getInt(bytes, remOff));
                    }
                    this.count = count + len;
                    return;
                }
...
//    }
    ...
    @Override
    public void writeJSONLocalDateTime(int year, int month, int day, int hour, int minute, int second, int nano,
    ...
    int writeNano(int nano, int off) {
        buf[off++] = '.';
        int top8 = (int) (nano * 1717986919L >>
                34); // EnvUtils.JDK_AGENT_INSTANCE.multiplyHighKaratsuba(nano, 0x199999999999999aL); // nano / 10;
        int remDigit = nano - top8 * 10;
        if (remDigit > 0) {
            // 8 + 1
            int top4 = (int) (top8 * 1759218605L >>
                    44); // EnvUtils.JDK_AGENT_INSTANCE.multiplyHighKaratsuba(top8, 0x68db8bac710ccL); // top8 / 10000;
    ...
    public void writeJSONYearMonth(int year, int monthValue) throws IOException {
        ...
        if (year < 10000) {
            off += JSONMemoryHandle.putLong(buf, off,
                    JSONMemoryHandle.JSON_ENDIAN.mergeYearAndMonth(year, monthValue)) - 1;
        } else {
            off += writeLong(year, buf, off);
