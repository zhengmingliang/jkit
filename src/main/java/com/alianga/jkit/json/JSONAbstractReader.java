...
abstract class JSONAbstractReader extends JSONGeneral {
    ...
    /**
     * Set the read options and apply them to the parse context.
     *
     * @param readOptions the read options to apply
     */
    ...
    /**
     * Set the max time to wait for the result in async mode.
     *
     * @param timeout the timeout in seconds used by {@link #getResult()}
     */
     ...
     *
     * @param multiple {@code true} to keep the stream open and read multiple JSONs
     */
    ...
    /**
     * Read the stream synchronously and return the parsed result.
     *
     * @return the parsed object, the actual type depends on the JSON content
     */
    public abstract Object read();

    /**
     * Read the stream without a hook, the result can be obtained by {@link #getResult()}.
     *
     * @param async {@code true} to read in a new thread, {@code false} to read synchronously
     */
    ...
    /**
     * Read the stream synchronously and return the result as the given type.
     *
     * @param actualType the expected result type
     * @param <T>        the expected result type
     * @return the parsed result cast to {@code T}, may be {@code null} if nothing was parsed
     */
    ...
    /**
     * Read the stream synchronously and return the result as the given generic type.
     *
     * @param genericType the expected result type with generic information
     * @param <T>         the expected result type
     * @return the parsed result cast to {@code T}, may be {@code null} if nothing was parsed
     */
    ...
    /**
     * Read the stream synchronously with the given hook.
     *
     * @param readerHook the hook called on the parsed values, may be {@code null}
     */
    ...
    /**
     * Read the stream with the given hook, does nothing if a reading is already in progress.
     *
     * @param readerHook the hook called on the parsed values, may be {@code null}
     * @param async      {@code true} to read in a new thread, {@code false} to read synchronously
     */
    public void read(JSONReaderHook readerHook, boolean async) {
        ...
        if (!async) {
            ...
            new Thread(() -> executeReadStream(null)).start();
        }
    }
    ...
    public final void skipNext() {
        try {
            ...
            switch (current) {
                ...
                default:
                    throw new JSONException("character stream start character error for '" + (char) current +
                            "'. the start token is only supported  '{' or '[' on multiple mode");
            }
        } catch (Exception e) {
     ...
     *
     * @param count the number of JSONs to skip
     */
        ...
        switch (current) {
            ...
            default:
                throw new UnsupportedOperationException(
                        "Character stream start character error. Only object({) or array([) parsing is supported");
        }
        // clear white space characters
        if (isAborted()) {
            return;
    ...
    private Number readNumber(char endSyntax) throws Exception {
        ...
        if (mode == 0) {
            ...
            double dv = NumberUtils.scientificToIEEEDouble(value,
                    expNegative ? expValue + decimalCount : decimalCount - expValue);
...
//                }
                    ...
                    if (isSkipValue) {
                        ...
                        switch (current) {
                            case '{':
                                ...
                                invokeValueOfObject(useHookValueParse, key, value, map, instance, setterInfo, nextPath,
                                        JSONNode.OBJECT);
                                break;
                            case '[':
                            ...
                            case '"':
                                value = parseStringTo(this.readString(), valueType,
                                        jsonProperty == null ? null : JSONPropertyDefinition.of(jsonProperty));
                                invokeValueOfObject(useHookValueParse, key, value, map, instance, setterInfo, nextPath,
                                        JSONNode.STRING);
                                break;
                            case 'n':
                                readNull();
                                invokeValueOfObject(useHookValueParse, key, null, map, instance, setterInfo, nextPath,
                                        JSONNode.NULL);
                                break;
                            case 't':
                                readTrue();
    ...
    final Object readArray(String path, GenericParameterizedType<?> genericType) throws Exception {
        ...
        boolean elementPrimitive = false;
        ...
        for (; ; ) {
            ...
            if (current == ']') {
                // always supported ','
                return isArrayCls ?
                        CollectionUtils.toArray(collection, actualType == null ? Object.class : actualType) :
                        collection;
            }
            boolean toBreakOrContinue = false;
            ...
            switch (current) {
                case '{': {
                    ...
                    this.parseCollectionElement(useHookValueParse, value, collection, instance, elementIndex, nextPath,
                            JSONNode.OBJECT);
                    break;
                }
                case '[': {
                ...
                case 'n':
                    readNull();
                    this.parseCollectionElement(useHookValueParse,
                            elementPrimitive ? ObjectUtils.defaulValue(actualType) : null, collection, instance,
                            elementIndex, nextPath, JSONNode.NULL);
                    break;
                case 't': {
                    readTrue();
        ...
        return isArrayCls ? CollectionUtils.toArray(collection, actualType == null ? Object.class : actualType) :
                collection;
    }
    ...
    private Object toBoolType(boolean b, GenericParameterizedType<?> valueType) {
        if (valueType == null || valueType == GenericParameterizedType.AnyType ||
                valueType.getActualClassCategory() == ReflectConsts.ClassCategory.BoolCategory) {
            return b;
        }
    ...
    private Object parseNumberTo(Object simpleValue, GenericParameterizedType valueType) {
        ...
        if (valueType == null ||
                (classCategory = valueType.getActualClassCategory()) == ReflectConsts.ClassCategory.ANY) {
            return simpleValue;
        }
        Class<?> actualType = valueType.getActualType();
    ...
    private Object parseStringTo(String value, GenericParameterizedType<?> valueType,
                                 JSONPropertyDefinition propertyDefinition) throws Exception {
        if (value == null) {
            return null;
        }
        if (valueType == null || valueType == GenericParameterizedType.AnyType) {
    ...
    private void invokeValueOfObject(boolean useHookParse, String key, Object value, Map map, Object instance,
                                     SetterInfo setterInfo, String nextPath, int type) throws Exception {
        if (useHookParse) {
    ...
    private void parseCollectionElement(boolean useHookParse, Object value, Collection collection, Object instance,
                                        int elementIndex, String nextPath, int type) throws Exception {
        if (useHookParse) {
    ...
    /**
     * Whether the reading has been aborted.
     *
     * @return {@code true} if the reading was aborted, otherwise {@code false}
     */
                ...
                switch (current) {
                    ...
                    case 'u':
                        ...
                        try {
                            ...
                            if (c64 < 0) {
                                throw new JSONException("Syntax error, from pos " + offset + ", invalid unicode " +
                                        new String(new char[]{(char) c1, (char) c2, (char) c3, (char) c4}));
                            }
     ...
     *
     * @return {@code true} if the whole stream has been read, otherwise {@code false}
     */
     ...
     *
     * @return the parsed result, in async mode the calling thread waits up to the configured
     *         timeout, may be {@code null} if nothing was parsed
     */
     ...
     * return the result of parsing
     *
     * @param timeout the max seconds to wait for the async reading thread
     * @return the parsed result, may be {@code null} if nothing was parsed or the reading is
     *         still in progress
     */
    ...
    /**
     * Set whether multiple JSONs can be read from the stream.
     *
     * @param multiple {@code true} to keep the stream open and read multiple JSONs
     * @return the current reader, for chained calls
     */
    ...
    /**
     * Set the read options, unlike {@link #setOptions(ReadOption...)} the options are only stored
     * and are not applied to the parse context immediately.
     *
     * @param readOptions the read options to use
     * @return the current reader, for chained calls
     */
