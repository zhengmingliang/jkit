...
import com.alianga.jkit.exception.ParserException;
...
public final class StringTemplate {
    ...
    /**
     * 解析模板文本并生成对应的 java 源码，随后编译为模板类。
     *
     * @param template 模板内容，支持 {@code $if}、{@code $for}、{@code ${k}} 等语法
     */
...
//        importPackages.append("import com.alianga.jkit.template.TemplateClass;\r\n");
//        importPackages.append("import com.alianga.jkit.json.internal.utils.*;\r\n");
                ...
                if (javaCodeBegin) {
                    ...
                    if (isTextLine) {
                        if (line.matches(PLACEHOLDER_REGEX)) {
                            ...
                            line = line.replace("\"", "\\\"");
                            appendIndent(source, scopeLevel);
                            appendPlaceHolderLine(source, line, userLocalContext ? "localContext" : "context",
                                    scopeLevel);
//                            if (userLocalContext) {
...
//                             source.append("\t\tprintln(\"" + line.replace("\"", "\\\"") + "\");\r\n");
        ...
        try {
            templateClass = (TemplateClass) JDKCompiler.compileJavaSource(
                    new JavaSourceObject(PACKAGE_NAME, simpleClassName, javaCode)).newInstance();
        } catch (Throwable e) {
...
//            appendIndent(source, scopeLevel);
     ...
     * 返回实例id
     *
     * @return 当前模板实例的唯一 id，创建时生成的 UUID
     */
