...
public class ExprParser extends Expression {
    ...
    // 空解析器
    /**
     * 构造一个不解析任何内容的空解析器，由子类自行调用 init 与 parse 完成初始化。
     */
    ...
    /**
     * token 分组：值类 token（数字、字符串、变量、集合、函数）
     */
    public static final int GROUP_TOKEN_VALUE = 2;

    /**
     * token 类型：初始（重置）状态，表示尚未读取到有效 token
     */
    ...
    /**
     * 末级（叶子）变量的链式调用器，变量只有一个时直接指向该变量的调用器
     */
    ...
    // global instance
    /**
     * 获取全局解析器实例，本类直接返回自身，子类可返回共享的全局实例。
     *
     * @return 当前解析器实例
     */
     ...
     * 获取根级变量调用器集合。
     *
     * @return 变量名到根级变量调用器的映射，表达式中不含变量时为 {@code null}
     */
     ...
     * 获取末级（叶子）变量调用器集合。
     *
     * @return 变量名到末级变量调用器的映射，表达式中不含变量时为 {@code null}
     */
     ...
     * 获取表达式的字符长度。
     *
     * @return 解析时使用的字符数（即表达式源码在缓冲区中的有效长度）
     */
    public int length() {
        return count;
    }
     ...
     * 获取查找模式下表达式的结束位置。
     *
     * @return 查找模式下返回表达式结束的下标，非查找模式返回 -1
     */
    ...
    /**
     * 使用字符数组的指定区间初始化解析器，数组会被直接引用而不复制。
     *
     * @param buf    表达式源码的字符数组
     * @param offset 起始下标
     * @param count  参与解析的字符数
     */
    ...
    private void checkEndTokenSyntaxError(int tokenType) {
        if (tokenType == OPS_TOKEN) {
            ...
            throw new ExpressionException(
                    "syntax error, pos: " + this.readIndex + ", unsupported operator ends, Expression[... " +
                            errorMessage + " ]");
        }
    }
    ...
    /**
     * 压缩变量访问模型：根据变量总数、去重后的变量数以及是否存在链式访问，
     * 选择最合适的上下文构建器实现（单变量复用、双变量兄弟节点复用、无复用、通用复用等），
     * 以减少求值时的变量查找开销。
     */
            ...
            if (variableCount == tailSize || !existChain) {
                ...
                if (tailSize == 1) {
                    evaluatorContextBuilder =
                            new EvaluatorContextBuilder.SingleReusableRootImpl((ElVariableInvoker) tailChainInvoker);
                } else if (tailSize == 2) {
                    Iterator<ElVariableInvoker> iterator = invokes.values().iterator();
                    evaluatorContextBuilder =
                            new EvaluatorContextBuilder.SibbingTwinsRootReusableImpl(iterator.next(), iterator.next());
                } else {
                ...
                if (tailSize == 1) {
                    ...
                    if (tailSize == 2) {
                        ...
                        if (one.parent == next.parent) {
                            ...
                            evaluatorContextBuilder =
                                    new EvaluatorContextBuilder.SibbingTwinsReusableImpl(one.parent, one, next);
                        } else {
                            evaluatorContextBuilder = new EvaluatorContextBuilder.GeneralReusableImpl(
                                    ElChainVariableInvoker.buildTailChainInvoker(tailInvokes), variableSize);
                        }
                    } else {
                        ...
                        if (sibbingMode) {
                            // parent + children
                            evaluatorContextBuilder = new EvaluatorContextBuilder.SibbingReusableImpl(tailParent,
                                    ElChainVariableInvoker.buildTailChainInvoker(tailInvokes), variableSize);
                        } else {
                            // default
                            evaluatorContextBuilder = new EvaluatorContextBuilder.GeneralReusableImpl(
    ...
    /**
     * 获取表达式中所有末级（叶子）变量的调用器。
     *
     * @return 末级变量调用器列表的副本，表达式中不含变量时返回空列表
     */
    public final Collection<ElVariableInvoker> getTailVariableInvokers() {
        return tailInvokes == null ? new ArrayList<ElVariableInvoker>() :
                new ArrayList<ElVariableInvoker>(tailInvokes.values());
    }
    ...
    final void parseBracketToken(ExprParserContext exprParserContext) {
        ...
        if (!bracketParserContext.bracketEndFlag) {
            String errorMessage = createErrorContextText(sourceChars, this.readIndex);
            throw new ExpressionException(
                    "syntax error, pos: " + this.readIndex + ", missing closing symbol ')', Expression[... " +
                            errorMessage + " ]");
        }
        //  置换子表达式优先级
    ...
    final void parseQuestionToken(ExprParserContext exprParserContext) {
        ...
        if (!questionParserContext.questionEndFlag) {
            String errorMessage = createErrorContextText(sourceChars, this.readIndex);
            throw new ExpressionException(
                    "syntax error, pos: " + this.readIndex + ", missing symbol ':'" + ", Expression[... " +
                            errorMessage + " ]");
        }
    ...
    List<Object> parseList(int startChar, int endChar, int begin) {
        ...
        if (currentChar != endChar) {
            String errorMessage = createErrorContextText(sourceChars, this.readIndex);
            throw new ExpressionException("syntax error, pos: " + this.readIndex + ", Expression[... " + errorMessage +
                    " ],未找到与开始字符'" + startChar + "'相匹配的结束字符 '" + endChar + "'");
        }
        String childEl = getString(beginIndex, readIndex - beginIndex).trim();
        if (childEl.isEmpty()) {
        ...
        if ((NumberUtils.isDigit(currentChar) || (isMinusSymbol = currentChar == '-')) && prevTokenType < NUM_TOKEN) {
            ...
            if (numberRadix == 10) {
                ...
                if (currentChar == 'E' || currentChar == 'e') {
                    ...
                    try {
                        ...
                        String errorMessage = createErrorContextText(sourceChars, this.readIndex);
                        throw new ExpressionException(
                                "syntax error, pos: " + this.readIndex + ", unexpected token '" + currentChar +
                ...
                if (mode == 0 && !overflow) {
                    ...
                    do {
                        if (overflow) {
                            ...
                            cnt = 0;
                            int j = begin;
                            int decimalPointIndex = this.readIndex;
                            char c;
                            decimalCount = 0;
            ...
            if (readable()
                ...
                if (/*opsSymbolIndex == 43 ||*/ elOperator == ElOperator.EQ) {
                    ...
                    throw new ExpressionException("syntax error, pos: " + this.readIndex +
                            ", '=' is not supported, please use '==' instead, Expression[" + readSource + "]");
                }
            }
                ...
                try {
                    for (; ; ) {
                        ...
                        if (currentChar == '.') {
                            ...
                            variableKeys.add(ExprParserContext.getString(sourceChars, localOffset + this.offset,
                                    readIndex - localOffset));
                        ...
                        if (currentChar == '[') {
                            ...
                            while (true) {
                                ...
                                if (currentChar != ']') {
                                    String errorMessage = createErrorContextText(sourceChars, this.readIndex);
                                    throw new ExpressionException("syntax error, pos: " + this.readIndex +
                                            ", missing closing symbol ']', Expression[... " + errorMessage + " ]");
                                }
                                ...
                                if (stringKey) {
                                    ...
                                    System.arraycopy(sourceChars, localOffset + this.offset, chars, 1,
                                            chars.length - 2);
                                ...
                                if (readable()) {
                                    if ((currentChar = read()) == '.') {
                                        ...
                                        handleParseFunctionToken(identifierValue, false, variableKeys,
                                                exprParserContext);
                                        return;
                                    }
                                }
                    ...
                    String identifierValue = ExprParserContext.getString(sourceChars, localOffset + this.offset,
                            readIndex - localOffset);
            ...
            if (readable()) {
                ...
                if (!Character.isJavaIdentifierStart(currentChar = read())) {
                    String errorMessage = createErrorContextText(sourceChars, this.readIndex);
                    throw new ExpressionException(
                            "syntax error, pos: " + this.readIndex + ", Expression[... " + errorMessage +
                                    " ], unexpected function start char  : '" + currentChar + "'");
                }
            }
            // 查找'('前面的字符
            ...
            if (currentChar != '(') {
                ...
                throw new ExpressionException("syntax error, pos: " + this.readIndex + ", source: '" + readSource +
                        "' function start symbol '(' not found !");
            }
                ...
                while (true) {
                    if (isIdentifierStart(currentChar = read())) {
                        ...
                        if (currentChar == '(') {
                            ...
                            exprParserContext.exprEvaluator.left =
                                    new ExprEvaluator.MemberImpl(exprParserContext.exprEvaluator.left, true, member,
                                            params);
                            ++this.readIndex;
                            return;
                        } else {
                            exprParserContext.exprEvaluator.left =
                                    new ExprEvaluator.MemberImpl(exprParserContext.exprEvaluator.left, false, member,
                                            null);
                            if (currentChar == '.') {
    ...
    /**
     * 将表达式源码指定区间的字符解析为 BigDecimal。
     *
     * @param beginIndex 数字起始下标（相对于表达式起点）
     * @param endIndex   数字结束下标（不包含）
     * @param negate     是否取负
     * @return 解析得到的 BigDecimal，negate 为 {@code true} 时返回其相反数
     */
    ...
    private void handleParseFunctionToken(String identifierValue, boolean oneLevelAccess, List<String> variableKeys,
                                          ExprParserContext exprParserContext) {
...
//    }
    ...
    private void checkValueTokenSyntaxError() {
        if (this.prevTokenType == BRACKET_END_TOKEN) {
            String readSource = createErrorContextText(sourceChars, this.readIndex);
            throw new ExpressionException("syntax error, pos: " + this.readIndex +
                    ", value cannot appear after the closing bracket, Expression[" + readSource + "]");
        }
    }
    ...
    private void checkOpsTokenSyntaxError() {
        if (this.prevTokenType == BRACKET_TOKEN) {
            String readSource = createErrorContextText(sourceChars, this.readIndex);
            throw new ExpressionException("syntax error, pos: " + this.readIndex +
                    ", unsupported operation symbols follow '(' , Expression[" + readSource + "]");
        }
    }
    ...
    private void checkTokenSyntaxError() {
        ...
        if (preGroupValue == groupValue) {
            if (preGroupValue == GROUP_TOKEN_OPS) {
                String readSource = createErrorContextText(sourceChars, this.readIndex);
                throw new ExpressionException(
                        "syntax error, pos: " + this.readIndex + ", duplicate token operation, Expression[" +
                                readSource + "]");
            } else if (preGroupValue == GROUP_TOKEN_VALUE) {
                String readSource = createErrorContextText(sourceChars, this.readIndex);
                throw new ExpressionException(
                        "syntax error, pos: " + this.readIndex + ", missing operation symbol, Expression[" +
                                readSource + "]");
            }
        } else {
    ...
    private void checkBeforeBracketEndTokenSyntaxError() {
        ...
        if (this.prevTokenType == BRACKET_TOKEN || preGroupValue == GROUP_TOKEN_OPS) {
            String readSource = createErrorContextText(sourceChars, --this.readIndex);
            throw new ExpressionException(
                    "syntax error, pos: " + this.readIndex + ", unexpected symbol : ')', Expression[" + readSource +
                            "]");
        }
    }
    ...
    private void throwUnsupportedError() {
        String readSource = createErrorContextText(sourceChars, this.readIndex);
        throw new ExpressionException(
                "syntax error, pos: " + this.readIndex + ", unexpected symbol '" + this.errorMsg + "', Expression[" +
                        readSource + "]");
    }
    ...
    static final int digit(char c, int radix) {
        switch (c) {
            case '0':
            case '1':
            case '2':
    ...
    public Object evaluate(Object context) {
        ...
        // evaluate(context, EvaluateEnvironment.DEFAULT);
    }
    ...
    public final Object evaluate(Map context, EvaluateEnvironment evaluateEnvironment) {
        if (variableSize == 0) {
            ...
            if (evaluateEnvironment.computable) {
                return doEvaluate(
                        evaluatorContextBuilder.createEvaluatorContext(evaluateEnvironment.computedVariables(context)),
                        evaluateEnvironment);
            }
    ...
    public final Object evaluate(EvaluateEnvironment evaluateEnvironment) {
        ...
        if (evaluateEnvironment.isMapContext()) {
            return doEvaluate(
                    evaluatorContextBuilder.createEvaluatorContext((Map) evaluateEnvironment.computedVariables()),
                    evaluateEnvironment);
        } else {
            return doEvaluate(evaluatorContextBuilder.createEvaluatorContext(evaluateEnvironment.computedVariables()),
                    evaluateEnvironment);
        }
    }
    ...
    /**
     * 压缩执行模型：将常量节点折叠为常量执行器、去掉无运算的中间节点，并标记为已压缩，
     * 多次求值同一表达式时只会执行一次。
     */
...
//                case QUESTION:
            ...
            // 三目运算
            return ExprEvaluator.TernaryImpl.of(
                    exprEvaluator.update(compressEvaluator(left), compressEvaluator(right)));
    ...
    /**
     * 构造语法错误提示用的上下文文本：截取出错位置前后各约 49 个字符，并在出错位置插入 {@code ^} 标记。
     *
     * @param buf       表达式源码的字符数组
     * @param readIndex 出错位置（相对于表达式起点的下标）
