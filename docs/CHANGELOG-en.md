...
- **Parsing coverage**: relative to Druid BVT / JSqlParser inline / file corpora approximately **96.3% / 92.3% / 90.0%**. Window functions, CTE, MERGE, PIVOT, flashback, procedure blocks, Hive / ClickHouse / Informix / DB2 edge-case syntax and more are documented in [`docs/sql.md`](sql.md) rather than listed per iteration.
...
- New module `com.alianga:jkit-notify`: DingTalk (with signing), WeCom, Feishu (with signature), ServerChan, Bark, generic Webhook, SMTP (raw socket: AUTH LOGIN + STARTTLS/SSL + MIME). `NotificationChannel` SPI + `NotificationManager`; `MessageType` (TEXT / MARKDOWN / HTML). Zero third-party dependencies. Usage: see [`docs/notify.md`](notify.md).
