...
- INSERT / REPLACE：列清单、VALUES 多行、INSERT SELECT、INSERT SET、ON DUPLICATE KEY UPDATE、PG `ON CONFLICT`（`DO NOTHING` / `DO UPDATE` / `ON CONSTRAINT`）、`RETURNING`（`*` 或多列列表）、SQL Server `OUTPUT` / `OUTPUT … INTO`、Oracle `INSERT ALL` / `INSERT FIRST`；MySQL `LOW_PRIORITY` / `DELAYED` / `HIGH_PRIORITY` / `IGNORE` 修饰符保留并回写；Hive `INSERT OVERWRITE [TABLE] t [PARTITION (...)] SELECT …`（`SqlInsert.overwrite`/`tableKeyword`/`partitionRaw`）
