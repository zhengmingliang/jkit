...
- SELECT：列、`*`、`t.*`、DISTINCT / DISTINCTROW / DISTINCT ON、HIGH_PRIORITY / STRAIGHT_JOIN 修饰符 / SQL_SMALL_RESULT / SQL_BIG_RESULT / SQL_BUFFER_RESULT / SQL_CACHE / SQL_NO_CACHE / SQL_CALC_FOUND_ROWS、TOP、`INTO` 表 / `@var` / 多变量 `INTO c,d` / `INTO (c,d)` / `OUTFILE`（抽目标表进 `tables`/`INSERT`）、ODPS `FORCE PARTITION / `SIGNED INTEGER` / `IGNORE NULLS` / `LIMIT BY` / jsonb `?` 'pt'` / `FORCE ALL PARTITIONS`、FROM（含 MySQL `PARTITION (p0,p1)` 表分区限定；别名后亦可 `FORCE/USE/IGNORE INDEX`）、JOIN（INNER/LEFT/RIGHT/FULL/CROSS/STRAIGHT/逗号；`LEFT|RIGHT ANTI|SEMI JOIN`；`NATURAL` 与连接类型**正交**）、SQL Server 表提示 `WITH (NOLOCK)` / `WITH (INDEX(ix))`（别名前后均可，原文保留）、`CROSS APPLY` / `OUTER APPLY`、`LATERAL` 子查询/表函数、`UNNEST(...) [WITH ORDINALITY]` / `TABLE(fn(...))` / `OPENJSON(...) WITH (...)` 表函数、`PIVOT` / `UNPIVOT [INCLUDE|EXCLUDE NULLS]`、`(VALUES …) AS v(cols)`、括号集合运算子查询 `((SELECT…) UNION …)`、ON/USING、WHERE、GROUP BY [WITH ROLLUP|WITH CUBE|DISTINCT|GROUPING SETS]、HAVING、Teradata/Snowflake `QUALIFY` 窗口过滤、`WINDOW … AS (…)`（可继承另一窗口名）、ORDER BY、LIMIT/OFFSET/`FETCH FIRST n ROWS ONLY`、FOR UPDATE [OF cols] [NOWAIT|SKIP LOCKED]、LOCK IN SHARE MODE、UNION/UNION ALL/INTERSECT/EXCEPT/MINUS、CONNECT BY [NOCYCLE] / START WITH / PRIOR / `CONNECT_BY_ROOT`、WITH CTE；Hive UDTF 多列别名 `fn(...) AS (c0,c1)`
...
- INSERT / REPLACE：列清单、VALUES 多行、INSERT SELECT、`INSERT … (WITH … SELECT …)`、INSERT SET、ON DUPLICATE KEY UPDATE、PG `ON CONFLICT`（`DO NOTHING` / `DO UPDATE` / `ON CONSTRAINT`）、`RETURNING`（`*` 或多列列表）、SQL Server `OUTPUT` / `OUTPUT … INTO`、Oracle `INSERT ALL` / `INSERT FIRST`；MySQL `LOW_PRIORITY` / `DELAYED` / `HIGH_PRIORITY` / `IGNORE` 修饰符保留并回写；Hive `INSERT OVERWRITE [TABLE] t [PARTITION (...)] SELECT …`（`SqlInsert.overwrite`/`tableKeyword`/`partitionRaw`）；ODPS `UPDATE/DELETE FORCE PARTITION …`
