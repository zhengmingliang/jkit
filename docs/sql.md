...
明确未做：窗口函数全集（如 WINDOW 继承另一窗口名）、存储过程/包体、ALTER CHANGE/CONSTRAINT 全量建模、WITHIN GROUP 结构化、TABLE/UNNEST、执行引擎、SQL 防火墙规则集。未知函数按普通函数调用解析，不失败。
