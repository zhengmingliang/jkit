...
- `SQL.bind` / `SQL.bindNamed`：把 `?` / `:name` 换成字面量或公式，并识别模板占位 IDENT（`@name@` / `#{table}` / `${*}` / <code v-pre>{{*}}</code> / `<*>` 等）。表名位置写成标识符（必要时加方言引号，防注入）；表达式位置与 `:name` 相同。字符串只加倍单引号；公式传 `SqlExpr`。`IN ?` 可填集合。布尔：Oracle / 达梦 / SQL Server / SQLite / DB2 / MySQL / Hive 写 `1`/`0`；PG / H2 / ANSI / Presto / ClickHouse 写 `TRUE`/`FALSE`。未传命名值时不扫描标识符。
...
- `docs/html.md` / `docs/en/html.md`：随 HTML 修复与内存反超全面刷新——§4 补 `:not()` 选择器列表与含组合器的复杂选择器示例、`Elements.select` 的去重语义；§5 补超 BMP 数字实体生成代理对；§6 补 `<!-->` / `<!--->` 空注释与 `<html>` 属性保留；§7 差异清单移除「`:not()` 内嵌组合符」（已支持）；§8 补齐畸形选择器解析期拒绝的完整清单；§9 数字按最新 tools-test 报告刷新（JDK 17），常驻内存结论由「高 13%」改为「低 15%」，优化清单由五处扩到六处（新增解析期字符串驻留缓存）。
- `docs/resilience.md` / `docs/en/resilience.md`：`HALF_OPEN` 状态说明改为「同一时刻只放行一个探测请求，未达成功阈值会继续放行下一个探测」。
- `docs/json.md` / `docs/en/json.md`：JSONPatch 小节补数组下标的 RFC 6901 严格规则（`0` 之外无前导零、不允许 `+`）与 `test` 的 `BigDecimal` 精确比较语义（大整数不再被 `double` 截断成假相等）。
- `docs/toolkit.md` / `docs/en/toolkit.md`：ULID 小节补 `parse` 的 48 位时间戳上限校验与往返保证。
- `docs/http.md` / `docs/en/http.md`：实例作用域改为「保存并恢复进入前的旧值」，补拦截器嵌套调用其它实例后外层配置不变的语义。
...
- `jkit-core`：`MultiKeyHashMap` 的读写两侧语义此前自相矛盾。类声明「获取值时可忽略 key 大小写」，但只有 `put` 登记了内部 `keyMap`，其它入口都没跟上：`putAll` 走父类实现，批量写入的条目**完全无法被忽略大小写地读到**（实测 `putAll({"UserName":"tom"})` 后 `get("username")` 返回 `null`，而逐条 `put` 能取到值）；`remove` 只按精确 key 删除，`put("UserName")` 之后 `remove("username")` 静默失败——`size()` 不变、`get("username")` 仍返回原值；`remove` / `clear` 都不清理 `keyMap`，已删除的 key 被长期持有无法回收；`containsKey` 同样只认精确 key，与 `get` 给出相反答案。现在 `putAll` 逐条走 `put` 完成登记，`remove` 支持忽略大小写删除并同步失效登记，`clear` 一并清空 `keyMap`，`containsKey` 与 `get` 语义对齐。
- `jkit-core`：`MultiKeyHashMap` 的忽略大小写匹配改用 `toLowerCase(Locale.ROOT)`。原来的无参 `toLowerCase()` 依赖运行时语言环境，在土耳其语（`tr-TR`）环境下 `"ID"` 会被转成 `"ıd"`（无点 i），写入侧与读取侧的规范化结果对不上，忽略大小写查找整体失效——实测 `put("ID", …)` 后 `get("id")` 返回 `null`、`containsKey("id")` 为 `false`。上游 wast 亦有 14 处同类无 Locale 写法（集中在 HTTP 头与协议名处理），不在本库范围内。
- `jkit-core`：`Maps.getNumber`（及其派生的 `getInteger` / `getLong` / `getDouble` / `getFloat` / `getShort` / `getByte`）不再把半截字符串当成数字。`NumberFormat#parse(String)` 只做前缀解析，此前 `"123abc"` 返回 `123`、`"2024-01"` 返回 `2024`、`"1.2.3"` 返回 `1.2`——格式错误的数据被静默转成合法数值继续往下传，且这种错误在 JDK 8 / 11 / 17 / 21 / 25 上表现完全一致，断言数值的调用方无从察觉。现在先 `trim` 再用 `ParsePosition` 校验整串被完整消费，未完整解析的一律返回 `null`（走 `defaultValue` 重载时回落默认值）。副作用是带首尾空白的 `" 42 "` 由原来的 `null` 变为 `42`，属行为变更。
- `jkit-core`：`com.alianga.jkit.collection` 补齐首轮测试（此前零测试）。新增 `MultiKeyHashMapTest`（12 例）、`MapsNumberParseTest`（9 例）、`CollectionsTest`（13 例，覆盖空判断、`getFirst` / `getLast`、按对象与谓词删除、查找与转换、`size` 的 Map / 数组 / Iterator / Enumeration / 原始类型数组分支、`contains` 与 `findValueOfType`）。回退修复后 9 例失败，全部落在上述修复点上。另修正 `Collections.size(Object)` 的 javadoc——`null` 实际返回 `0`，并非声明的抛 `IllegalArgumentException`；`CountMap` 补充 `get` 与继承来的 `containsKey` / `getOrDefault` 的语义差异说明，判断「是否发生过计数」应统一用 `containsKey`。
- `jkit-core`：`com.alianga.jkit.html` 常驻内存反超 Jsoup 1.18.1（150 份 300 KB 页面 DOM：203.5 MB → 153.3 MB，由高出 13% 变为**低 15%**）。新增解析期字符串驻留缓存 `StringCache`（开放寻址哈希表，容量与装载率有上限、超过 128 字符的长文本跳过），把重复的文本节点内容与属性值合并为同一实例——真实页面里 class 值、模板化列表文本高度重复，此前逐节点各建一份，字符串 + 字节数组合计比 Jsoup 多出 324 KB/DOM，是内存对比吃亏的全部来源（Jsoup 1.18 同样内置字符串缓存）。堆直方图逐类对比定位：集合、属性结构两侧 jkit 本就更省，只有字符串一项落后。解析开销约 5%（大页面 ~0.06 ms），选择器与端到端领先幅度不变；差分验证保持 112 组一致 / 7 组已知差异。
- `jkit-core`：HTML 选择器（`Selector`）对畸形输入全面收紧并修复崩溃。空选择器此前抛 `IndexOutOfBoundsException`，`",p"` / `"p,,p"` 这类残缺分组会因空复合对任何元素都返回 true 而**静默匹配整个文档（含 `#document` 根）**，`"p >"` 的末尾悬空组合符被静默忽略——现在空选择器、分组缺操作数、组合符缺右操作数一律抛 `SelectorException`。属性选择器同步收紧：`img[src`（未闭合 `[`）此前抛 `StringIndexOutOfBoundsException`，`[src^png]` / `[src!]`（操作符后缺 `=`）此前被静默丢弃操作符、退化成存在性匹配，现均报错。`:not()` 补齐完整语义：支持选择器列表（`:not(a,p)`，此前静默丢弃第二个之后的参数）与含组合器的复杂选择器（`:not(div p)`）；伪类参数括号未闭合（`p:nth-child(23`）此前被截成 `nth-child(2)` 返回错误结果，现报错；不支持的伪类（如 `table:hover`）改在**解析期**报错，不再因文档无候选元素而静默返回空结果；`:nth-child(abc)` 抛 `SelectorException` 而非 `NumberFormatException`。标签名归一补 `Locale.ROOT`。
- `jkit-core`：HTML 树操作递归改迭代，防恶意深嵌套输入。解析主循环本是迭代实现，但 `text()` / `outerHtml()` / `nodeText()` / 选择器收集与 `Document` 索引构建都是递归——20000 层 `<div>` 能解析成功，随后调用任何读取 API 就 `StackOverflowError`。上述遍历全部改为显式栈迭代，产出语义（文档顺序、块级元素补空格、索引文档序）不变。
- `jkit-core`：HTML 解析与序列化修三处。`<!-->` / `<!--->` 是 HTML5 定义的完整空注释（abrupt-closing-of-empty-comment），此前被当作未闭合注释把**剩余整个文档吞进注释**；`&#128512;` 这类高于 BMP 的数字实体此前被 `(char)` 强转截断成 U+F600 乱码（现用 `Character.toChars` 生成代理对）；`<html lang="en">` 标签上的属性此前被静默丢弃（`head` / `body` 都保留，唯独 `html` 丢）。
- `jkit-core`：选择器与 `Elements` 两处性能 / 一致性。分组选择器（`"p, .c"`）的去重由结果集线性 `contains`（O(n²)）改为 `HashSet` 标记；`Elements.select` 落实文档声明的「合并去重」——嵌套 `div` 下 `doc.select("div").select("p")` 此前返回 2 个重复 `p`，现与 jsoup 一致只保留一次。
- `jkit-core`：表达式求值器上一轮修复只落在通用求值路径，压缩后的 Impl 快速路径没跟上——解析后的表达式**复用求值第 3 次起**（`ExprParser` 会在第 2 次求值后把表达式树压缩为 `Impl` 类）行为突变：`flag && (1/0)` 此前短路返回 `false`，第 3 次起抛除零异常；字符串 `"a" < "b"` 第 3 次起抛裸 `ClassCastException`。现在 `LogicalAndImpl` / `LogicalOrImpl` 与通用路径一致短路求值并对齐异常包装，`Gt` / `Lt` / `GE` / `LE` 四个 Impl 改用 `compareRelational`；`ExpressionRegressionTest` 新增「同一表达式复用求值 ≥3 次」用例堵住回归逃逸（此前的回归测试每条表达式只求值一次，恰好绕开压缩路径）。
- `jkit-core`：`JSONPatch` 的 `test` 操作数值比较此前走 `doubleValue()`，超过 2^53 的整数被截断产生**假相等**——`9007199254740993` 与 `9007199254740992` 判等，两个相差 1 的 30 位 `BigInteger` 也判等（`toBigDecimal` 对 `BigInteger` 还经 `longValue()` 截断）；`test` 是乐观锁原语，假相等会在前置条件不成立时误放行补丁。现在统一走 `BigDecimal` 精确比较，`1` 与 `1.0` 语义相等保留。另按 RFC 6901 收紧数组索引解析：`/a/01`（前导零）、`/a/+1` 此前被 `Integer.parseInt` 宽松放行，现报 `JSONPatchException`。
- `jkit-core`：`CircuitBreaker` 两处并发缺陷。状态转换此前基于进入任务前的**陈旧快照**做无条件 `set()`——半开下 T1 / T2 并发探测，T2 失败置 OPEN 后 T1 成功仍会把熔断覆盖回 CLOSED 并清零失败计数（反向同理），与「多线程下安全使用」的类文档不符；半开状态下**并发探测不限流**，冷却结束瞬间放行的是全量流量而非「少量探测」。现在状态转换全部改 CAS 循环（`openedAt` 先于 CAS 写入，避免并发半开转移读到旧时间戳），半开用探测许可保证同一时刻只放行一个探测，成功阈值 >1 时未达阈值续发许可继续探测；任务抛出的 `Error`（OOM 等）保持原类型抛出，与 `Retryer` 语义一致。
- `jkit-core`：`HttpClient` 的实例作用域此前是「执行后清空」而非「恢复旧值」——拦截器（如 token 刷新）里嵌套调用另一个客户端实例后，内层返回时把 ThreadLocal 清成 null，**外层剩余链路（代理 / CookieJar / 超时 / 引擎）静默回退到全局配置**且无任何报错。现在保存进入前的作用域并在 finally 恢复；原「执行后清空」测试固化了错误语义，已改为恢复语义并补嵌套场景回归。
- `jkit-core`：`MultiKeyHashMap` 的读写一致性修复只完成一半。`remove` 无条件清除小写别名——`put("A",1); put("a",2); remove("A")` 后幸存的 `"a"` 条目忽略大小写查找失效（`get("A")` 返回 null，比修复前还回退），现在删除后按现存条目重建别名；`putIfAbsent` / `compute` / `computeIfAbsent` / `computeIfPresent` / `merge` 未覆写、绕过 `put` 的 keyMap 登记，忽略大小写查找对这些入口静默失效，现全部补齐；`remove(null)` 此前因 `String.valueOf(null)` 塌缩成 `"null"` **误删键为字面量 `"null"` 的条目**，现在 null key 不参与忽略大小写匹配（`get(null)` / `containsKey(null)` 也不再误命中 `"null"` 条目）。类 javadoc 补充视图迭代删除不维护 keyMap 的限制说明。
- `jkit-core`：`JwtUtils` 加固。HS256 共享密钥不足 32 字节（256 位）时签发与验签均抛 `IllegalArgumentException`（低熵密钥签出的令牌可被离线爆破，与类文档「建议不小于 32 字节」对齐）；`verify` 新增 `leewaySeconds` 重载，exp / nbf 校验可配置时钟偏移余量（默认 0，行为不变）；分段解析改用 `split("\\.", -1)`，`"h.p.sig.."` 这类多余分段不再被默认 split 吞掉尾部空串后按 3 段接受；签名段的 base64url 解码异常包装为 `JwtException`（此前裸抛 `IllegalArgumentException`，破坏 `@throws JwtException` 契约）。
- `jkit-core`：`EncryptUtils.RSA` 的 String 版 `encryptOaep` / `decryptOaep` 不再吞异常返回空串——解密失败（密钥错、密文被篡改的 `BadPaddingException`）与「明文本身为空」此前不可区分，调用方不检查就把空串当解密结果落库属静默数据损坏；现改为向上抛出（签名补 `throws Exception`，与同组 `byte[]` 变体一致），顺带修掉日志里永不生效的 `%s` 占位符。
- `jkit-core`：`DesensitizeUtils` 单字符输入不再原样返回——`mask` 在保留位数覆盖全文时输出「首字符 + 掩码」，长度为 1 时退化为原字符串，单字姓名（`name("王")`）整条敏感信息原样漏出；现在单字符整体打码。
- `jkit-core`：`ULID` 三处。单调模式 80 位随机位溢出时此前随机部分归零而时间戳不动，产生一个字典序**小于**前驱的 ID，违反「单 JVM 严格递增」的文档承诺（同批 `UUIDv7` 是进位到下一毫秒的），现对齐为溢出进位；`parse` 补 48 位时间戳上限校验（此前 `parse("ZZZ…")` 放行而 `encode` 拒绝，`parse(x).toString()` 往返不自洽），`isUlid` 同步覆盖；`toBytes` 去掉同一字符串的重复解码。
- `jkit-core`：`Retryer` 对中断的语义。任务自身抛出的 `InterruptedException` 此前被当普通失败重试且不清中断标志——多烧一次尝试、吞掉取消信号，现在恢复中断标志并立即终止重试；睡眠被打断时，触发重试的最后一次业务异常不再被丢弃（以 suppressed 附加在 `ResilienceException` 上）。
- `jkit-core`：HTTP 实例化收尾。`UrlConnectionHttpEngine` 写请求体时的拷贝缓冲改走 active-aware 的 `HttpUtils.config()`（此前固定读全局 `HttpConfig.shared()`，实例级 `downloadBufferSize` 不生效）；`method` 字段的延迟查找缓存补 `volatile`；`HttpConfig.copy()` 的 javadoc 如实说明 `executor` / `endpointPool` / `sslContext` / `retryPolicy` 为共享引用（原「深拷贝」表述失实）；删除无调用点的 `HttpUtils.currentDefaultMediaType()`。
