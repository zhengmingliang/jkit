...
|------|------|--------------|---------|
| 小页面解析 | 0.013 ms | 0.016 ms | 1.28x |
| 中页面解析 | 0.177 ms | 0.192 ms | 1.09x |
| 大页面解析 | 1.775 ms | 1.834 ms | 1.03x |
| 大页面解析 + `text()` | 2.092 ms | 2.063 ms | 0.99x |
| 中页面 `#main` | 0.007 ms | 0.010 ms | 1.60x |
| 中页面 `.post` | 0.008 ms | 0.014 ms | 1.65x |
| 中页面 `article.post h2` | 0.007 ms | 0.030 ms | 4.14x |
| 中页面 `a[href^=/p/]` | 0.008 ms | 0.015 ms | 1.92x |
| 大页面 `#main` | 0.112 ms | 0.102 ms | 0.91x |
| 大页面 `a[href^=/p/]` | 0.118 ms | 0.135 ms | 1.15x |
| 200 份大页面 DOM 常驻堆 | 268.0 MB | 241.5 MB | 0.90x |
| 端到端（解析 + 3 次查询 + 取文本） | 2.572 ms | 2.487 ms | 0.97x |

结论：

- **解析与选择器已追平或快于 Jsoup**，端到端吞吐基本持平（轮间波动约 10%，0.97x 到 1.05x 之间摆动）；
- **常驻内存仍高约 11%**——这是模型差异，`Element` 用并行数组存属性、`ArrayList` 存子节点，换来的是实现简单与零依赖，代价是每份 DOM 比 Jsoup 略胖；
- 大页面上的全量遍历选择器（如 `#main`）比 Jsoup 慢约 9%，因为本模块对每次查询都做完整深度优先遍历，没有建 id / class 索引。

优化手段集中在三处：属性表由 `LinkedHashMap` 改为并行 `String[]` 并懒创建、标签名与属性名走 `Names` 驻留表复用；解析期用下标缓存替代 O(栈深) 的栈内查找、属性写入复用缓冲区；选择器遍历去掉中间列表与迭代器、属性匹配合并为一次扫描、解析结果按访问序缓存（上限 256 条）。
