...
模块以 Jsoup 1.18.1 为对照做了差分验证，在 119 组「HTML + 选择器」用例上 112 组结果完全一致，剩余差异集中在 Jsoup 专有扩展与 HTML5 收养算法（见 [第 7 节](#7-与-jsoup-的差异)）。
...
doc.select("div :not(a, p)");        // :not 支持选择器列表
doc.select("div b:not(a b)");        // :not 支持含组合器的复杂选择器
doc.select("li:nth-child(2n+1)");    // 奇数项
doc.select("p:contains(详情)");       // 文本包含
doc.selectFirst("title");            // 首个匹配，无则 null
```

`Elements` 上的 `select(css)` 会在集合每个元素的子树中查询并**合并去重**（嵌套子树命中同一元素只保留一次）；`Elements.text()` 以空格拼接各元素文本。
...
- **隐式骨架**：任何输入都会补出 `html` / `head` / `body`。`head` 专用标签（`title` / `meta` / `link` / `base` / `style`）与 body 之前的 `script` 归入 `head`，其余进入 `body`；`<html lang="en">` 标签上的属性会保留；
...
- `<!-->` 与 `<!--->` 按 HTML5 视为完整的空注释（abrupt closing），不会把后续内容吞进注释；未闭合的注释按规范延伸到文档末尾。
...
非法选择器**一定抛异常，绝不静默降级或死循环**——畸形输入在解析期就被拒绝：空选择器 / 空白选择器、分组缺操作数（`,p` / `p,,p`）、组合符缺右操作数（`p >`）、属性选择器未闭合或缺 `=`（`img[src`、`[src^png]`）、伪类括号未闭合（`p:nth-child(2`）、不支持的伪类（`table:hover`，即使文档里没有 `table` 也报错）等都会立即得到明确的错误信息。

解析本身不抛异常——所有不规范输入都会被尽量适配为 DOM，不会因 HTML 脏数据中断流程。

## 9. 性能

同机与 Jsoup 1.18.1 实测对比（预热 3 轮、计时 7 轮取中位数，maxHeap 2 GB，JDK 17；Jsoup 仅用于基准测试，未进入项目依赖）。测试页面为拼接的文章列表页：小页面 2 条约 1.6 KB、中页面 60 条约 30 KB、大页面 600 条约 300 KB。倍数为 `Jsoup 耗时 / jkit 耗时`，大于 1 表示 jkit 更快。
...
|------|------|--------------|---------|
| 小页面解析 | 0.010 ms | 0.023 ms | 2.27x |
| 中页面解析 | 0.142 ms | 0.197 ms | 1.39x |
| 大页面解析 | 1.294 ms | 2.275 ms | 1.76x |
| 大页面解析 + `text()` | 1.654 ms | 2.022 ms | 1.22x |
| 中页面 `#main` | <0.001 ms | 0.013 ms | 68x |
| 中页面 `.post` | 0.001 ms | 0.019 ms | 14.8x |
| 中页面 `article.post h2` | 0.002 ms | 0.024 ms | 9.8x |
| 中页面 `a[href^=/p/]` | 0.002 ms | 0.018 ms | 8.8x |
| 中页面 `li:first-child` | 0.004 ms | 0.020 ms | 4.9x |
| 大页面 `#main` | <0.001 ms | 0.164 ms | 1344x |
| 大页面 `.post` | 0.015 ms | 0.185 ms | 12.7x |
| 大页面 `article.post h2` | 0.024 ms | 0.212 ms | 8.7x |
| 大页面 `a[href^=/p/]` | 0.016 ms | 0.160 ms | 10.2x |
| 150 份大页面 DOM 常驻堆（只解析） | **153.3 MB** | 180.2 MB | **1.18x** |
| 150 份大页面 DOM 常驻堆（解析 + 一次查询） | **159.1 MB** | 180.9 MB | **1.14x** |
| 端到端（解析 + 3 次查询 + 取文本） | 2.847 ms | 3.405 ms | 1.20x |
...
| 场景 | jkit | Jsoup 1.18.1 | jkit 快 |
|------|------|--------------|---------|
| 首页解析（70 KB） | 0.306 ms | 0.456 ms | 1.49x |
| 端到端：解析 + 抽 10 篇全部字段 | 0.360 ms | 0.465 ms | 1.29x |
...
- 解析快 **1.4x–2.3x**，端到端快 **1.2x–1.3x**，选择器在 id / class / 标签索引加持下快 **3.5x–1344x**；
- **常驻内存低 15%**（153.3 MB vs 180.2 MB），建索引后低 12%（159.1 MB vs 180.9 MB）。索引本身约占一份 DOM 的 4%，换来的是上面那个数量级差距；只解析不查询时完全不建。此前内存反而高 13%，差距全部来自字符串——真实页面里 class 值、模板化文本高度重复，Jsoup 1.18 内置了字符串缓存而本模块此前逐节点各建一份；补上解析期字符串驻留缓存后反超；
...
优化手段集中在六处：属性表由 `LinkedHashMap` 改为并行 `String[]` 并懒创建、标签名与属性名走 `Names` 驻留表复用；解析期用下标缓存替代 O(栈深) 的栈内查找、属性写入复用缓冲区；`script`/`style`/`textarea` 的结束标签查找去掉整串 `toLowerCase()`，改为 `regionMatches` 忽略大小写逐字符扫描；选择器遍历去掉中间列表与迭代器、属性匹配合并为一次扫描、解析结果按访问序缓存（上限 256 条）；`Document` 上的惰性 id / class / 标签索引，把「每次查询全树遍历」变成「取候选 + 校验」；以及解析期字符串驻留缓存，把重复的文本与属性值合并为同一实例（前五项让内存与 Jsoup 持平后仍略高，最后一项才完成反超）。

前五项加起来，选择器在合成页面上只是 1.3x–2.2x、内存仍比 Jsoup 高 13%；加上索引这一步，中页面才到 4.6x–80x、大页面 8.6x–749x；最后补上字符串驻留缓存，常驻内存反超 15%。索引之所以敢这么用，是因为本模块的 DOM 没有 setter、没有 `appendChild`，解析完就定型了，索引不需要任何失效逻辑。
