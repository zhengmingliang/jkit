...
/**
 * 常用 HTML 命名实体表（{@code &copy;}、{@code &mdash;} 等）。
 *
 * <p>只包含真实网页中出现频率最高的约 200 个实体，未收录的命名实体保持原样输出；
 * 数字实体 {@code &#nnn;} / {@code &#xhh;} 在 {@link Html#unescape} 中独立支持，不受本表限制。
 *
 * @author 郑明亮
 * @since 2.0.2
 */
final class Entities {
    private static final String TABLE =
            ...
            + "|iquest=191|times=215|divide=247|fnof=402"
            ...
            + "|frasl=8260|euro=8364|weierp=8472|image=8465|real=8476|trade=8482"
            ...
            + "|exist=8707|empty=8709|nabla=8711|isin=8712|notin=8713|ni=8715|prod=8719"
    ...
    static {
        for (String item : TABLE.split("\\|")) {
            int eq = item.indexOf('=');
            if (eq > 0) {
                MAP.put(item.substring(0, eq), String.valueOf((char) Integer.parseInt(item.substring(eq + 1))));
            }
        }
    }
    ...
    /**
     * 按实体名返回对应字符，未收录时 {@code null}。
     *
     * @param name 实体名，不含 {@code &} 与 {@code ;}
     * @return 对应字符
     */
