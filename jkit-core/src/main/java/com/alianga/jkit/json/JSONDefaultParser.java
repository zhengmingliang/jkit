...
final class JSONDefaultParser extends JSONGeneral {
    ...
    public Object parse(String json, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            ...
            if (bytes.length == json.length()) {
                return parseInternal(AsciiStringSource.of(json), bytes, 0, bytes.length, null,
                        JSONParseContext.of(readOptions));
            } else {
                char[] chars = json.toCharArray();
                return parseInternal(UTF16ByteArraySource.of(json), chars, 0, chars.length, null,
                        JSONParseContext.of(readOptions));
            }
        }
    ...
    public Object parse(char[] buf, ReadOption... readOptions) {
        if (EnvUtils.JDK_9_PLUS) {
            ...
            if (bytes.length == buf.length) {
                ...
                return parseInternal(UTF16ByteArraySource.of(json), buf, 0, buf.length, null,
                        JSONParseContext.of(readOptions));
            }
        }
    ...
    Map parseMap(String json, Class<? extends Map> mapCls, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            byte[] bytes = (byte[]) JSONMemoryHandle.getStringValue(json);
            if (bytes.length == json.length()) {
                return (Map) parseInternal(AsciiStringSource.of(json), bytes, 0, bytes.length,
                        createMapInstance(mapCls), JSONParseContext.of(readOptions));
            } else {
                char[] chars = json.toCharArray();
                return (Map) parseInternal(UTF16ByteArraySource.of(json), chars, 0, chars.length,
                        createMapInstance(mapCls), JSONParseContext.of(readOptions));
            }
        }
        char[] chars = (char[]) JSONMemoryHandle.getStringValue(json);
        return (Map) parseInternal(null, chars, 0, chars.length, createMapInstance(mapCls),
                JSONParseContext.of(readOptions));
    }
    ...
    Collection parseCollection(String json, Class<? extends Collection> listCls, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            byte[] bytes = (byte[]) JSONMemoryHandle.getStringValue(json);
            if (bytes.length == json.length()) {
                return (Collection) parseInternal(AsciiStringSource.of(json), bytes, 0, bytes.length,
                        createCollectionInstance(listCls), JSONParseContext.of(readOptions));
            } else {
                char[] chars = json.toCharArray();
                return (Collection) parseInternal(UTF16ByteArraySource.of(json), chars, 0, chars.length,
                        createCollectionInstance(listCls), JSONParseContext.of(readOptions));
            }
        }
        char[] chars = (char[]) JSONMemoryHandle.getStringValue(json);
        return (Collection) parseInternal(null, chars, 0, chars.length, createCollectionInstance(listCls),
                JSONParseContext.of(readOptions));
    }

    //    static Object parseInternal(CharSource source, char[] buf, int fromIndex, int toIndex, Object defaultValue,
    //    ReadOption... readOptions) {
    //        return parseInternal(source, buf, fromIndex, toIndex, defaultValue, JSONParseContext.of(readOptions));
    //    }

    Object parseInternal(CharSource source, char[] buf, int fromIndex, int toIndex, Object defaultValue,
        ...
        try {
            ...
            switch (beginChar) {
                case '{':
                    result = parseJSONObject(source, buf, fromIndex,
                            defaultValue == null ? parseContext.defaultMap() : (Map) defaultValue, parseContext);
                    break;
                case '[':
                    result = parseJSONArray(source, buf, fromIndex,
                            defaultValue == null ? parseContext.defaultList() : (Collection) defaultValue,
                            parseContext);
                    break;
                case '\'':
                case '"':
                    result =
                            stringDeserializer.deserializeString(source, buf, fromIndex, beginChar, null, parseContext);
                    break;
                default:
                    try {
                        switch (beginChar) {
                            ...
                            default: {
                                ...
                                result = JSONTypeDeserializer.NUMBER.deserialize(source, numBuf, 0,
                                        parseContext.useBigDecimalAsDefault ? GenericParameterizedType.BigDecimalType :
                                                GenericParameterizedType.AnyType, null, ',', parseContext);
                            }
                        }
                    } catch (Throwable throwable) {
                        throw throwable instanceof JSONException ? (JSONException) throwable :
                                new UnsupportedOperationException(
                                        "Unsupported for begin character with '" + beginChar + "'");
                    }
            }
            ...
            if (endIndex != toIndex - 1) {
                ...
                throw new JSONException(
                        "Syntax error, at pos " + endIndex + ", context text by '" + errorContextTextAt +
                                "', extra characters found, '" + new String(buf, endIndex + 1, wordNum) + " ...'");
            }
            return result;
        } catch (Exception ex) {
    ...
    Collection parseJSONArray(CharSource source, char[] buf, int fromIndex, Collection list,
        ...
        for (; ; ) {
            ...
            switch (ch) {
                ...
                case '[': {
                    // Multilayer collections are relatively rare and can easily cause waste of collection space. The
                    // specified initialization capacity is displayed as 2
                ...
                case 'f':
                    ...
                    break;
                default: {
                    value = JSONTypeDeserializer.NUMBER.deserialize(source, buf, i,
                            parseContext.useBigDecimalAsDefault ? GenericParameterizedType.BigDecimalType :
                                    GenericParameterizedType.AnyType, null, ']', parseContext);
                    i = parseContext.endIndex;
                    list.add(value);
            ...
            throw new JSONException(
                    "Syntax error, at pos " + i + ", context text by '" + errorContextTextAt + "', unexpected '" + ch +
                            "', expected ',' or ']'");
        }
    }

    Map parseJSONObject(CharSource source, char[] buf, int fromIndex, Map instance, JSONParseContext parseContext)
            throws Exception {
        int i = fromIndex;
        char ch;
        boolean empty = true;
        final boolean allowomment = parseContext.allowComment;
        final boolean disableCacheMapKey = parseContext.disableCacheMapKey;
        for (; ; ) {
            while ((ch = buf[++i]) <= ' ') {
                // skip white spaces
            }
            ...
            int fieldKeyFrom = i;
            if (ch == '"') {
                key = disableCacheMapKey ?
                        (String) stringDeserializer.deserializeString(source, buf, i, '"', null, parseContext) :
            ...
            if (ch == ':') {
                ...
                throw new JSONException(
                        "Syntax error, at pos " + i + ", unexpected '" + ch + "', token ':' is expected.");
            }
        }
    }
    ...
    // bytes #####################################################################################################
    Object parseInternal(CharSource source, byte[] bytes, int fromIndex, int toIndex, Object defaultValue,
                         ReadOption... readOptions) {
    ...
    Object parseInternal(CharSource source, byte[] bytes, int fromIndex, int toIndex, Object defaultValue,
        ...
        try {
            ...
            switch (beginByte) {
                case '{':
                    result = parseJSONObject(source, bytes, fromIndex,
                            defaultValue == null ? parseContext.defaultMap() : (Map) defaultValue, parseContext);
                    break;
                case '[':
                    result = parseJSONArray(source, bytes, fromIndex,
                            defaultValue == null ? parseContext.defaultList() : (Collection) defaultValue,
                            parseContext);
                    break;
                case '\'':
                case '"':
                    result = stringDeserializer.deserializeString(source, bytes, fromIndex, beginByte,
                    ...
                    try {
                        switch (beginByte) {
                            ...
                            default: {
                                ...
                                result = JSONTypeDeserializer.NUMBER.deserialize(source, numBuf, 0,
                                        parseContext.useBigDecimalAsDefault ? GenericParameterizedType.BigDecimalType :
                                                GenericParameterizedType.AnyType, null, (byte) ',', parseContext);
                                toIndex = numBuf.length - 1;
                            }
                        }
                    } catch (Throwable throwable) {
                        throw throwable instanceof JSONException ? (JSONException) throwable :
                                new UnsupportedOperationException(
                                        "Unsupported for begin character with '" + (char) beginByte + "'");
                    }
            }
            ...
            if (endIndex != toIndex - 1) {
                ...
                throw new JSONException(
                        "Syntax error, at pos " + endIndex + ", context text by '" + errorContextTextAt +
                                "', extra characters found, '" + new String(bytes, endIndex + 1, wordNum) + " ...'");
            }
            return result;
        } catch (Exception ex) {
    ...
    Collection parseJSONArray(CharSource source, byte[] bytes, int fromIndex, Collection list,
                              JSONParseContext parseContext) throws Exception {
        byte b;
        int i = fromIndex;
        for (; ; ) {
//            while ((b = bytes[++i]) <= ' ') ;
            ...
            switch (b) {
                ...
                case '\'':
                case '"': {
                    value = stringDeserializer.deserializeString(source, bytes, i, b,
                            GenericParameterizedType.StringType, parseContext);
                    list.add(value);
                    i = parseContext.endIndex;
                    break;
                ...
                case 'f':
                    ...
                    break;
                default: {
                    value = JSONTypeDeserializer.NUMBER.deserialize(source, bytes, i,
                            parseContext.useBigDecimalAsDefault ? GenericParameterizedType.BigDecimalType :
                                    GenericParameterizedType.AnyType, null, END_ARRAY, parseContext);
                    i = parseContext.endIndex;
                    list.add(value);
                    byte next = bytes[++i];
                    if (next == END_ARRAY) {
            ...
            throw new JSONException(
                    "Syntax error, at pos " + i + ", context text by '" + errorContextTextAt + "', unexpected '" + b +
                            "', expected ',' or ']'");
        }
    }

    Map parseJSONObject(CharSource source, byte[] bytes, int fromIndex, Map instance, JSONParseContext parseContext)
            throws Exception {
        byte b;
        boolean empty = true;
        boolean allowomment = parseContext.allowComment;
        boolean disableCacheMapKey =
                        parseContext.disableCacheMapKey;
        int i = fromIndex;
        for (; ; ) {
            if ((b = bytes[++i]) <= ' ') {
                b = bytes[i = skipWhiteSpaces(bytes, i + 1)];
            ...
            int fieldKeyFrom = i;
            if (b == '"') {
                key = disableCacheMapKey ? JSONTypeDeserializer.parseMapKey(bytes, i, '"', parseContext) :
                        JSONTypeDeserializer.parseMapKeyByCache(bytes, i, '"', parseContext);
                i = parseContext.endIndex;
                empty = false;
                ++i;
            } else {
                ...
                if (b == '\'') {
                    if (parseContext.allowSingleQuotes) {
                        while (i + 1 < parseContext.toIndex && (bytes[++i] != '\'' || bytes[i - 1] == '\\')) {
                            // skip until the end of the single quote
                        }
                        empty = false;
                        ++i;
            ...
            if (b == ':') {
                ...
                switch (b) {
                    ...
                    default: {
                        value = JSONTypeDeserializer.NUMBER.deserialize(source, bytes, i,
                                parseContext.useBigDecimalAsDefault ? GenericParameterizedType.BigDecimalType :
                                        GenericParameterizedType.AnyType, null, END_OBJECT, parseContext);
                        i = parseContext.endIndex;
                        instance.put(key, value);
                        byte next = bytes[++i];
                        if (next == END_OBJECT) {
                ...
                throw new JSONException("Syntax error, at pos " + i + ", context text by '" + errorContextTextAt +
                        "', unexpected token '" + (char) b + "', expected ',' or '}'");
            } else {
                throw new JSONException(
                        "Syntax error, at pos " + i + ", unexpected '" + (char) b + "', colon ':' is expected.");
            }
        }
    }
}
