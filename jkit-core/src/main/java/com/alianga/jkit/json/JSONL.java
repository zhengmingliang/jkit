...
final class JSONL extends JSONGeneral {
    ...
    static List parseNdJson(String json, JSONTypeDeserializer typeDeserializer, ReadOption[] readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            ...
            if (bytes.length == json.length()) {
                return parseNdJson(typeDeserializer, AsciiStringSource.of(json), bytes,
                        JSONParseContext.of(readOptions));
            } else {
                char[] chars = json.toCharArray();
                return parseNdJson(typeDeserializer, UTF16ByteArraySource.of(json), chars,
                        JSONParseContext.of(readOptions));
            }
        }
    ...
    static List parseNdJson(final JSONTypeDeserializer typeDeserializer, CharSource source, byte[] bytes,
        ...
        try {
            while (true) {
                ...
                Object result =
                        typeDeserializer.deserialize(source, bytes, fromIndex, GenericParameterizedType.AnyType, null,
                                ZERO, parseContext);
                results.add(result);
    ...
    static List parseNdJson(final JSONTypeDeserializer typeDeserializer, CharSource source, char[] chars,
                            JSONParseContext parseContext) {
        int fromIndex = 0;
        ...
        try {
            while (true) {
                ...
                Object result =
                        typeDeserializer.deserialize(source, chars, fromIndex, GenericParameterizedType.AnyType, null,
                                ',', parseContext);
                results.add(result);
                fromIndex = parseContext.endIndex + 1;
            }
