...
/**
 * {@link JSONEndian} 的纯 java 位运算实现，不依赖 Unsafe 的批量读写，
 * 按运行环境的本机字节序（{@link ByteOrder#nativeOrder()}）逐字节组装与拆解 short/int/long 数据
 */
public class JSONEndianRawCode extends JSONEndian {
    ...
    public long mergeYearAndMonth(int year, int month) {
        if (LITTLE_ENDIAN) {
            return 0x2d00002d00000000L | (long) JSONWriter.TWO_DIGITS_16_BITS[month] << 40 |
                    JSONWriter.FOUR_DIGITS_32_BITS[year];
        } else {
    ...
    public long mergeHHMMSS(int hour, int minute, int second) {
        if (LITTLE_ENDIAN) {
            short[] TWO_DIGITS_16_BITS = JSONWriter.TWO_DIGITS_16_BITS;
            return 0x00003a00003a0000L | (long) TWO_DIGITS_16_BITS[second] << 48 |
                    (long) TWO_DIGITS_16_BITS[minute] << 24 | TWO_DIGITS_16_BITS[hour];
        } else {
            short[] TWO_DIGITS_16_BITS = JSONWriter.TWO_DIGITS_16_BITS;
            return 0x00003a00003a0000L | (long) TWO_DIGITS_16_BITS[hour] << 48 |
                    (long) TWO_DIGITS_16_BITS[minute] << 24 | TWO_DIGITS_16_BITS[second];
        }
    }
    ...
    public int digits2Bytes(byte[] buf, int offset) {
        if (LITTLE_ENDIAN) {
            ...
            if ((bigShortVal & 0xF0F0) == 0x3030) {
                int l = bigShortVal & 0xf;
                int h = (bigShortVal >> 8) & 0xf;
                if (h > 9 || l > 9) {
                    return -1;
                }
                return (h << 3) + (h << 1) + l;
