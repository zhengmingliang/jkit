...
public class TemporalLocalTimeDeserializer extends JSONTemporalDeserializer {
    ...
    protected Object deserializeTemporal(char[] buf, int fromIndex, int endIndex, JSONParseContext jsonParseContext)
        ...
        return LocalTime.of(generalDate.getHourOfDay(), generalDate.getMinute(), generalDate.getSecond(),
    ...
    @Override
    protected Object deserializeDefault(char[] buf, int offset, char endToken, JSONParseContext jsonParseContext)
            throws Exception {
        int i = offset;
        int hour;
        int minute;
        ...
        throw new JSONException(
                "Syntax error, at pos " + i + ", context text by '" + errorContextTextAt + "', unexpected token '" +
                        c1 + "', expected '" + endToken + "'");
    }

    // default hh:mm:ss.SSS
    @Override
    protected Object deserializeDefault(byte[] buf, int offset, byte endToken, JSONParseContext jsonParseContext)
            throws Exception {
        int i = offset;
        int hour;
        int minute;
