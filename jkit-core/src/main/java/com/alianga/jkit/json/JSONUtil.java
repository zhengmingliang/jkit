...
//        return (((v64 ^ quoteMask) - 0x0001000100010001L) | ((v64 ^ 0x005C005C005C005CL) - 0x0001000100010001L)) &
//        0x8000800080008000L;
//    }
//
...
//    }
    ...
    static final int offsetChars(long result) {
     ...
     */
    static final int indexOfQuoteOrBackslashVector256(char[] buf, int offset, long quoteMask) {
        /*JSONUnsafe.CHAR_ARRAY_OFFSET + ((long) offset << 1)*/
        long v = JSONMemoryHandle.JSON_ENDIAN.getLong(buf, offset);
        long result = (((v ^ quoteMask) - 0x0001000100010001L) | ((v ^ 0x005C005C005C005CL) - 0x0001000100010001L)) &
                0x8000800080008000L;
        if (result != 0) {
            return offsetChars(result);
        }
        v = JSONMemoryHandle.JSON_ENDIAN.getLong(buf, offset + 4);
        result = (((v ^ quoteMask) - 0x0001000100010001L) | ((v ^ 0x005C005C005C005CL) - 0x0001000100010001L)) &
                0x8000800080008000L;
        if (result != 0) {
     ...
     *
     * @param buf       待扫描的字符数组
     * @param offset    开始扫描的下标
     * @param quote     引号
     * @param quoteMask 8个quote字符组成的long值，用于掩码快速匹配
     * @return 从offset开始, 第一个quote或者'\\'的位置，如果找不到将抛出IndexOutOfBoundsException
     * @throws IndexOutOfBoundsException 扫描到数组末尾仍未找到引号或转义符时抛出
     */
    ...
    static final long getQuoteOrBackslashOrUTF8Mask(byte[] buf, int offset, long quoteMask) {
        ...
        return (((v64 ^ quoteMask) - 0x0101010101010101L) | ((v64 ^ 0x5C5C5C5C5C5C5C5CL) - 0x0101010101010101L)) &
                0x8080808080808080L;
    }

    // check: quote or \\(0x5C) or UTF8 begin
    static final long getQuoteOrBackslashOrUTF8Mask(long value, long quoteMask) {
        return (((value ^ quoteMask) - 0x0101010101010101L) | ((value ^ 0x5C5C5C5C5C5C5C5CL) - 0x0101010101010101L)) &
                0x8080808080808080L;
    }
     ...
     */
    static final int indexOfQuoteOrBackslashOrUTF8Vector256(byte[] buf, int offset, long quoteMask) {
        /*JSONUnsafe.BYTE_ARRAY_OFFSET +*/
        long v = JSONMemoryHandle.JSON_ENDIAN.getLong(buf, offset);
        long result = (((v ^ quoteMask) - 0x0101010101010101L) | ((v ^ 0x5C5C5C5C5C5C5C5CL) - 0x0101010101010101L)) &
                0x8080808080808080L;
        if (result != 0) {
            return offsetTokenBytes(result);
        }
        v = JSONMemoryHandle.JSON_ENDIAN.getLong(buf, offset + 8);
        result = (((v ^ quoteMask) - 0x0101010101010101L) | ((v ^ 0x5C5C5C5C5C5C5C5CL) - 0x0101010101010101L)) &
                0x8080808080808080L;
        if (result != 0) {
    ...
    public int ensureIndexOfQuoteOrBackslashOrUTF8Byte(byte[] buf, int offset, int quote, long quoteMask) {
        ...
        if (offset <= limit32) {
            if ((firstPos = indexOfQuoteOrBackslashOrUTF8Vector256(buf, offset, quoteMask)) != 32) {
                ...
                while ((limitFlag = (offset += 32) <= limit32) &&
        ...
        while ((limitFlag = offset <= limit8) &&
                (result = getQuoteOrBackslashOrUTF8Mask(buf, offset, quoteMask)) == 0) {
            offset += 8;
        }
        if (limitFlag) {
...
//     * @throws IndexOutOfBoundsException if not found
//     */
//    public final static int ensureIndexOfQuoteOrBackslashOrUTF8Vector256(byte[] buf, int offset, int quote, long
//    quoteMask) {
//        final int limit32 = buf.length - 32;
//        long result = 0;
//        boolean limitFlag;
...
//                } else {
//                    final int limit8 = buf.length - 8;
//                    while ((limitFlag = offset <= limit8) && (result = getQuoteOrBackslashOrUTF8Mask(buf, offset,
//                    quoteMask)) == 0) {
//                        offset += 8;
//                    }
//                    if (limitFlag) {
...
//    }
     ...
     * scan token
     *
     * @param source     待查找的源字符串
     * @param buf        源字符串对应的字节数组，默认实现未使用
     * @param beginIndex 开始查找的下标
     * @param token      要查找的字符
     * @return 从 {@code beginIndex} 开始首次出现 {@code token} 的下标，未找到时返回 -1
     */
    ...
    public int toNoEscapeOffset(byte[] buf, int offset) {
        if (JSONGeneral.isNoneEscaped16Bytes(JSONMemoryHandle.getLong(buf, offset),
                JSONMemoryHandle.getLong(buf, offset + 8))
                && JSONGeneral.isNoneEscaped16Bytes(JSONMemoryHandle.getLong(buf, offset = offset + 16),
                JSONMemoryHandle.getLong(buf, offset + 8))) {
            offset += 16;
    ...
    static final boolean isNoneEscaped4Chars(char[] buf, int offset) {
        ...
        return (((value + 0x7FE07FE07FE07FE0L) & (value ^ 0xFFDDFFDDFFDDFFDDL) + 0x0001000100010001L &
                (value ^ 0xFFA3FFA3FFA3FFA3L) + 0x0001000100010001L) & 0x8000800080008000L) == 0x8000800080008000L;
    }
    ...
    /**
     * 当前实现是否支持并通过了向量化(jdk.incubator.vector)可用性自检
     *
     * @return 基础实现固定返回 {@code false}，向量化子类实现自检通过时返回 {@code true}
     */
