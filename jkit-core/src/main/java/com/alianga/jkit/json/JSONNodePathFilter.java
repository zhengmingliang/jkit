...
//    }
     ...
     *
     * @param condition 过滤条件表达式源码，如 {@code age > 18 && name == 'zml'}
     * @return 基于该条件表达式的属性级别过滤器
     */
    ...
    /**
     * <p> 使用已解析（或已编译）的表达式对象构建属性级别过滤器；
     * <p> 表达式执行上下文是当前节点映射的map对象；
     * <p> 表达式结果为false或者0时则视为不通过，其他非零非false均代表通过；
     *
     * @param expression 已创建好的表达式对象
     * @return 基于该表达式的属性级别过滤器
     */
...
//    }
     ...
     * @see Expression
     */
    static final class ExpressionFilterImpl extends JSONNodePathFilter {
        ...
        public boolean doFilter(JSONNode node) {
            ...
            Object evalResult = expression.evaluate(context, EVALUATE_ENVIRONMENT);
