...
//    }
    ...
    protected final T deserialize(CharSource charSource, char[] buf, int fromIndex,
        ...
        if (beginChar == '{') {
            ...
            if (jsonParseContext.unMatchedEmptyAsNull && (beginChar == '"' || beginChar == '\'') &&
    ...
    protected final T deserialize(CharSource charSource, byte[] buf, int fromIndex,
        ...
        if (beginByte == '{') {
            ...
            if (jsonParseContext.unMatchedEmptyAsNull && (beginByte == DOUBLE_QUOTATION || beginByte == '\'') &&
    ...
    JSONPojoFieldDeserializer matchFieldDeserializer(CharSource charSource, char[] buf, int offset, int endToken,
    ...
    final Object deserializePojo(CharSource charSource, char[] buf, int fromIndex,
        ...
        for (; ; ) {
            ...
            if (c == '"' || c == '\'') {
                fieldDeserializer = matchFieldDeserializer(charSource, buf, i + 1, c,
                        parseContext); // fieldDeserializerMatcher.matchValue(charSource, buf, i + 1, c, parseContext);
            ...
            if (c == ':') {
                                ...
                                if (className == null) {
                                    ...
                                    try {
                                        ...
                                        if (fieldDeserializer.isAvailableImpl(cls)) {
                                            ...
                                            JSONPojoDeserializer fieldPojoDeserializer =
                                                    (JSONPojoDeserializer) deserializer;
                                            i = parseContext.endIndex;
                                            while ((c = buf[++i]) <= ' ') {
                                                // skip
                                            }
                                            Object value;
                                            if (c == ',') {
                                                value = fieldPojoDeserializer.deserializePojo(charSource, buf, i,
                                                        valueType, null, parseContext);
                                                i = parseContext.endIndex;
                                            } else if (c == '}') {
                ...
                if (isDeserialize) {
                    Object value = deserializer.deserialize(charSource, buf, i, valueType, defaultFieldValue, '}',
                            parseContext);
                    setFieldValue((T) entity, fieldDeserializer, value);
                } else {
    ...
    JSONPojoFieldDeserializer matchFieldDeserializer(CharSource charSource, byte[] buf, int offset, int endToken,
                                                     JSONParseContext parseContext) {
        return fieldDeserializerMatcher.matchValue(charSource, buf, offset, endToken, parseContext);
    }
...
//                fieldDeserializer = fieldDeserializerMatcher.matchValue(charSource, buf, i + 1, c, parseContext);
            ...
            if (c == COLON_SIGN) {
                    ...
                    if (deserializer != null) {
                        ...
                        if ((implClass = fieldDeserializer.getImplClass()) != null) {
                            ...
                            if (c == '{') {
                                ...
                                Object value =
                                        ANY.deserialize(charSource, buf, i, null, null, (byte) '}', parseContext);
                                if (fieldDeserializer.isInstance(value)) {
                                    setFieldValue((T) entity, fieldDeserializer, value);
                                    i = parseContext.endIndex;
                                    while ((c = buf[++i]) <= ' ') {
                ...
                if (isDeserialize) {
                    Object value =
                            deserializer.deserialize(charSource, buf, i, valueType, defaultFieldValue, END_OBJECT,
                                    parseContext);
                    setFieldValue((T) entity, fieldDeserializer, value);
                } else {
    ...
    private GenericParameterizedType<?> getGenericValueType(GenericParameterizedType<?> parameterizedType,
    ...
    static JSONPojoDefaultDeserializer create(JSONPojoStructure pojoStructure) {
        if (pojoStructure.fieldDeserializerMatcher.isPlhv()) {
            final JSONKeyValueMap<JSONPojoFieldDeserializer> valueMap =
            ...
            return new JSONPojoDefaultDeserializer(pojoStructure) {
                @Override
                JSONPojoFieldDeserializer matchFieldDeserializer(CharSource charSource, byte[] buf, int offset,
                                                                 int endToken, JSONParseContext parseContext) {
                    byte c;
                    int i = offset;
                    if ((c = buf[i]) != endToken) {
                        long hashValue = c;
                ...
                @Override
                JSONPojoFieldDeserializer matchFieldDeserializer(CharSource charSource, char[] buf, int offset,
                                                                 int endToken, JSONParseContext parseContext) {
                    char c;
                    int i = offset;
                    if ((c = buf[i]) != endToken) {
     ...
     * 将反序列化得到的值写入实体对应的字段
     *
     * @param entity 待赋值的实体对象
     * @param fieldDeserializer 字段反序列化器，提供目标字段的 setter 信息
     * @param value 待写入的字段值
     */
    protected void setFieldValue(T entity, JSONPojoFieldDeserializer fieldDeserializer, Object value) {
        JSON_SECURE_TRUSTED_ACCESS.set(fieldDeserializer.setterInfo, entity,
                value); // fieldDeserializer.setterInfo.invoke(entity, value);
    }
    ...
    // record supported
    /**
     * record 类型的反序列化实现：先将各字段值收集到构造参数数组中，最后通过全参构造器创建实例
     *
     * @param <T> 反序列化生成的 record 类型
     */
