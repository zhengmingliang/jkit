...
import com.alianga.jkit.json.internal.beans.KeyValuePair;
 ...
 * JSON 序列化与反序列化的实例入口。
 *
 * <p> 每个实例持有独立的 {@link JSONStore}（序列化器、反序列化器、类型映射器与模块注册表），
 * 因此可以针对不同场景注册各自的类型映射而互不影响；
 * 通过 {@link #create()} 创建的实例与全局默认实例相互隔离。
 */
public final class JSONInstance {
    ...
    /**
     * 创建实例，内部使用独立的存储（注册表），并完成默认模块与类型映射的初始化。
     */
    public JSONInstance() {
        this(new JSONStore());
    }
     ...
     * 创建一个使用独立注册表的实例。
     *
     * @return 新创建的实例，其注册的序列化器与模块不会影响其他实例
     */
        ...
        if (EnvUtils.JDK_8_PLUS) {
            try {
                if (JAVA_TIME_MODULE == null) {
                    Class<JSONTypeModule> moduleClass =
                            (Class<JSONTypeModule>) Class.forName("com.alianga.jkit.json.JSONTypeModuleJavaTime");
     ...
     */
    public Collection<?> parseCollection(String json, Class<? extends Collection> collectionCls,
    ...
    public Object parse(byte[] bytes, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            if (!EnvUtils.JDK_AGENT_INSTANCE.hasNegatives(bytes, 0, bytes.length)) {
                return store.parser.parseInternal(AsciiStringSource.of(JSONMemoryHandle.createAsciiString(bytes)),
                        bytes, 0, bytes.length, null, readOptions);
            } else {
                return store.parser.parseInternal(UTF8CharSource.of(JSONMemoryHandle.createAsciiString(bytes)), bytes,
                        0, bytes.length, null, readOptions);
            }
        }
    ...
    public Object parse(byte[] bytes, final JSONMapHandler handler, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            if (!EnvUtils.JDK_AGENT_INSTANCE.hasNegatives(bytes, 0, bytes.length)) {
                return store.parser.parseInternal(AsciiStringSource.of(JSONMemoryHandle.createAsciiString(bytes)),
                        bytes, 0, bytes.length, null, createContext(handler, readOptions));
            } else {
                return store.parser.parseInternal(UTF8CharSource.of(JSONMemoryHandle.createAsciiString(bytes)), bytes,
                        0, bytes.length, null, createContext(handler, readOptions));
            }
        }
    ...
    public Object parse(byte[] buf, Class<?> actualType, ReadOption... readOptions) {
        if (EnvUtils.JDK_9_PLUS) {
            if (!EnvUtils.JDK_AGENT_INSTANCE.hasNegatives(buf, 0, buf.length)) {
                return parseInternal(AsciiStringSource.of(JSONMemoryHandle.createAsciiString(buf)), buf, actualType,
                        readOptions);
            } else {
                return parseInternal(UTF8CharSource.of(JSONMemoryHandle.createAsciiString(buf)), buf, actualType,
                        readOptions);
            }
        }
    ...
    // 基于偏移位置和长度解析
    private Object parseInternal(final CharSource charSource, char[] buf, int fromIndex, int toIndex,
                                 final Class<?> actualType, ReadOption... readOptions) {
        return deserialize(buf, fromIndex, toIndex, new Deserializer() {
            Object deserialize(char[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                ...
                if (beginChar == '[') {
                    return store.COLLECTION_DESER.deserializeCollection(charSource, buf, fromIndex,
                            GenericParameterizedType.collectionType(ArrayList.class, actualType), new ArrayList(),
                            jsonParseContext);
                }
     ...
     * @return 对象
     */
    private Object parseInternal(final CharSource charSource, byte[] buf, final Class<?> actualType,
                                 ReadOption... readOptions) {
        return deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(byte[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                ...
                JSONTypeDeserializer typeDeserializer = store.getTypeDeserializer(actualType);
                GenericParameterizedType<?> type = typeDeserializer.getGenericParameterizedType(actualType);
                return typeDeserializer.deserialize(charSource, buf, fromIndex, type, null, (byte) '\0',
                        jsonParseContext);
            }
        }, readOptions);
    }

    private Object deserialize(char[] buf, int fromIndex, int toIndex, Deserializer deserializer,
                               ReadOption... readOptions) {
        ...
        try {
            ...
            if (endIndex != toIndex - 1) {
                ...
                throw new JSONException("Syntax error, at pos " + endIndex + " extra characters found, '" +
                        new String(buf, endIndex, wordNum) + " ...'");
            }
     ...
     * @return 结果
     */
    private Object deserialize(byte[] buf, int fromIndex, int toIndex, Deserializer deserializer,
                               ReadOption... readOptions) {
        JSONParseContext parseContext = JSONParseContext.of(readOptions);
        try {
            byte beginByte;
    ...
    public <T> T parseObject(String json, Class<T> actualType, ReadOption... readOptions) {
        ...
        return parseObjectInternal(deserializer, null, (char[]) JSONMemoryHandle.getStringValue(json), actualType,
                readOptions);
    }

    <T> T parseObjectInternal(final JSONTypeDeserializer deserializer, final CharSource charSource, char[] buf,
                              final Class<T> actualType, ReadOption... readOptions) {
        return (T) deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(char[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                GenericParameterizedType<?> genericParameterizedType =
                        deserializer.getGenericParameterizedType(actualType);
                return deserializer.deserialize(charSource, buf, fromIndex, genericParameterizedType, null, '\0',
                        jsonParseContext);
            }
        }, readOptions);
    }

    <T> T parseObjectInternal(final JSONTypeDeserializer deserializer, final CharSource charSource, byte[] buf,
                              final Class<T> actualType, ReadOption... readOptions) {
        return (T) deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(byte[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                GenericParameterizedType<?> genericParameterizedType =
                        deserializer.getGenericParameterizedType(actualType);
                return deserializer.deserialize(charSource, buf, fromIndex, genericParameterizedType, null,
                        JSONGeneral.ZERO, jsonParseContext);
            }
        }, readOptions);
    }
    ...
    public <T> T parseObject(byte[] buf, final Class<T> actualType, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            if (!EnvUtils.JDK_AGENT_INSTANCE.hasNegatives(buf, 0, buf.length)) {
                return parseObjectInternal(typeDeserializer,
                        AsciiStringSource.of(JSONMemoryHandle.createAsciiString(buf)), buf, actualType, readOptions);
            } else {
                return parseObjectInternal(typeDeserializer, UTF8CharSource.of(JSONMemoryHandle.createAsciiString(buf)),
                        buf, actualType, readOptions);
            }
        }
    ...
    public <T> T parseObject(char[] buf, final Class<T> actualType, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            ...
            if (bytes.length == buf.length) {
                ...
                // utf16
                return parseObjectInternal(typeDeserializer, UTF16ByteArraySource.of(json), buf, actualType,
                        readOptions);
            }
        }
        return parseObjectInternal(typeDeserializer, null, buf, actualType, readOptions);
    ...
    public <T> T parse(String json, GenericParameterizedType<T> genericParameterizedType, ReadOption... readOptions) {
        ...
        if (EnvUtils.JDK_9_PLUS) {
            ...
            return parseInternal(null, (char[]) JSONMemoryHandle.getStringValue(json), genericParameterizedType,
                    readOptions);
        }
    }
     ...
     * @return 对象或者数组
     */
    private <T> T parseInternal(final CharSource charSource, char[] buf,
                                final GenericParameterizedType<T> genericParameterizedType, ReadOption... readOptions) {
        return (T) deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(char[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
     ...
     * @return 对象或者数组
     */
    private <T> T parseInternal(final CharSource charSource, byte[] buf,
                                final GenericParameterizedType<T> genericParameterizedType, ReadOption... readOptions) {
        return (T) deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(byte[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                JSONTypeDeserializer deserializer = store.getTypeDeserializer(genericParameterizedType.getActualType());
     ...
     * @return 对象或者数组
     */
    public <T> T parse(byte[] buf, final GenericParameterizedType<T> genericParameterizedType,
                       ReadOption... readOptions) {
        if (EnvUtils.JDK_9_PLUS) {
            if (!EnvUtils.JDK_AGENT_INSTANCE.hasNegatives(buf, 0, buf.length)) {
                return parseInternal(AsciiStringSource.of(JSONMemoryHandle.createAsciiString(buf)), buf,
                        genericParameterizedType, readOptions);
            } else {
                return parseInternal(UTF8CharSource.of(JSONMemoryHandle.createAsciiString(buf)), buf,
                        genericParameterizedType, readOptions);
            }
        }
     ...
     * @return 对象或者数组
     */
    public <T> T parse(char[] buf, final GenericParameterizedType<T> genericParameterizedType,
                       ReadOption... readOptions) {
        if (EnvUtils.JDK_9_PLUS) {
            String json = new String(buf);
            byte[] bytes = (byte[]) JSONMemoryHandle.getStringValue(json);
     ...
     * @return 集合
     */
    private <T> List<T> parseArrayInternal(final CharSource charSource, char[] buf, final Class<T> actualType,
                                           ReadOption... readOptions) {
        return (List<T>) deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(char[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                return store.COLLECTION_DESER.deserialize(charSource, buf, fromIndex,
                        GenericParameterizedType.collectionType(ArrayList.class, actualType), null, '\0',
                        jsonParseContext);
            }
        }, readOptions);
    }
     ...
     * @return 集合
     */
    private <T> List<T> parseArrayInternal(final CharSource charSource, byte[] buf, final Class<T> actualType,
                                           ReadOption... readOptions) {
        return (List<T>) deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(byte[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                return store.COLLECTION_DESER.deserialize(charSource, buf, fromIndex,
                        GenericParameterizedType.collectionType(ArrayList.class, actualType), null, JSONGeneral.ZERO,
                        jsonParseContext);
            }
        }, readOptions);
    }
    ...
    public <T> List<T> parseArray(byte[] buf, final Class<T> actualType, ReadOption... readOptions) {
        if (EnvUtils.JDK_9_PLUS) {
            if (!EnvUtils.JDK_AGENT_INSTANCE.hasNegatives(buf, 0, buf.length)) {
                return parseArrayInternal(AsciiStringSource.of(JSONMemoryHandle.createAsciiString(buf)), buf,
                        actualType, readOptions);
            } else {
                return parseArrayInternal(UTF8CharSource.of(JSONMemoryHandle.createAsciiString(buf)), buf, actualType,
                        readOptions);
            }
        }
        return parseArrayInternal(null, buf, actualType, readOptions);
    ...
    public List<?> parseNdJson(InputStream is, ReadOption... readOptions) {
        return JSONL.parseNdJson(IOUtils.is2String(is), store.ANY_DESER, readOptions);
    }
    ...
    private Object parseToObjectInternal(final CharSource charSource, char[] buf, final Object instance,
                                         ReadOption... readOptions) {
        return deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(char[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                if (instance instanceof Map) {
                    return store.MAP_DESER.deserialize(charSource, buf, fromIndex, GenericParameterizedType.DefaultMap,
                            instance, '\0', jsonParseContext);
                }
    ...
    private Object parseToObjectInternal(final CharSource charSource, byte[] buf, final Object instance,
                                         ReadOption... readOptions) {
        return deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(byte[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                if (instance instanceof Map) {
                    return store.MAP_DESER.deserialize(charSource, buf, fromIndex, GenericParameterizedType.DefaultMap,
                            instance, JSONGeneral.ZERO, jsonParseContext);
                }
                Class<?> actualType = instance.getClass();
                JSONTypeDeserializer deserializer = store.getTypeDeserializer(actualType);
                GenericParameterizedType<?> type = deserializer.getGenericParameterizedType(actualType);
                return deserializer.deserialize(charSource, buf, fromIndex, type, instance, JSONGeneral.ZERO,
                        jsonParseContext);
            }
        }, readOptions);
    }
     ...
     * @return 集合
     */
    public <E> Object parseToList(String json, final Collection<?> instance, final Class<E> actualType,
                                  ReadOption... readOptions) {
        if (instance == null || json == null) {
            return null;
        }
        if (EnvUtils.JDK_9_PLUS) {
            ...
            if (json.length() == bytes.length) {
                ...
                // utf16
                return parseToListInternal(UTF16ByteArraySource.of(json), json.toCharArray(), instance, actualType,
                        readOptions);
            }
        }
        return parseToListInternal(null, (char[]) JSONMemoryHandle.getStringValue(json), instance, actualType,
                readOptions);
    }

    private <E> Object parseToListInternal(final CharSource charSource, char[] buf, final Collection<?> instance,
                                           final Class<E> actualType, ReadOption... readOptions) {
        return deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(char[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                return store.COLLECTION_DESER.deserializeCollection(charSource, buf, fromIndex,
                        GenericParameterizedType.collectionType(instance.getClass(), actualType), instance,
                        jsonParseContext);
            }
        }, readOptions);
    }

    private <E> Object parseToListInternal(final CharSource charSource, byte[] buf, final Collection<?> instance,
                                           final Class<E> actualType, ReadOption... readOptions) {
        return deserialize(buf, 0, buf.length, new Deserializer() {
            Object deserialize(byte[] buf, int fromIndex, JSONParseContext jsonParseContext) throws Exception {
                return store.COLLECTION_DESER.deserializeCollection(charSource, buf, fromIndex,
     ...
     * @param readOptions 解析配置项
     * @return T对象
     * @throws IOException 建立连接或读取远程资源失败时抛出
     */
     ...
     * @throws IOException 建立连接或读取远程资源失败时抛出
     */
    public <T> T read(URL url, Class<T> actualType, boolean forceStreamMode, int timeout, ReadOption... readOptions)
            throws IOException {
        ...
        conn.connect();
        return read(conn.getInputStream(), forceStreamMode ? Integer.MAX_VALUE : conn.getContentLength(), actualType,
                readOptions);
    }
     ...
     * @throws IOException 文件不存在或读取失败时抛出
     */
    public <T> T read(File file, GenericParameterizedType<T> genericType, ReadOption... readOptions)
            throws IOException {
     ...
     * @throws IOException 读取流失败时抛出
     */
    public <T> T read(InputStream is, GenericParameterizedType<T> genericType, ReadOption... readOptions)
            throws IOException {
     ...
     *
     * @param obj 源对象
     * @return 美化（换行缩进、冒号后补空格）后的json字符串；对象为 {@code null} 时返回 {@code null}
     */
     ...
     *
     * @param json json字符串
     * @return 美化（换行缩进、冒号后补空格）后的json字符串；入参为 {@code null} 时返回 {@code null}
     */
     ...
     * @param obj     目标对象
     * @param options 配置
     * @return UTF-8 编码的json字节数组；对象为 {@code null} 时返回 {@code null}
     */
     ...
     * @param charset 指定编码
     * @param options 配置
     * @return 按指定编码序列化的json字节数组；对象为 {@code null} 时返回 {@code null}
     */
    ...
    public void writeSuperLargeJsonTo(Object object, OutputStream os, WriteOption... options) {
        writeToJSONWriter(object, JSONWriter.wrap(new OutputStreamWriter(os, EnvUtils.CHARSET_DEFAULT)),
                JSONConfig.of(options));
    }
    ...
    /**
     * 模块注册
     * @param moduleClass 模块类型，内部会创建其实例并注册
     */
     ...
     * @return 当前对象，便于链式调用；运行环境低于 JDK16 时不做任何切换
     */
