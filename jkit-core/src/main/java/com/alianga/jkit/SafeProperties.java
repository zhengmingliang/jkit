...
/**
 * Created by 郑明亮 on 2017/11/17 11:05.
 *
 * @author 郑明亮 @email 1072307340@qq.com
 * @version 1.0
 * @time 2017/11/17 11:05
 */
...
public class SafeProperties extends Properties {
    ...
    /** 记录注释行与键值对原始顺序的上下文，用于回写时保留文件原貌。 */
    private final PropertiesContext context = new PropertiesContext();

    /**
     * 获取记录注释与键值顺序的上下文对象。
     *
     * @return 当前实例持有的属性上下文，不会为 {@code null}
     */
    ...
    public synchronized void load(InputStream inStream) throws IOException {
        BufferedReader in = new BufferedReader(new InputStreamReader(inStream, StandardCharsets.ISO_8859_1));
        while (true) {
            // Get next line
    ...
    public synchronized void store(OutputStream out, String header) throws IOException {
        BufferedWriter awriter = new BufferedWriter(new OutputStreamWriter(out, StandardCharsets.ISO_8859_1));
        if (header != null) {
    ...
    /**
     * 写入属性并同时记录该属性在文件中的原始行内容，输出时将原样回写该行。
     *
     * @param key   属性键
     * @param value 属性值
     * @param line  该属性在原文件中的完整行内容
     * @return 该键之前关联的值，此前不存在时返回 {@code null}
     */
    ...
    /**
     * 在当前内容末尾追加一行注释，写出时会自动补上 {@code #} 前缀。
     *
     * @param comment 注释内容，为 {@code null} 时不做任何处理
     */
