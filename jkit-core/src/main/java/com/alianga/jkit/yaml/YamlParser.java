 ...
 * YAML 文本解析器的基础实现。
 *
 * <p>负责按行扫描源文本，并生成 {@link YamlNode} 节点列表。</p>
 */
class YamlParser extends YamlGeneral {
    ...
    /**
     * 多文档解析的根节点入口，由 {@link YamlDocument} 实现。
     *
     * @param i 本次解析的起始字符下标
     */
     ...
     * 解析指定范围内的 YAML 节点。
     *
     * @param yamlNodes 用于接收解析结果的节点列表
     * @param offset 起始字符下标
     * @param toIndex 结束字符下标（不包含）
     */
        ...
        for (int i = offset; i < toIndex; i++) {
            ...
            if (ch == '\t') {
                throw new YamlParseException(
                        "YAML indent must use spaces, tab found at line " + (lineNum + 1));
            }
            // 注释行跳过处理
            if (ch == '#') {
                // 去除注释行
...
//            YamlNode yamlNode = new YamlNode();
            ...
            if (rootIndent == -1) {
                ...
                if (indent < rootIndent) {
                    throw new YamlParseException(
                            "indent value " + indent + " error, cannot less then the root indent " + rootIndent +
            ...
            if (ch == '-') {
                ...
                if (isAppendSpaceChar || isNewLineChar(append)) {
                    ...
                    while (isAppendSpaceChar) {
                        ...
                        if (ch == '-') {
                            ...
                            if (isAppendSpaceChar || isNewLineChar(append)) {
                                ...
                                addYamlNode(yamlNodes, indent, lineIndex, null, null, valueType, null, false, false,
                                        true, anchorKey, referenceKey);
                                // 更新下一个indent
                                indent += j - i + 1;
                                j++;
                                i = j;
                    ...
                    if (splitIndex > -1) {
                        ...
                        // 添加Node
                        addYamlNode(yamlNodes, indent, lineIndex, null, null, valueType, null, false, false, true,
                                anchorKey, referenceKey);
                        // 更新indent
                        indent += n + 1;
                        // 下一个键值对节点解析
                        arrayToken = false;
            ...
            while (!isNewLineChar(ch = source[i])) {
                ...
                if (ch == '#') {
                    ...
                    switch (ch) {
                        ...
                        default:
                            throw new RuntimeException("expected chomping or indentation indicators, but found " + ch +
                                    " in 'string', line " + (lineNum + 1) + ", column n:");
                    }

                    // 添加到line
                    YamlLine blockYamlNode =
                            addYamlNode(yamlNodes, indent, lineIndex, key, value, valueType, typeOfValue, leaf,
                                    textBlock, arrayToken, anchorKey, referenceKey);
                    ...
                    if (anchorKey == null) {
                        ...
                        if (anchorAppend == SPACE_CHAR || isNewLineChar(anchorAppend)) {
                            throw new YamlParseException(
                                    "anchor token '&' cannot be followed by ' ' and '\\n', line " + lineNum);
                        }
                        i++;
                        j = i;
                        while ((ch = source[++i]) != SPACE_CHAR && !isNewLineChar(ch)) {
                    ...
                    if (referenceAppend == SPACE_CHAR || isNewLineChar(referenceAppend)) {
                        throw new YamlParseException(
                                "reference token '*' cannot be followed by ' ' and '\\n', line " + lineNum);
                    }
                    i++;
                    j = i;
                    while ((ch = source[++i]) != SPACE_CHAR && !isNewLineChar(ch)) {
                    ...
                    if (ch == SPACE_CHAR) {
                        ...
                        if (ch != '#' && !isNewLineChar(ch)) {
                            throw new YamlParseException("reference token '*" + referenceKey +
                                    "' cannot be followed by any value character, line " + lineNum);
                        }
                        // 清除注释
                        if (ch == '#') {
                            // 注释内容
                    ...
                    if (valueType == 0) {
                        ...
                        if (typeAppend != '!') {
                            valueType = 1;
                            continue;
//                            throw new YamlParseException("'!' must be followed by '!' but found '" + typeAppend +
//                            "', line " + lineNum);
                        }
                        i++;
                        i++;
                        ...
                        if (ch == SPACE_CHAR) {
                            ...
                            if (typeValues.containsKey(typeName)) {
                                ...
                                throw new YamlParseException(
                                        "Mandatory type '!!" + typeName + "' is not supported, line " + lineNum);
                            }
                        } else {
                            throw new YamlParseException(
                                    "token '!!' must be followed by one type such as int, str, bool ...,  but empty, " +
                                            "line " +
                                            lineNum);
                        }
                    }
            ...
            // 添加到list
            addYamlNode(yamlNodes, indent, lineIndex, key, value, valueType, typeOfValue, leaf, textBlock, arrayToken,
                    anchorKey, referenceKey);
        }
    }
    ...
    private YamlNode addYamlNode(List<YamlNode> yamlNodes, int indent, int lineNum, String key, String value,
                                 int valueType, Object typeOfValue, boolean leaf, boolean textBlock, boolean arrayToken,
                                 String anchorKey, String referenceKey) {
        YamlNode yamlNode = new YamlNode();
