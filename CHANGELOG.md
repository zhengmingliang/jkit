...
- `jkit-sql`：分页 API `SQL.getLimit` / `getOffset` / `setLimit` / `setOffset` / `setPage`（clone-then-mutate；MySQL/PG LIMIT-OFFSET、SQL Server TOP/OFFSET FETCH、Oracle FETCH FIRST）；`SqlDialect` 能力矩阵与别名；AST `toString()` 输出紧凑 SQL。
