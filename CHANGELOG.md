...
- `jkit-sql`：**覆盖率提升第三轮**（druid-bvt-inline 89.0→90.1%、jsqlparser-inline 77.7→78.3%、jsqlparser-files 73.9→74.3%，jkitGaps 1040→956）——MySQL SELECT 修饰符链（`STRAIGHT_JOIN`/`SQL_SMALL_RESULT`/`SQL_BIG_RESULT`/`SQL_BUFFER_RESULT`/`SQL_CACHE`/`SQL_NO_CACHE`/`DISTINCTROW`，新 API 同名字段）；`replace(...)` 语句起始按函数调用解析（不再误路由 REPLACE INTO）；MySQL 多表删除第二形式 `DELETE FROM a1, a2 USING …`（`SqlDelete.targets`）；字符集前缀字面量 `_latin1'x'`（`SqlLiteral.name` 存前缀）；`NOT REGEXP`（`SqlBinaryOp.NOT_REGEXP`）；模板占位 `#{}` 命中不再被 `#` 行注释吞掉（lexer 占位优先）；JDBC/ODBC 转义 `{fn …}`/`{d|t|ts '…'}`/`{oj …}`/`{call …}`/`{escape …}` 解包；MODEL MEASURES 裸别名（`sale s`）；`UNPIVOT INCLUDE|EXCLUDE NULLS`（`SqlPivotTable.nullsClause`）；`GROUP BY [DISTINCT] a GROUPING SETS(…)`（`groupByDistinct` + 列表与扩展共存回写）；Oracle `fn@dblink` / `t@dblink`（`SqlIdentifier.dblink`）；CTAS 尾缀 `WITH [NO] DATA`（query+tail 回写）。
- `jkit-sql`：**覆盖率提升第二轮**（druid-bvt-inline 87.6→89.0%、jsqlparser-inline 76.7→77.7%、jsqlparser-files 73.7→73.9%，jkitGaps 1158→1040）——位置游离的 optimizer hint 统一吸收（WHERE/AND 中的 `/*+TDDL:MASTER*/` 等约 96 条，表达式层暂存、语句层挂到 SELECT）；Hive `INSERT OVERWRITE [TABLE] t [PARTITION (...)]`（`SqlInsert.overwrite/tableKeyword/partitionRaw`）；`CONNECT BY NOCYCLE`（`SqlSelect.connectByNocycle`）；`GROUP BY … WITH CUBE`（`groupByCube`，修掉 WITH 被误吞）；Teradata/Snowflake `QUALIFY` 窗口过滤（`SqlSelect.qualify`，`QUALIFY` 加入别名停用词防误当表别名）；MySQL 8 函数索引 `ADD KEY idx ((expr))`；XML 系函数（`XMLSERIALIZE`/`XMLPARSE`/`XMLROOT`/`XMLAGG`/`XMLELEMENT`/`XMLFOREST`/`EXTRACTVALUE` 原文保留）；Spark `OVER (DISTRIBUTE BY … SORT BY …)`（`SqlOverExpr.sparkStyle`，修掉 DISTRIBUTE 被误当继承窗口名）。
